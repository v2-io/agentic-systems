...
*Naming note:* the mathematical core was previously called Agentic Cycle Theory (ACT) and was renamed on 2026-04-16 to resolve a collision with "AI Consciousness Test" (Schneider & Turner) in AI welfare literature. See `msc/naming/name-transition-aad.md` for the rationale and transition record.
...
- `mono/common/` — **Per-volume build infrastructure (kaobook target).** `main.tex` (LaTeX entrypoint template), `preamble/` (LaTeX preamble fragments), `lib/` (Ruby modules: `outline_walker`, `segment_renderer`, `ingest`, `assemble`, `typeset`, `typeset_scrbook`), `vendor/kaobook/`. The markdown-first pipeline (`bin/build-monograph`) runs three stages — ingest (`OUTLINE.md` + segments → `index.md` + chunks), assemble (chunks + index → canonical per-volume `mono/<slug>-v<sem>.md`), typeset (assembled markdown → `body.tex` → PDF). See [`msc/markdown-first-pipeline.md`](msc/markdown-first-pipeline.md) for the architectural commitments and chunk-format contract; see [`FORMAT-TODO.md`](FORMAT-TODO.md) for current state and remaining work. **Build-pipeline vocabulary** (Joseph 2026-05-12): "narrow-area" = anywhere the Tufte-style wide right margin is in play (body column + free margin column); "wide-area" = anywhere the text spans the full segment band (Discussion / Findings via the `segmentwidesection` wrapper, with both page margins equal). Tables, working-notes boxes, and segment-header rules all interact with this distinction.
- `mono/common-scrbook/` — **Sister build target (vanilla KOMA scrbook).** `main.tex` + `preamble/` for a classical math-monograph rendering of the same per-volume assembled markdown. Reuses Stages 1+2 of the markdown-first pipeline; only Stage 3 differs (`mono/common/lib/typeset_scrbook.rb` defines `Mono::TypesetScrbook` + `Kramdown::Converter::AsfScrbookLatex`). Driver script: `bin/build-scrbook`. Output: `mono/<slug>-v<sem>-scrbook.pdf` parallel to the kaobook artifact.
...
- `audits/` — **Audit-cycle final outputs only.** Everything in `audits/` is consumable as a FINAL deliverable; intermediates live in `msc/AUDIT-WORKING-NNNNNN/` (separate dir per audit cycle). Naming pattern for cycle-bound finals: `audits/audit-NNNNNN-FINAL[-suffix].md` and `audits/audit-NNNNNN-SUPPLEMENT-{topic}.md` (cycle-id prefix disambiguates across the corpus). Older standalone audit reports preserved with their original names (`audits/audit-*.md`, `audits/audits-*.md`, `audits/analysis-*.md`, `audits/feedback-*.md`, `audits/opus-*.md`); the cycle-id-prefix discipline applies to new audits going forward. `audits/pending-findings-YYYY-MM-DD.md` carries original audit-finding characterizations and resolution trails. `audits/RECOMMENDED-FORMAT-2026-04-28.md` holds a draft format-standardization recommendation pending Joseph's decisions on §5 open questions. Established 2026-04-28; AUDIT-WORKING-* dirs lifted to `msc/` and FINALs lifted out per the new convention.
- `msc/` — **Other working artifacts** (the non-final residue of session work). Currently: `brainstorm-findings.md` (permanent workspace for findings-cycle scratch), `FINDINGS-RANKED-DRAFT.md` (curated findings catalog), `judgment-calls-readme-cycle-*.md` (per-cycle session notes), `llm-causal-access-note.md` (working note), `working-composition-admissibility.md` (active brainstorming for composition-closure), `2026-03-14-section-iv-paper-outline.md` (pre-AAD paper draft archaeology that still references current segments), the `AUDIT-WORKING-NNNNNN/` per-cycle audit-intermediate workspaces (auditor's lowercase-named predictions / per-segment reflections / running outline; FINAL outputs from these cycles live in `audits/`), and the `naming/` and `reflections/` subdirectories.
- `msc/naming/` — **Current naming-cycle artifacts.** Aggregates (`naming-aggregate-{review,round2,votes.json}`), cleanup scans, alias clusters, brainstorm, pilot rename plan, the original Round-1 vote files under `naming-votes/`, and `name-transition-aad.md` (ACT→AAD rename rationale). The `bin/rename-slug` exclusion glob skips this entire subdirectory so the rename-mapping content survives sweeps verbatim.
