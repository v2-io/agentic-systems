...
- **2026-04-23 Gemini Gap A/B** (103 → 109): six new segments (`#contraction-template`, `#strategic-composition`, `#fisher-whitened-update-rule`, `#l1-update-bias`, `#variational-sector-condition`, `#agent-opacity`) plus the **(PI) parameterization-invariance axiom** in `#scope-agent-identity` and explicit **monotone-operator-theory lineage** acknowledgment.
- **2026-04-24 Gemini pressure-point Tier 1** (109 → 110): six Tier 1 items across the `#strategy-cost-regret-bound`, `#composition-closure`, `#contraction-template`, `#sector-condition-derivation`, `#information-bottleneck`, `#loop-interventional-access` segments, plus the new `#bias-bound-derivation` appendix lifting the Class-2 bias bound from "order-of-magnitude guidance" to "conditional theorem" under two explicit tracks.
...
- **Slug role-prefix mapping.** `bin/align-slug` reads the segment's `type:` and uses it as the slug prefix, except where the `TYPE_TO_PREFIX` constant near the top of the script collapses long type tokens. Current mapping: `worked-example → example` (so `worked-example-kalman` would align to `example-kalman`). To adjust the project-wide mapping, edit that constant and re-run `bin/align-slug --all` — the single source of truth.
