...
**Is $C$ a fifth instance of `#discussion-identifiability-floor`?**
...
**M3 — Candidate fifth `#discussion-identifiability-floor` instance.** The no-go for universal $C$ under non-(PI) norms (Attempt E) is a candidate Instance-4 for `#discussion-identifiability-floor`. The floor is: without a canonical metric on $\mathcal M$, no universal information-to-distance constant exists. The unique escape is (PI) + Čencov-forced Fisher-Rao. Composition with existing Instance 3 (`#agent-opacity` candidate) would complete a four-instance meta-segment pattern.
...
| Candidate fifth instance of `#discussion-identifiability-floor` | **Hypothesis** | Structural fit looks clean but no explicit promotion yet |
