...
The segment heavily references `#deriv-edge-credence-dynamics` (Props B.1-B.7), which seems to be the mathematical appendix where these update rules are formally proven to satisfy the sector condition. I also see references to `#der-observability-dominance`. I predict the next main-line segment will be `#der-observability-dominance`, formalizing the claim made here that unobservable edges cannot be updated.
