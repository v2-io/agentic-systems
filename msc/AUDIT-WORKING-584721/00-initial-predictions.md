...
I have *not* read: `PROPOSALS.md`, `spikes/INDEX.md`, `audits/pending-findings-*.md`, `msc/architectural-proposals-*.md`, prior `audits/audit-*.md` files, `spikes/spike-*.md` files, any segment file, `bin/` source, ref/ contents.
...
Two ways to resolve:
- **(a)** Accept that auto-load priming is structural, and the "de-novo" framing applies only to the discretionary sources (TODO, PROPOSALS, spikes/spike-*, audits/audit-*). This is what I'm going to do for this session, but it lowers the de-novo bar.
