...
**Epistemic status of this result: conditional (sketch).** The proof above is a sketch because it elides the discrete-time/continuous-time distinction (the Lyapunov argument is in continuous time; closure defect is defined per-step in discrete time) and because the contraction of $f_c$ applied to trajectory error requires the correction to contract error states as well as mismatch states (these are the same function applied to different arguments, but the argument types differ). Making this fully rigorous requires specifying whether the macro-update is a contraction mapping on the full state space or only on the mismatch subspace. I believe the argument goes through but the technical details need work.
