...
**(Q1) What structural form must the macro-dynamics have?** Must $(\pi_c, E_c, f_c)$ decompose into the same components as a single AAT agent? Or can it be any dynamical system that happens to predict the micro-behavior?

**(Q2) What properties must the macro-quantities satisfy?** Must the macro-state decompose as $(M_c, G_c)$? Must the macro-update be recursive? Must directed separation hold? Each AAT result depends on specific properties — which are required?
...
**Statistical mechanics → thermodynamics.** A thermodynamic description is admissible when the system has well-defined intensive variables (temperature, pressure) and equations of state relating them. Not every microscopic system has a useful thermodynamic description — those near equilibrium do; far-from-equilibrium systems require kinetic theory. The analog: not every group of agents has a useful macro-agent description. Those with fast internal coordination (near "coordination equilibrium") do. The admissibility is: the macro-state has well-defined AAT state variables and update dynamics.
...
**Renormalization group (RG).** The RG asks: what theories are fixed points of coarse-graining? The effective Hamiltonian after coarse-graining must live in the same function space as the original. AAT's analog: the macro-dynamics must live in the same "function space" as individual AAT agents — same structural components, same update logic, same relationship between tempo and persistence. The admissibility is: coarse-graining maps AAT agents to AAT agents.

What makes this sharp: the RG framework says the admissible class is determined by what SURVIVES coarse-graining. Properties that are destroyed by projection are not part of the admissible class. This suggests: $\mathcal{M}_{adm}$ should require exactly those AAT properties that survive projection from micro to macro, and not require properties that are destroyed.

**Control theory → model reduction.** Balanced truncation preserves the Hankel singular values — the modes that are simultaneously most controllable and most observable. The admissibility is: the reduced model preserves these modes. AAT analog: $\Lambda$ should preserve the most observable and most actionable aspects of the micro-state. Admissibility requires the macro-model to have good observability (macro-observations are informative about macro-state) and good controllability (macro-actions affect macro-state).

**Software architecture → interface contracts.** A module is valid when its public API (interface) is a sufficient specification for clients. The internal implementation can be anything. Analog: the macro-agent's "API" — how it responds to observations and produces actions — must satisfy AAT's structural contracts. The internal micro-dynamics are implementation details.

This maps to a behavioral admissibility: don't constrain the macro-agent's internals, constrain its input-output behavior. A macro-agent is admissible if its behavior (observation → action mapping over time) is indistinguishable from SOME AAT agent with well-defined tempo, mismatch, gain, etc.
...
This doesn't prevent: macro-dynamics that are technically recursive but have no meaningful decomposition into epistemic and purposeful substates. A lookup table mapping macro-observations to macro-actions is recursive but not an AAT agent in any meaningful sense.
...
**Disadvantage**: doesn't guarantee AAT-structure at the macro level. A neural network could satisfy this without having decomposed state, mismatch signals, etc.

**Approach E: Behavioral equivalence.**

The macro-agent's externally observable behavior (actions given observation history) must be $\epsilon$-close to the behavior produced by some well-formed AAT agent with known tempo, mismatch, and gain:

$$\sup_{\text{obs histories}} \lVert \pi_c(\Lambda_x(X_{micro})) - \pi_{AAT}(M_{AAT}, G_{AAT}) \rVert \leq \epsilon$$

for some AAT agent $(M_{AAT}, G_{AAT}, \pi_{AAT})$ with well-defined quantities.

**Advantage**: doesn't constrain internals, only requires that the external behavior is consistent with AAT.
**Disadvantage**: existential quantifier — hard to verify. How do you show such an AAT agent exists?
...
This suggests a different angle on admissibility: $\mathcal{M}_{adm}$ should be the class of macro-dynamics for which bounded closure defect implies bounded trajectory error. The admissibility isn't about AAT structure at all — it's about stability.
...
Connection to AAT: the sector condition from the persistence framework (δᵀF ≥ α‖δ‖² for ‖δ‖ ≤ R) IS a Lipschitz-like condition on the correction function. If the macro-dynamics satisfy the sector condition, they satisfy the Lipschitz requirement. So: **macro-dynamics in the persistence regime automatically satisfy the bridge lemma requirement.**

This is promising. It means: if we require $\mathcal{M}_{adm}$ to satisfy the sector condition, we get both AAT-structure (persistence condition applies) and trajectory-error boundedness (bridge lemma holds) in one requirement.
...
1. **Structural form**: $(\pi_c, f_c, E_c)$ decomposes as an AAT agent — state $X_c = (M_c, G_c)$, recursive update $X_{c,t+1} = f_c(X_{c,t}, o_{c,t+1})$, state-dependent policy $a_{c,t} = \pi_c(X_{c,t})$.
...
**What this buys:**
- (1) ensures AAT structure at the macro level
...
Define $\mathcal{M}_{adm}$ as the class of dynamics that are "AAT-shaped" in the following sense:

An AAT agent is characterized by a tuple $(\mathcal{T}, \rho, \delta_{\text{crit}}, R, \alpha)$ — tempo, disturbance, critical mismatch, reserve, correction rate. These are the "slow variables" that survive coarse-graining (like temperature and pressure survive the micro-to-thermodynamic projection).
...
**The RG connection**: the projection $\Lambda$ maps micro-quantities to macro-quantities. The admissibility is: the macro-quantities satisfy the same dynamical law (persistence condition) as the micro-quantities. This is a fixed-point condition — AAT is invariant under coarse-graining within the admissible class.
...
**(A1) AAT agent structure.** The macro-state decomposes as $X_c = (M_c, G_c) \in \mathcal{X}_c$. The macro-update is recursive: $X_{c,t+1} = f_c(X_{c,t}, o_{c,t+1})$. The macro-policy is state-dependent: $a_{c,t} = \pi_c(X_{c,t})$.
...
team-persistence does NOT derive (A1)-(A4) for the COMPOSITE macro-agent. It derives persistence conditions from the sub-agents' perspective: each sub-agent persists given the cooperative/adversarial network. The macro-agent level — whether the composite AS A WHOLE satisfies AAT's structural requirements — is not addressed.
...
- Composition admissibility: "the macro-agent satisfies AAT's structural requirements" (macro-level structure)

These are RELATED but DIFFERENT. Micro-level persistence is NECESSARY for macro-level admissibility (if sub-agents diverge, the composite can't be a valid macro-agent). But it's not SUFFICIENT — the macro-dynamics must also have AAT structure (decomposed state, mismatch signal, tempo, sector condition on the correction function).
...
**Sub-case: $\alpha_1 = \alpha_2 = \alpha$.** Then $\dot{\delta}_c = -\alpha \delta_c + w_c$, where $\lVert w_c \rVert \leq (\rho_1 + \rho_2)/2$. The macro-dynamics are exactly AAT-shaped. $\varepsilon^\ast = 0$ (no closure defect). $\alpha_c = \alpha$, $\rho_c = (\rho_1 + \rho_2)/2$.
