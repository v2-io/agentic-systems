...
- **`doc/sop/spikes.sop.md`:** `:48/:55` (§0c) aligned; `:110–112` (§2-bis(2) need-vs-mention / spike-archivability) is load-bearing *for* the sharpening (it already establishes that an unneeded pointer trips the archivability test) — not drift.
...
- **Route at write-time, don't just note.** A WN forward-pointer is registered in a real tracker when written (the standing cycle per `doc/sop/spikes.sop.md` §0c — TODO / PROPOSALS / PRACTICA / the audit cycle / `spikes/PROPOSED.md`); the WN then holds a *pointer* to that tracked obligation, **never the sole obligation.**
