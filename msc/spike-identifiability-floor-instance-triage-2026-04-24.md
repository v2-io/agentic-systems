# Spike: Triaging the Four Candidate `#discussion-identifiability-floor` Instances

*Started 2026-04-24. Research / analytic spike. Not canon. No segment files modified.*

**Status.** Triage spike for the gating decision on which (if any) of four candidate instances should promote to `#discussion-identifiability-floor`, in what order, with what subsumption structure, and with what effect on the meta-segment's growth.
...
## 1. Extracting the test: the five-element shape of `#discussion-identifiability-floor`
...
| E5 | **Strengthens a *proposed* appendix, not an existing segment.** The floor elevates the yet-to-be-promoted `#rho-decomposition` / `#internal-external-decomposition` and refines `#sector-persistence-template`, `#discussion-separability-pattern`, and `#additive-coordinate-forcing`. No elevation of an existing load-bearing segment at the standard I1–I3 produce. |
...
### 2.4 Candidate 4 — Mechanism-design no-go (pre-existing `#discussion-identifiability-floor` Working Note)
...
**Honest reframe:** Candidate 3 is best understood as a **derived theorem downstream of `#additive-coordinate-forcing`'s fourth primary instance**, not as an instance of `#discussion-identifiability-floor`. It belongs in `#bias-bound-derivation` (proposed appendix) with a Discussion cross-reference noting that the (PI)-as-load-bearing observation shares structural shape with the floor pattern but does not match its five-element criteria. The spike itself reaches this conclusion ("not as a new primary instance") for the parallel question about `#additive-coordinate-forcing`; the conclusion for `#discussion-identifiability-floor` is the same.

*Strength of this claim:* **Robust qualitative**, with the further structural observation that *the spike itself acknowledges this* in §4: "Is $C$ a fifth instance of `#discussion-identifiability-floor`? Possibly." The "possibly" softens to "no, not by the five-element test" once E4 is applied strictly.
...
| 3 (constant $C$) | Downstream theorem of `#additive-coordinate-forcing` Instance 4 | Not a `#discussion-identifiability-floor` instance; belongs in `#bias-bound-derivation` |
...
**Candidate segment-level follow-on:** Update `#loop-interventional-access` Discussion with a subsection *"Three modes of deployment across `#discussion-identifiability-floor` instances"* naming the three modes. This is one paragraph plus cross-references; it surfaces the structural unification without collapsing the semantic distinction.
...
`#discussion-identifiability-floor` currently holds three instances. Candidates 2 and 4 (per §3) would take it to five. Does the meta-pattern dilute at five? Is there a principled termination criterion?
...
- I3 elevates `#critical-mass-composition`, composite-extended `#loop-interventional-access` (mode 2: observer-on-subagent), and `#scope-composite-agent`.
...
`#additive-coordinate-forcing` has a "1-anchor + 3-theorem" characterization. Does `#discussion-identifiability-floor` admit an analogous structure?
...
- *Why primary:* Fills a genuinely distinct layer (L5). The pattern-match is clean under the broad reading of `#discussion-identifiability-floor`. The external theorems are canonical; the escapes are canonical.
...
- *Tag location:* In the ρ-decomposition appendix when it promotes, add a Discussion subsection "Connection to `#discussion-identifiability-floor` Instance 4" noting the projection relationship.
...
**Redirect (not a `#discussion-identifiability-floor` promotion): Candidate 3.**
...
- *Cross-reference to `#discussion-identifiability-floor`:* None required at promotion time. If a reviewer later notices the pattern-shape and asks about it, a brief Discussion note in `#bias-bound-derivation` observing "shares the shape of `#discussion-identifiability-floor` but with a single escape rather than multiple; see `#additive-coordinate-forcing` Instance 4 for the structural home" would preempt the ambiguity.
...
- Extracted a five-element test from Instances 1–3 of `#discussion-identifiability-floor` with element-level criteria. (§1)
...
- **Resolve the narrow-vs-broad reading of `#discussion-identifiability-floor`'s scope** (information-theoretic only vs. general impossibility-theorem). Recommended broad reading in §2.4 and §6.1, but this is a formulation decision; both positions are coherent. A follow-up framing-pass could revisit.
