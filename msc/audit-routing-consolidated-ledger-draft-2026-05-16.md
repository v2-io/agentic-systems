...
### research-seed — parent-generated this cycle (not from a cluster; durable home before it evaporates)

| # | Finding (attributed) | Source | Status |
|---|---|---|---|
| **S30** | **Does Cor A.1S.1 license the structural-adaptation hand-off claim?** `#deriv-sector-condition` / `#deriv-stochastic-non-exit` state Cor A.1S.1 "sharpens the hand-off into `#result-structural-adaptation-necessity` — the structural-adaptation trigger is generic, not exceptional." But that target segment derives necessity from **model-class inadequacy** ($\mathcal F(\mathcal M) \lt 1-\varepsilon$), references neither Model-S nor region-exit nor Cor A.1S.1, and the corollary's own OU instance is *recurrent* (exit-and-**return**). Certain region-exit licenses "the pathwise-forever *parametric* guarantee is structurally unavailable; long-lived agents generically enter the out-of-region regime" — it does **not** by itself license "model-class structural adaptation is generically necessary" (a categorically different, fitness-driven trigger). Gate-2-class: a Discussion/Findings claim that *sounds* like it follows from the corollary. Per strengthen-before-soften the non-soften option is preferred: rather than tighten the phrasing, attempt to **derive the bridge** — under what conditions does certain region-exit *imply* structural-adaptation necessity (e.g. A2' genuinely failing outside $\mathcal B_R$, or excursion statistics interacting with $\mathcal F(\mathcal M)$)? Marked as *inference, verified against both segments first-hand*; not an established defect — "sharpens" is deliberately soft and a weaker "the *regime* is generically entered" reading is defensible. Joseph's adjudication (tighten / derive / tolerate) was surfaced at the 2026-05-16 checkpoint and is unresolved. | parent deep-read of `result-structural-adaptation-necessity.md` vs the Model-S segments, 2026-05-16 (not a cluster finding) | open (research-seed; strengthen-direction — derive-the-bridge is the preferred attempt; **graduate-watch** if the bridge is real) |
