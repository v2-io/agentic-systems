...
## What to include (from existing AAT segments)
...
- **Source**: Stripped-down #form-agent-model, #def-mismatch-signal, #def-adaptive-tempo — enough to define the formal objects, not the full AAT apparatus
...
- AAT's contribution: the *formal tradeoff structure* that explains why these metrics matter and how they relate

### 9. Discussion: Connection to Adaptive Agency (brief)
- This paper derives from a broader framework (AAT) that connects software engineering to control theory, causal inference, and agent composition
...
- Full AAT apparatus (Lyapunov stability, sector conditions)
...
These are all essential to AAT but create barriers for the SE audience. The paper should stand on its own with the minimum formal apparatus needed for the results.
...
- This paper doesn't require anyone to accept AAT. It stands on temporal optimality (a postulate most developers would accept intuitively) and the cost decomposition (straightforward).
