...
From `msc/spike-unity-closure-mapping.md` §6: composition is IB with appropriate relevance variables. Proposed refinement:
...
- It does not resolve the bridge lemma's contraction obstruction for non-stationary purposeful agents (that's a separate open problem; see `msc/spike-mori-zwanzig-composition.md`).
