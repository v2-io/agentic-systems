...
*[Definition ($H_b$-AAD-internal, from #agent-identity + #directed-separation + #model-class-fitness)]*
...
The Jacobian-level B1 strengthening spike (`msc/spike-jacobian-b1-strengthening.md`) proposes parameterization-invariance (PI) as a candidate fourth meta-pattern instance, extending `#agent-identity`'s token-level commitment. (PI) says AAD predictions should not depend on arbitrary coordinate choices.
...
- **The observer-indexing and trajectory-indexing** (§2.4) are *derived* from AAD-internal commitments (`#agent-identity`'s singular trajectory; `#directed-separation`'s filtration-indexed update) — theorem-style extensions of existing segments.
...
- **Čencov 1982, *Statistical Decision Rules and Optimal Inference*.** The (PI) / parameterization-invariance candidate fourth meta-pattern, per `msc/spike-jacobian-b1-strengthening.md`. §11 shows $H_b$ is coordinate-free at the action-marginal level, compatible with any (PI)-extension of `#agent-identity`.
