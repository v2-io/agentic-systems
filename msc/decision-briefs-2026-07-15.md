...
### A2 — D-2: the bulk-64 `.integrated/` wipe (semantics, timing, scope)

*Since the 2026-07-07 census:* Unchanged since the census. Verified 2026-07-15: `.integrated/` = 127 entries, `.archived/` = 1, D-2 still the sole JOSEPH-TODO needs-your-decision item; no spikes/.integrated commits since 07-07.

**What**: Authorize (or decline) the deletion of `spikes/.integrated/` + `.archived/` you signalled 2026-05-19 — never executed. **Context**: 64 spikes were bulk-filed to `.integrated/` in May 2026 *without* per-spike verification that their math/no-gos landed in canon; `.integrated/` still holds 127 entries (verified 2026-07-15), `.archived/` 1. This is the only item on JOSEPH-TODO's "needs your decision" list, and it also blocks your provisional "segments may assume traces live in `.integrated/`" principle (INTEGRATION-CLEANUP §item-5). Three coupled sub-calls: discharge-vs-set-down; `rm`+commit (git-recoverable) vs history-purge; scope (also `02-tst-core/simulations/`, `track-b-nonlinear-sims/`, `ref/`?). **Options**: (a) Agent verify-pass on the 64 first (parallelizable, not one-evening), then `rm`+commit whatever's confirmed landed. (b) Conscious set-down: `rm`+commit now, accept possible loss of unlanded archaeology (git still recovers). (c) History-purge (truly irreversible). (d) Leave as-is. **Lead recommendation**: (a) — verify then git-recoverable removal, scope limited to `.integrated/`+`.archived/` for now; never (c). Effort is a false constraint; the verify-pass is cheap for agents and it's the role's undischarged duty. Honest uncertainty: none material — (a) dominates unless you positively want the set-down as a statement. **Pointer**: `INTEGRATION-CLEANUP-TODO.md` D-2/F3 (lines ~128, ~157), `JOSEPH-TODO.md`.
