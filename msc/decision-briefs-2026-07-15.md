...
*Updated 2026-07-16:* the verify-pass the lead-rec called for has been executed (64 spikes read + checked against canon content; every claimed-unlanded item adversarially challenged; ledger at `spikes/.integrated/VERIFICATION-2026-07-16.md`). Result: 54 landed / 6 partial / 3 superseded / 1 nothing-to-land; **21 confirmed-unlanded items across 10 spikes** (2 high-value), queued in `TODO.md` §"Bulk-64 unlanded content".

**What**: Authorize the deletion of `spikes/.integrated/` + `.archived/` you signalled 2026-05-19. **Context**: `.integrated/` membership is now a *verified* record for the bulk-64; the only remaining content-loss exposure is the 21-item landing queue, whose carrier spikes would be deleted (git-recoverably) by the wipe. Your provisional "segments may assume traces live in `.integrated/`" principle is also unblocked by the verification. Remaining sub-calls: timing relative to the landing queue; `rm`+commit vs history-purge; scope (also `02-tst-core/simulations/`, `track-b-nonlinear-sims/`? — both still unverified corpora, G2). **Options**: (a) Land/decline the 21 queued items first (~10 spike-landing cycles, agent-executable), then `rm`+commit. (b) `rm`+commit now — git still recovers the carriers, so the queue can land from history; slightly higher friction, no true loss. (c) History-purge (irreversible — still recommended never). **Lead recommendation**: (a), and it is now cheap and concrete rather than open-ended; (b) is acceptable if you want the tree clean immediately. **Pointer**: ledger + `TODO.md` §Bulk-64 + `INTEGRATION-CLEANUP-TODO.md` F3.
