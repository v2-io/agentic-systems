...
### Repo weight: untrack _obs/ (162M) + Garamond fonts (35M)?
...
### A2 — D-2: the bulk-64 `.integrated/` wipe — verify duty now DISCHARGED; decision simplified

*Updated 2026-07-16 (evening):* verify-pass AND the full landing cycle are done — all 21 unlanded items landed or discharged (CHANGELOG 2026-07-16, 6 landings), and the 3 superseded + 1 nothing-to-land spikes are re-sorted to `.archived/` with MANIFEST reasons. Both bucket labels now state verified truth.

**What**: Authorize the deletion of `spikes/.integrated/` + `.archived/` you signalled 2026-05-19. **Context**: `.integrated/` membership is now a *verified* record for the bulk-64; the only remaining content-loss exposure is the 21-item landing queue, whose carrier spikes would be deleted (git-recoverably) by the wipe. Your provisional "segments may assume traces live in `.integrated/`" principle is also unblocked by the verification. Remaining sub-calls: timing relative to the landing queue; `rm`+commit vs history-purge; scope (also `02-tst-core/simulations/`, `track-b-nonlinear-sims/`? — both still unverified corpora, G2). **Options**: (a) `rm`+commit both directories now — nothing unverified or unlanded remains; git recovers everything. (b) Keep them as the addressable trace corpus — which would also *activate* your provisional "segments may assume traces live in `.integrated/`" principle, now that membership is verified. (c) History-purge (still recommended never). **Lead recommendation**: none between (a) and (b) — they serve opposite goods you weigh (clean tree vs. addressable trails; note (b) reverses the original wipe instinct, which predated verification). A word suffices. **Pointer**: `spikes/.integrated/VERIFICATION-2026-07-16.md`; `INTEGRATION-CLEANUP-TODO.md` F3 (closed).
