...
| `catastrophic forgetting regime` | `empty feasibility window` | canonicalize | +3 | The text specifically defines this as the "empty window limit" of the stability-plasticity window. This grounds it in AAD math rather than an ML imported term. |
| `catastrophic forgetting regime` | `plasticity-bound failure` | rename | +2 | Emphasizes that the bounds of the window are failing. |
| `catastrophic forgetting regime` | `stability-plasticity collapse` | rename | +2 | Focuses on the two constraints crashing into each other. |
...
| `learning freeze from low model uncertainty or high observation uncertainty` | `epistemic gain collapse` | canonicalize | +3 | Already heavily referenced as "gain collapse". Adding "epistemic" clarifies it's the update gain dropping to near 0. |
| `learning freeze from low model uncertainty or high observation uncertainty` | `update calcification` | add-alias | +2 | Good prose handle for the failure mode where the model stops taking in new info. |
...
| `calibration laboratory` | `privileged grounding domain` | rename | +3 | Describes exactly what software is for AAD: the domain where formal properties are cleanly grounded without extra transfer assumptions. |
| `calibration laboratory` | `high-identifiability testbed` | rename | +2 | Captures the "high-identifiability" empirical claim perfectly. |
...
| `unobservable strategy subgraph` | `epistemic dead zone` | canonicalize | +3 | Strong prose phrase. Better than generic "unobservable" since it focuses on the epistemic failure to track it. |
| `unobservable strategy subgraph` | `feedback-starved branch` | rename | +2 | Explains the consequence. |
...
| `dormant structural variation that becomes useful after regime change` | `latent adaptive capacity` | canonicalize | +3 | Describes capacity that isn't currently used but is preserved. |
| `dormant structural variation that becomes useful after regime change` | `exaptive reserve` | rename | +2 | "Exaptive" specifically means an adaptation used for a new purpose, fitting the regime change requirement perfectly. |
...
| `terminal alignment error` | `terminal alignment gap` | canonicalize | +3 | Gap pairs nicely with satisfaction gap, and fits the terminology. |
| `terminal alignment error` | `objective misspecification` | rename | +2 | Explicitly names the cause: the operational criteria didn't match the true objective. |
...
| `managing memory across session boundaries to prevent the sufficiency discontinuity` | `reconstruction relay` | add-alias | +2 | Highlights the relay race nature of state across session boundaries. |
| `managing memory across session boundaries to prevent the sufficiency discontinuity` | `artificial hippocampus` | canonicalize | +3 | Specifically maps to the biological analog of cross-episode memory consolidation referenced in AAD. |
...
| `unnamed calibration laboratory framing for software TST` | `calibration laboratory` | canonicalize | +3 | A highly functional grounding metaphor for TST's role. |
...
| `unnamed the strengthen before soften work posture` | `strengthen-first posture` | canonicalize | +3 | Secures this as a first-class named engineering principle. |
| `unnamed the gain collapse failure when both u m → 0 and u o → ∞` | `epistemic gridlock` | add-alias | +2 | Vividly captures the "freeze" aspect where learning stops entirely despite ongoing mismatch signals. |
| `unnamed the gain collapse failure when both u m → 0 and u o → ∞` | `gain collapse` | canonicalize | +3 | Defended canonicalization of the exact AAD mechanism for this freeze. |
...
| `bounded mismatch region` | `persistence envelope` | add-alias | +3 | Geometrically evocative and already gaining traction elsewhere in the targets. |
...
| `projection contraction must beat target drift` | `contraction over drift principle` | canonicalize | +3 | Short, memorable slogan for the core Lyapunov inequality. |
...
| `retrieval keyed by state rather than current objective` | `state-keyed retrieval` | rename | +2 | Contrasts explicitly with objective-keyed / goal-biased retrieval. |
...
| `unnamed coupling between an agent s model of self and model of other the prose form of kappa cross` | `cross-agent strategic coupling` | rename | +3 | Names the specific game-theoretic linkage that creates Class 3 composites. |
| `unnamed organizing principle slogan an adaptive system is a projection whose contraction rate exceeds its target s drift rate` | `contraction over drift principle` | canonicalize | +3 | Standardizes the O-BP10 slogan across the framework. |
| `unnamed software as AAD s privileged high identifiability calibration laboratory` | `software calibration laboratory` | canonicalize | +3 | Formalizes TST's role as the cleanly identifiable testbed for AAD. |
| `unnamed stronger composition consistency demand that composite admissibility inherit from sub agent properties plus topology` | `composition heredity axiom` | canonicalize | +3 | Names the strict requirement that composite properties must be derivable from constituents. |
...
| `code quality and tempo positive feedback` | `quality-tempo compound effect` | rename | +3 | Formally identifies the feedback loop between code quality and developer adaptive tempo. |
| `code quality feedback loop through tempo` | `virtuous-vicious quality cycle` | rename | +2 | Names the specific bifurcation dynamic driven by the quality-tempo compound effect. |
...
| `empty stability plasticity feasibility window` | `catastrophic forgetting regime` | canonicalize | +3 | Identifies the specific regime where the stability-plasticity window collapses entirely. |
...
| `externalization reconstruction across sessions` | `externalization-reconstruction cycle` | canonicalize | +3 | Names the core operational loop preserving state for logogenic agents. |
...
| `minimum sufficiency after a session rebuild` | `reconstruction adequacy threshold` | rename | +3 | Formally identifies the inter-session survival boundary. |
| `model state written into the environment` | `externalized state` | canonicalize | +3 | Elevates the core mechanism of logogenic continuity. |
...
| `persistent storage reconstruction of class 2 state` | `Class 2 state reconstruction` | canonicalize | +2 | Formally names the specific recovery operation. |
| `quality to tempo chain` | `quality-tempo compound effect` | canonicalize | +3 | Aligns with the previously renamed code-quality dynamic. |
...
| `unnamed empty stability plasticity feasibility window in consolidation dynamics` | `catastrophic forgetting regime` | canonicalize | +3 | Matches the earlier resolution of this specific limit. |
| `unnamed externalizing part of $M_t$ into the environment for future agents` | `stigmergic externalization` | rename | +3 | Adopts the biological term (stigmergy) for leaving state in the environment. |
| `unnamed fourth diagnostic where terminal conditions are met but the objective is still missed` | `attainability failure` | canonicalize | +3 | Pairs directly with the satisfaction gap and control regret. |
...
| `unnamed minimum sufficiency required after a session rebuild` | `reconstruction adequacy threshold` | canonicalize | +3 | Standardizes this inter-session persistence boundary. |
...
| `unnamed the externalization reconstruction cycle across sessions` | `externalization-reconstruction cycle` | canonicalize | +3 | Elevates the logogenic continuity loop to a proper noun. |
| `unnamed the self reinforcing code quality → tempo loop` | `quality-tempo compound effect` | canonicalize | +3 | Standardizes the code-quality feedback loop mechanism. |
...
| `unnamed RLHF5 queries biased by the current goal acting as an echo chamber` | `goal-biased retrieval` | rename | +2 | Matches the architectural term for this feedback failure. |
...
| `unnamed managing memory across session boundaries to prevent the sufficiency discontinuity` | `reconstruction relay` | canonicalize | +2 | Standardizes the term for bridging context turnover. |
...
| `unnamed regions of the strategy DAG that cannot be updated because feedback cannot reach them` | `epistemic dead zone` | canonicalize | +3 | Standardizes the geometric phrasing for unobservable subgraphs. |
| `unnamed retrieving context based only on state not goal` | `state-keyed retrieval` | canonicalize | +3 | Standardizes the goal-blind routing requirement. |
...
| `unnamed strengthen first posture` | `strengthen-first posture` | canonicalize | +3 | Standardizes the methodological commitment. |
...
| `unnamed terminal alignment error as a formal signal $\delta_\text{align}$` | `terminal alignment gap` | canonicalize | +3 | Pairs directly with satisfaction gap. |
...
| `unnamed the logogenic analog to the persistence condition for session reconstruction` | `reconstruction adequacy condition` | canonicalize | +3 | Standardizes the inter-session survival boundary. |
...
| `unnamed the mathematical limit of bayesian learning without forgetting` | `dogmatic convergence limit` | canonicalize | +3 | Names the state where $U_M \to 0$ without a noise injection term. |
...
| `unnamed the phenomenon where both $U_M \to 0$ and $U_o \to \infty$ freeze learning` | `epistemic gridlock` | canonicalize | +3 | Identifies the specific double-failure mode of gain collapse. |
...
| `unnamed the physical compute bounds on survival between forgetting rate and consolidation cadence` | `stability-plasticity feasibility window` | canonicalize | +3 | Standardizes the regime boundary terminology. |
...
| `unnamed the region where the persistence condition holds` | `persistence envelope` | canonicalize | +3 | Geometric metaphor for the safe operating region. |
...
| `unnamed the state where credit assignment collapses and learning freezes` | `epistemic gain collapse` | canonicalize | +3 | Unifies this with the generic gain collapse terminology. |
...
| `unnamed variation in correction architectures across a population that is invisible to current persistence analysis` | `latent structural capacity` | canonicalize | +3 | Maintains the formal name for un-executed resilience. |
...
| `unnamed $U_o \to \infty$ freezing the learning rate` | `observation ambiguity freeze` | rename | +2 | Describes the nihilistic failure mode of gain collapse. |
...
| `unnamed dormant unused architectural complexity that survives until an environmental shift` | `latent adaptive capacity` | canonicalize | +3 | Aligns with the earlier rename for structural variation. |
...
| `unnamed master developers writing clean code in the same time as messy code` | `quality-tempo compound effect` | canonicalize | +3 | Maps the mastery phenomenon to the formal tempo feedback loop. |
...
| `unnamed agent as a projection whose contraction rate must exceed its target s drift` | `contraction over drift principle` | canonicalize | +3 | Resolves the core O-BP10 slogan. |
...
| `unnamed the AAD expressible failure mode of an empty stability plasticity window` | `catastrophic forgetting regime` | canonicalize | +3 | Resolving another instance of the forgetting limit. |
...
| `unnamed variation in correction architectures invisible to persistence analysis` | `latent structural capacity` | canonicalize | +3 | Matches the prior un-executed resilience alias. |
...
| `unnamed the regime where mismatch is bounded and the agent maintains adaptive capacity indefinitely` | `structural persistence regime` | canonicalize | +3 | The stable regime governed by the persistence condition. |
| `unnamed the section of a strategy where a decision has no observable consequences and thus cannot be improved by learning` | `epistemic dead zone` | canonicalize | +3 | Reaffirming the strong alias for unobservable edges. |
...
| `unnamed the complete adaptive cycle from anticipation through action` | `adaptive cycle` | canonicalize | +3 | The generic term for the Prolepsis-Praxis loop. |
...
| `unnamed the phenomenon that unobservable edges freeze and paths become epistemically dead` | `unobservable strategy subgraph` | canonicalize | +3 | Identifies the geometric cause of the epistemic dead zone. |
| `unnamed the region in parameter space where parametric updates remain effective before structural change is forced` | `parametric feasibility window` | rename | +2 | The region governed by the current model class $\mathcal{M}$. |
...
| `unnamed the unobservable edges in a strategy DAG that cannot be revised because their values cannot be inferred` | `epistemic dead zone` | canonicalize | +3 | Third hit on this concept; clearly needs this exact canonical name. |
...
| `unnamed the five phases of the adaptive cycle` | `adaptive cycle phases` | rename | +2 | Collectivizes Prolepsis, Aisthesis, Aporia, Epistrophe, and Praxis. |
...
| `the five cycle phases prolepsis aisthesis aporia epistrophe praxis` | `five adaptive cycle phases` | rename | +2 | Collective grouping. |
...
| `unnamed the chain layer anchor role in additive coordinate forcing` | `chain-layer anchor` | rename | +2 | Foundation of the coordinate forcing meta-pattern. |
...
| `unnamed the persistence region in $(\alpha, \rho, R)$ parameter space` | `persistence envelope` | canonicalize | +3 | Standard geometric boundary for survival. |
| `unnamed the region in parameter space where sector persistence holds` | `persistence envelope` | canonicalize | +3 | Same geometric boundary. |
| `unnamed the sector persistence region in parameter space where the agent is guaranteed to maintain bounded mismatch` | `persistence envelope` | canonicalize | +3 | Same geometric boundary. |
...
| `unnamed the projection whose contraction rate must exceed target drift the opus organizing principle slogan` | `contraction over drift principle` | canonicalize | +3 | Final lock-in for the core slogan. |
| `unnamed the strengthen first attempt the improbable meta approach to theory development` | `strengthen-first posture` | canonicalize | +3 | Secures the methodological commitment. |
...
| `unnamed the reconstruction adequacy condition for logogenic agents` | `reconstruction adequacy condition` | canonicalize | +3 | Locks in the boundary test for logogenic persistence. |
...
| `unnamed the specific moment when $\eta^st 	o 0$ because $U_o 	o 0$ too certain rather than because $U_M 	o 0$ model confident` | `dogmatic convergence limit` | canonicalize | +3 | Names the state where update stops purely from objective certainty. |
...
| `gemini s competency trap for $\eta^st 	o 0$` | `stability-induced myopia` | canonicalize | +3 | Retains the descriptive alias for success-driven failure. |
| `gemini s epistemic death for the gain collapse unobservable DAG failure` | `epistemic dead zone` | canonicalize | +3 | Locks in the geometric dead zone over "death". |
...
| `unnamed the contraction over drift insight` | `contraction over drift principle` | canonicalize | +3 | Asserts the fundamental slogan. |
...
| `unnamed the strengthen before soften posture applied to apparent overclaims` | `strengthen-first posture` | canonicalize | +3 | Solidifies the specific methodological approach. |
| `unnamed the virtuous vicious cycle between $M_t$ quality and $\Sigma_t$ evaluable complexity` | `quality-tempo compound effect` | canonicalize | +3 | Reaffirms the core feedback loop coupling code to agency. |
...
| `unnamed chain layer anchor role in additive coordinate forcing` | `chain-layer anchor` | canonicalize | +3 | Identifies the foundation of the additive meta-pattern. |
...
| `unnamed cycle phase sequence as whole` | `orient cascade` | canonicalize | +3 | Maps the specific five-phase execution order. |
