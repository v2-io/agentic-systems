...
*Decided with him, 2026-08-22 (the gate's standing resolution for lifted material):* audit gold may be mined into the chapter-end `impl-*` segments' Working Notes as teaching frames and implications — Joseph: *"audit gold (the pedagogy and wandering thoughts from the auditors on related segments) often have critical insights and implications and teaching frames that can go directly into the working notes for the impl- files"* — and *"once notes from the audit gold are in working notes, those parts (up to the entire thoughts files) can be considered fully .integrated finally (do the manifest and changelog work)."* So a reflection file whose gold has been lifted, per-item and first-hand, moves to `audits/.integrated/` with a MANIFEST entry naming where each item landed and a CHANGELOG line; files or items not yet lifted stay under the gate. What lands in a Working Note is the teaching content itself (frame, analogy, implication, candidate figure) — not narration about the audit.
