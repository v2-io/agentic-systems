...
**Sibling projects** (not in this repo, but relevant): `~/src/_core/tst/` prior TST corpus (most absorbed into `02-tst-core/`; the 965 vault analyses remain) · `~/src/shoshin/` PROPRIUM runtime prototype (Python skeleton; only code attempt at the architecture) · `~/src/firmatum/` PROPRIUM ontology + architecture source (defines what an ELI is) · `~/src/embeddings/` epistemic-hedging-geometry experiments ($\rho = 0.991$ vs psychometric data; supports the logogenic claim).
