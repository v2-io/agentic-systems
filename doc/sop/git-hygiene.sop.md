...
> **2026-08-22 (Joseph):** the gate is `md-press --check <files>` (`md-press --help` is the live truth), not `bin/lint-md` — *"you shouldn't be using lint-md"*, and *"there shouldn't be anything that lint-md does that md-press doesn't do better. If you find any gaps, it's a md-press bug to fix."* `--check` exits 1 listing files that would change; most trackers and some segments already fail it at HEAD (pre-existing wrap debt), so the commit-time question is *did you add to it*. Anything md-press gets wrong or does not check — found so far: it joins house pseudo-display `$$…$$` lines at low confidence, which FORMAT forbids; it does not yet run the GitHub math-compat checks (`\lt`/`\gt`, `\vert`, `\ast`) — goes to `~/src/arch/firmatum/utils/md-press/` as a dated `FEEDBACK-*.md`, not into a workaround here. The paragraph below records what lint-md used to do, so the bug reports have a checklist to work from. (Scope note, 2026-08-24: the deprecation is `bin/lint-md` **only** — `bin/lint-outline` is a live, current tool doing a different job: OUTLINE dependency/stage/orphan consistency, which md-press does not cover. A brief that day mis-relayed the deprecation as covering both; Joseph's correction — *"bin/lint-outline is not deprecated, only bin/lint-md"* — governs. Same day, Joseph-ratified: `bin/lint-md` itself now prints this deprecation notice to stderr at startup and then runs anyway — advisory-only, kept runnable because its math-compat checks still cover the filed md-press gap — so the deprecation is self-announcing and can no longer be lost in relay.)

`bin/lint-md` formerly gated the commit for any `.md` you touched — run it before you report the file clean or commit it, every time, memo and spike and README alike, not only FORMAT-governed segments. This is the forcing function for the math-in-files discipline (LaTeX-not-Unicode, one-logical-line); knowing the rule has proven necessary-but-not-sufficient, so the habit is lint-before-claim, not more resolve. One caution the linter can't save you from: its bare-Greek check skips code spans, so backtick-wrapped Unicode math (`` `η → ϱ` ``) passes lint *and* renders as ugly monospace — **lint-clean ≠ renders-well**, so also eyeball for Unicode hiding inside backticks. The full statement of the discipline and its failure modes lives in the auto-loaded layer (`CLAUDE.md` §"Math in conversation vs files") — this is just the commit-time hook.
