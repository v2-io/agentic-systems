...
*Sketch.* Max attainable: exact (with a complete proof). The inequality is structurally motivated: composition cannot create corrective capacity out of nothing, and internal reconciliation consumes tempo that doesn't contribute to external mismatch correction. The $\varepsilon^\ast \to C_{\text{coord}}$ connection is now sketched: closure defect as internal disturbance rate gives $C_{\text{coord}} \geq \varepsilon^\ast \nu_c$. The inequality $\mathcal T_c \leq \sum \mathcal T_i$ follows directly. The remaining gap: whether $\varepsilon^\ast \nu_c$ is a tight lower bound or whether additional coordination costs (negotiation, synchronization, conflict resolution) contribute to $C_{\text{coord}}$ beyond the closure-defect mechanism.
