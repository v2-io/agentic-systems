...
**Double depth penalty.** Chain depth creates two independent penalties that compound multiplicatively. This segment identifies the first: **confidence decay** — deeper chains have lower aggregate confidence because $\log P(\text{chain})$ accumulates negative terms. The two-edge strategic dynamics analysis (`msc/spike-two-edge-strategic-dynamics.md`) identifies the second: **evidence starvation** — downstream edge $k$ in a chain is tested only when all upstream edges succeed, so its effective correction rate is attenuated by $\prod_{j<k}\theta_j$. A deep plan is both less confident (this segment's contribution) and harder to calibrate (the evidence-starvation effect). These are structurally independent: confidence decay is a property of the propagation formula at fixed credences, while evidence starvation is a property of the learning dynamics. They compound because a deep edge with low confidence (from chain decay) also receives fewer observations (from evidence starvation), making the low confidence harder to correct.
