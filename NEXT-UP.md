...
**Remaining — full plan in [`msc/sop-shift-completion-plan-2026-06-02.md`](msc/sop-shift-completion-plan-2026-06-02.md).** Phase A **done** (audit pair). Phase B/B1 **done** — both orphan SOPs (`multi-agent.sop.md`, `git-hygiene.sop.md`) authored in peer register. **B2 done** — the WN-discipline single-sourced into `format.sop.md` §"What earns a Working Note" (authoritative), repo sites narrowed to pointers (`agents.sop.md`, `audit.sop/routing.sop.md`, `spikes/README.md`); memory-layer carriers deferred to the global/memory pass. The **memory-thinning** flagged decision was effectively resolved by the `MEMORY.md` condensation (the budget problem was index-line length, not substrate — fixed without gutting). **What's left: B3** — slim `agents.sop.md`'s body, demoting its procedure restatements to pointers at the new SOPs (orientation header + SOP index already done; it's the keystone and the highest-impact remaining edit since every agent loads it). The `sop.md` master-index question is resolved: `agents.sop.md` is the index.
