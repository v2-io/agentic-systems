...
**Remaining — full plan in [`msc/sop-shift-completion-plan-2026-06-02.md`](msc/sop-shift-completion-plan-2026-06-02.md).** Phase A is **done** (audit pair, above). What's left is **Phase B (the substantive value; needs judgment, not a mechanical pass):** **author the orphan SOPs** (`multi-agent`, `git-hygiene`) from the memory files, then thin those memories to pointers; **single-source the drifted disciplines** (WN-discipline → `format.sop.md`, etc.); and **slim `agents.sop.md`'s body** — demote its procedure restatements to pointers (its orientation header + SOP index are already done). The `sop.md` master-index question is resolved: `agents.sop.md` is the index.
