...
**Landed this cycle:** `naming.sop` fully interred — `methodology.sop.md` (`ad59d57`) + `principles.sop.md` (`305c541`) under the `naming.sop.md` index; **`spike-routing.md` interred → `doc/sop/spikes.sop.md`**; `bin/check-links` built (`f6ffaa5`); 3 stale links fixed (`5bb5177`); `JOSEPH-TODO.md` created (`1382d64`); legacy `bin/build-tex` sunset (`60a201f`); **`format.sop` interred** (`038e2ed` — `FORMAT.md` symlinks to it); **`agents.sop.md` is the agent-orientation home + SOP index** (`4d5b1be` — `CLAUDE.md` / `AGENTS.md` / `GEMINI.md` symlink to it); **Phase A done** — the **audit pair interred** into `doc/sop/audit.sop/{de-novo,routing}.sop.md` with the `audit.sop.md` index (symlink trick; ~127 inbound refs resolve untouched; check-links green).

**Remaining — full plan in [`msc/sop-shift-completion-plan-2026-06-02.md`](msc/sop-shift-completion-plan-2026-06-02.md).** Phase A **done** (audit pair). Phase B/B1 **done** — both orphan SOPs (`multi-agent.sop.md`, `git-hygiene.sop.md`) authored in peer register. What's left in **Phase B (judgment, not mechanical):** the **memory-thinning** of the now-consolidated carriers is a *flagged decision* (cross-project substrate; lead leans retain-with-banner — in `JOSEPH-TODO.md`); **B2 single-source the drifted disciplines** (WN-discipline → `format.sop.md`, etc.); **B3 slim `agents.sop.md`'s body** — demote its procedure restatements to pointers (orientation header + SOP index already done). The `sop.md` master-index question is resolved: `agents.sop.md` is the index.

**The SOP shift needs nothing from Joseph** — the WN-discipline "gate" and the `sop.md` master-index question were both false forks the consolidation already answers (single-source in `doc/sop/`; `agents.sop.md` *is* the index). **Decisions for Joseph → [`JOSEPH-TODO.md`](JOSEPH-TODO.md)** is the queue for the genuinely-his items, all in *other* threads (D-2 / G3 / SP-27 / SP-29, the Greek-vocab + README-v2 taste calls). D-citation is a future SOP-*content* discussion (what discipline to write into `format.sop.md`), not a placement blocker.
...
**WN-discipline — no longer a gate.** It single-sources into `format.sop.md` §Working Notes as part of the SOP shift's Phase B (the old A/B framing in [`msc/wn-discipline-coherence-pass-2026-05-31.md`](msc/wn-discipline-coherence-pass-2026-05-31.md) predates `doc/sop/`; single-sourcing is the cycle's premise, not a decision). The global `~/.claude/` copy is a separate later pass.
