...
**Reasoning trail.** `spikes/.integrated/spike-identity-sufficiency-formalization.md` carries the strengthening attempt in expanded form — the audit walkthrough behind (IS-A1)–(IS-A3), the alternative-operationalization survey for $\text{identity}_{t+1:}$, the rate-distortion bonus derivation, and the substrate-asymmetry no-go that now lives at `#hyp-substrate-transfer-asymmetry`.
