...
$$B_{\min}(S_{\text{id}}) \;\ge\; S_{\text{id}} \cdot I(\mathcal C_t; \text{identity}_{t+1:}).$$
...
- **Reasoning trail.** `spikes/.integrated/spike-identity-sufficiency-formalization.md` §4 carries the derivation in expanded form, including the explicit DPI step and the inverse-bound restatement. The spike also names the open questions on tightness, optimal compression family, and asymmetric substrate transfer (the last of which is now `#hyp-substrate-transfer-asymmetry`).
