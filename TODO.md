...
- **Deaths taxonomy — released follow-ons (cycle 2026-06-10; plan `msc/deaths-grounding-plan-2026-06-10.md`).** `#def-death-as-factor-loss` landed (factor-generated taxonomy; Phase 1–3 of the cycle). Open, roughly in leverage order: (a) ~~**agency-death actuation-half derivation**~~ **LANDED 2026-06-11** as #der-severed-actuation-dynamics (the *output* leg of factor-(iii) failure; independently verified pre-landing; carrier (D3) row → derived-conditional; spike trail `spikes/spike-severed-actuation-dynamics-2026-06-11.md`) — its own released follow-ons, in the segment's WN: the witnessing-vs-wirehead selection problem, partial-severing dynamics, learned-helplessness prior-art adoption (Seligman; Abramson-Seligman-Teasdale) before advancing past `draft`. **Agency death's *input* leg LANDED 2026-06-17** as #der-captured-objective-dynamics (objective capture = the agency-death reading of #deriv-reward-channel-learning-no-go; carrier D3 now two-legged; independently refute-verified; spike `spikes/spike-captured-objective-2026-06-17.md`) — released follow-ons in its WN, see the *mood-layer arc* entry below for the $\dot\alpha$ drift-rate derivation (which bridges to #def-mood/#der-mood-timescale), the active/passive formal unification (one operation, two factors — the sibling-paper "meat"), and partial-capture dynamics; (b) **continuity-death unified composition** — a derivation segment composing #result-persistence-condition + #deriv-persistence-cost + #der-turnover-information-recursion under the factor-(i) two-faces reading; (c) **per-death terminal forms** — reversibility analogues for (D1)/(D4) (now derived: (D2)'s barrier, (D3)'s output-leg helplessness fixed point + agent-terminal abdication, and (D3)'s input-leg captured-objective absorbing state; (D1)/(D4) remain open); (d) **deaths-as-harms norm segment** — the carrier's conditional normative paragraph is the seed (PRACTICA normative-register allowance; carried by philosophical arguments per Joseph 2026-06-10); (e) phenomenological-death adjudication (deliberately deferred); (f) terminology batch for the death names + `death-apt` (queued in TERMINOLOGY-TODO) — now also `captured-objective` / `reward-port-care` ($\mu_{\text{prox}}$) from the input leg.
...
**Anticipated segments queued (modularity cycle 2026-05-09):** *(`#disc-strategic-self-coupling` and `#disc-modularity-state-dynamics` landed 2026-05-24 — see CHANGELOG; pruned here per the prune-completed discipline.)*
...
- [ ] **`spike-pid-a2prime`** (B3; PID A2' via SPR/KYP positive-real; explicit $\alpha_{\mathrm{PID}}$) — Tier 2/3
...
- [ ] **`spike-update-operator-sector`** — orphan, **regression-checked CLEAR** 2026-05-17 (genuine deferred orphan, not corrected-away); **tractable landing pending this cycle**: $\alpha$-op/$\beta$-op refresh into `#deriv-sector-condition` + operator-layer no-go into `#disc-identifiability-floor` (present-tense canon); parent-owned §266(iii)/§8.2 placement.
...
2026-05-16 audit-routing graduation (not a Cluster-C audit finding; blocks no graduation; the standalone `audits/` backlog retired regardless).
`bin/lint-outline` currently reports **3 ordering violations + 1 missing dependency** (0 orphans):
...
- [x] **Landed this session (partial fix):** `\AlphAlph` (A..Z, then AA, AB, …)
  ...
  ("Details", Part IV) now renders correctly A…AM.** No regression to other volumes (their `*Preface*` path is untouched).
