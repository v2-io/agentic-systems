...
- **Chapter-closing discussion segments** (new pattern, pilot landed 2026-05-13) use an `impl-` *prefix*: `impl-persistence-and-limits`. The remaining 7+ AAD chapter-ends and the eventual TST / logogenic chapter-ends will follow this form.
...
Three audits dropped 2026-05-12 on the markdown-first monograph builds (`msc/codex-audit-results-2026-05-12.md` line-precise cross-corpus; `msc/gemini-audit-results-2026-05-12.md` thematic cross-corpus; `msc/gemini-aad-audit-2026-05-12.md` AAD-only math-verification — all now archived at `audits/.integrated/`). The cross-corpus audits read the *mono* builds; some findings are partly build-pipeline artifacts (preface-prose leakage; cross-component "missing" segments) rather than content defects. The surgical strengthen-first edits that landed from this intake (AAD-2/3/4/6, Gemini-AAD L1' + CIY→LMI, TST-1/2/3/5, ELI preface, 451729 Finding 1, the def-pearl Part I→II move) plus the four spike-and-integrate stages (AAD-5/7/1 + ELI-8) are now consolidated in **[CHANGELOG 2026-05-12](CHANGELOG.md)** — see "The eight surgical strengthen-first edits" ledger and the four-stage arc. Only the open items are carried below.
...
- [ ] **`spike-bridge-lemma-nonlinear-strengthening-2026-04-24` §7.2 passivity / dissipativity** — Tier 2 math, ready to land. Target: new `#dissipativity-template` appendix + Class 1/2/3 port-structure addition to `#der-directed-separation`. INDEX-tracked but not yet authored. Gated on the Group II / PROPOSALS §D.9 SP-22 architectural decision (separate appendices vs unified operator-family meta-segment vs hybrid).
...
Group III INDEX refresh and Group IV zombie archiving are done (2026-05-12 (late) → 2026-05-14). Group I is down to two items, both gated on the Group II / SP-22 architectural decision. Remaining phasing:

1. **Group II architectural decision** (PROPOSALS §D.9 SP-22 first): decide on the dissipativity / operator-sector meta-segment structure before authoring; otherwise risk parallel half-segments. Unblocks the two remaining Group I items (bridge-lemma §7.2 dissipativity; ρ-decomposition appendix).
2. **Group IV attention-pair** — Joseph's framework-scope call on `spike-attention-causal-graphs` + `spike-attention-governance` (zombie-archive vs keep as exploratory).
3. **Matrix-composition lift** (the natural-next AAD-1 follow-on) — sketched in `spikes/.integrated/spike-matrix-persistence-condition.md` §5.5 and §7. Composition machinery (`#form-composition-closure`, `#der-team-persistence`, `#deriv-critical-mass-composition`) lifted to matrix form via composite stationary covariance solving composite Lyapunov equation; expected structural finding is sub-agent-specialization-as-formal-property (a sub-agent strong on $\hat v_1$ paired with a sub-agent strong on $\hat v_2$ gives a composite strong on the spanned plane, even when each alone fails on the other's strong direction).
