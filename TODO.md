...
- [x] **AAD-5 strengthening at `#emp-update-gain`** — "any optimal adaptation must approximate" replaced with the **Fisher-local invariance regime is exact** form: for any smooth log-likelihood admitting local quadratic expansion, the natural-gradient Bayesian update at first order in the step size has the form $\eta^\ast = U_M/(U_M + U_o)$ exactly. Linear-Gaussian and conjugate-Bayesian instances are then special cases where the local expansion is globally exact. Outside the regime the direction is preserved and the first-order form is recovered locally; only global quantitative fidelity is what need not hold (`01-aad-core/src/emp-update-gain.md`). **Spike candidate (low-risk, high-value):** a clean derivation of $\eta^\ast = U_M / (U_M + U_o)$ from natural-gradient Bayesian update at first order in the Fisher metric would promote this segment from `empirical / robust-qualitative` to `derived (conditional on Fisher-local invariance regime)`. The structural result is standard information geometry (Amari) but the AAD-specific instantiation isn't carried in any current segment. Worth ~½ day of spike work; result would tighten the entire downstream tempo / persistence chain.
...
- [ ] **AAD-1 scalar-tempo propagation** — `#def-adaptive-tempo` is scalar; tensor tempo is needed for anisotropic gains, Fisher-whitened update, LMI causal-IB, and per-dimension persistence. The tensor-tempo TODO at the appendix (mono line 13003 / segment `#deriv-causal-ib-lmi`) and per-dimension repair at `#result-per-dimension-persistence` already acknowledge this. Open: propagate the scope tag ("scalar / isotropic / nonredundant-channel scope") to every downstream summary statement, and promote tensor / vector tempo to the exact-theory layer for persistence, adversarial, and composition results.
- [ ] **AAD-2 value-object monotone-hierarchy preconditions** — `#def-value-object`'s one-step / receding-horizon / full-horizon hierarchy is monotone only under fixed $M_t$, fixed evaluation convention, and nested policy classes. Headline statement should carry those preconditions explicitly; replanning under updated $M_t$ should not be treated as automatic monotone improvement.
...
- [ ] **AAD-7 strategic-equilibrium game-theoretic wording at `#deriv-strategic-composition`** — distinguish *no pure fixed point of the selected dynamics* from *no equilibrium exists*; finite games generally have mixed Nash; no-regret dynamics converge in distribution / CCE under explicit assumptions.
...
- [ ] **TST-2 Lindy / Jeffreys baseline naming** — rename "best prediction" to "uninformed median baseline" wherever it appears (preserves the accountability benefit without overstating universality).
...
- [ ] **TST-5 temporal-optimality application checklist** — add a short "equivalence checklist" near `#post-temporal-optimality` (correctness, safety, maintainability, sustainability, coordination cost, future optionality must be held equivalent before time can break ties).
...
- [ ] **ELI-8 identity-sufficiency $S_{\text{id}}$ mathematical underdefinition** — define `identity_{t+1:}` as a vector of measurable future tests tied to the five factors; state assumptions under which $0 \le S_{\text{id}} \le 1$; keep at sketch / formulation status until then.
