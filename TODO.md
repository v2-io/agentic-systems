...
- [x] ~~**AAD-3 propagation audit on "from its own data" shorthand**~~ — landed at primary site: `#def-strategy-dag` "Detecting latent common causes" paragraph rewritten to lead with "*under joint sibling observability generated by its own interventions* — not from on-policy traces alone" and to make the on-policy no-go (the load-bearing scope) explicit at the headline. Closing sentence now warns that summary references in dependent segments that elide the on-policy / interventional distinction should be read with caution. The intro segment `strategy-structure-intro.md` already carried the careful version; the def-strategy-dag headline was the loose-shorthand site that needed sharpening. Cross-segment sweep beyond these two: not yet performed; the loose-shorthand pattern was concentrated at this site per a quick grep.
- [x] ~~**AAD-4 CIY action-distinguishability scope**~~ — landed: `def-causal-information-yield` was already careful at the local level (line 29 marks the λ-weighted approximation as heuristic; line 37 names the compensation as heuristic). The propagation site needing tightening was `causal-access-intro.md`, which presented the scalar unified objective $\pi^\ast = \arg\max_a [\mathbb E[\text{value}(a)] + \lambda(M_t) \cdot \text{CIY}(a; M_t)]$ confidently as "what RL's exploration-vs-exploitation tradeoff looks like." Now carries the explicit "*The scalar form above is heuristic*" caveat plus the blank-wall attack named, the matrix-LMI strengthening at `#deriv-causal-ib-lmi` cross-referenced, and the closing line: *summary uses of the scalar form should carry the heuristic flag unless the LMI/trace-product form is in force*.
