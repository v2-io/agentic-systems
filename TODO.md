...
- [ ] **HIGH — `spike-hb-agent-opacity`:** (a) the backward variance decomposition $H_b^{A\mid B} \approx \tfrac12\log(2\pi e) + \tfrac12\log[U_{\pi,B\to A} + U_{a,A}]$ — Gaussian-exact structural dual of the Kalman $U_M + U_o$ decomposition, in no segment; (b) the five-factor adversarial-edge-targeting arg-max (canon *claims* the targeting result qualitatively in `#der-agent-opacity` but never displays the formula). Natural single landing cycle.
- [ ] **MEDIUM — the causal-Level-4 pair (`spike-causal-level-4`, `-formal`):** model-modifying-action formalism ($\sigma(M,a)$ model-transition + Level-4 planning query); hallucination-as-level-confusion ($W_t$-promoted-to-$M_t$, epistemic-tag architecture); the aspiration corollary + "shoot for the stars" gradient (infeasible $O_a$/$M^\ast$ can dominate feasible targets — Joseph's own aphorism prompt, hypothesis-grade); the $M_t$-vs-$W_t$ goal-directed-imagination structure (half-landed: `#der-directed-separation` has the two-channel structure; the $W_t$ workspace itself is nowhere).
- [ ] **MEDIUM — `deriv-l1-update-bias` truth-status defect (fix first, small):** the segment's *[Empirical claim (monte-carlo-confirmation)]* describes four scenarios/$\rho$-grid that do **not** match the simulation record it cites in the spike — either rerun the sim to the claimed spec or restate the claim to match the actual record. Also from that spike: the declared cross-segment integrations (pointer from `#disc-identifiability-floor` Instance 2) were never executed; `spikes/INDEX.md:356` still says PENDING REVIEW.
- [ ] **MEDIUM — `spike-kappa-session-residual` §7 latent-processing thread:** capture/triage/deferred three-stage observation pipeline (+ startle-reflex placement) — carried nowhere but the archived spike; candidate 03-llm-core material.
- [ ] **MEDIUM — `spike-ib-unification-plan` §5.5/§5.6:** the falsifiable heterogeneous-gain consequence (means-sum not IB-optimal when $\Delta K \neq 0$; a rotation beats it) + the derived PCA/CCA-coincidence fragment.
...
- **Directed-separation foundation pass — remaining tail** (the W₁-seed foundation pass itself landed 2026-05-31 — narrative in CHANGELOG 2026-05-31; tail durable in `#der-directed-separation` Working Notes; full design in `_obs/directed-separation-foundation-execution-plan-superseded-2026-05-31.md`):
...
- [ ] **impl-series status-tagging pass** (B-4 remainder). Alignment-check each of the 9 AAT-core `impl-*` implications against the *weakest* status of the component it imports, applying `#der-directed-separation`'s narrowest-object discipline + degradation norm. The specific verified drifts are enumerated in `impl-cooperative-adversarial` WN (items ii–vi; item i executed 2026-07-03) and the FINAL §B-4 (impl-orient-cascade F89–F91; impl-unity-communication F173/F177/F180; impl-composition-machinery / impl-strategy-structure F46/F139–F145). Add the two framing moves: one-line proof-provenance orientation at each impl chapter-open + the hub-and-spoke / three-zone proof-flow figure. This is the elegance-protecting edit; the standard is already written internally.
