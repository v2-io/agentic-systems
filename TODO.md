...
**Session reconciliation 2026-05-20→21 (pre-print prep, CHANGELOG entry of same date).** Major arcs landed: 451729 D.1 Phases 1+2+2.5+3+4a+4b (sector-condition cluster + schema-strategy-persistence + form-strategy-complexity-cost); A2' split into new `#form-sector-condition` with corpus cross-ref sweep; new exact-tier appendix `#deriv-strategic-persistence-hard-ceiling` (Prop C.1+C.2); new `#disc-constructive-impossibility-posture` + AAT Layer-0 preamble + meta-segment cross-refs; lint cleanup sweep (362 → 0 across all four src/); summary-attempt sweep (107 description-prose upgrades across ~143 segments). All four spikes archived to `spikes/.integrated/` per MANIFEST-2026-05-20. Strict-form independent-verify items added in §2026-05-10 below carry forward as queued. Full audit-findings fan-out (13 of 14 audits beyond pilot 471203) deferred per pre-print-first framing.
...
- ~~Mechanism-design Instance 5 promotion in `#disc-identifiability-floor`~~ **RESOLVED 2026-05-20** by `spikes/.integrated/spike-4th-identifiability-floor-instance-2026-05-20.md` — Outcome C, Gibbard-Satterthwaite is **not** a clean M1-fit (composition fails: GS is an existence no-go over preference profiles, not the identifiability-from-data shape M1 names). Worked attempt + rejection recorded in `#deriv-strategic-composition` Working Notes; routing to `#disc-separability-pattern` general-open tier. The candidacy as Open in §"Adjacent Floors" should be retired or re-framed; the genuine 4th candidate is Object B (PROPOSALS §D.9).
- Misspecification-cost formalization (candidate Adjacent Floor under `#disc-identifiability-floor` §"Adjacent Floors").
- ~~Kalman-Ho closed-form follow-up spike — distinct vs subsumed?~~ **RESOLVED 2026-05-18** by `spikes/spike-identifiability-floor-instance4-resolution-2026-05-18.md`: distinct — the constant-C "Instance 4" is a category error (Object A), the agent-internal-architecture candidate is the genuine floor (Object B). Disposition is the **single reserved decision** in [`PROPOSALS.md`](PROPOSALS.md) §D.9 "RESERVED DECISION (2026-05-18)" — the durable home.
- ~~ρ-factorization no-go tightening~~ **RESOLVED 2026-05-18**: the no-go is the one-line constitutive argument, landed in `#internal-external-decomposition` (CHANGELOG 2026-05-18). Cleanup folded into the same reserved decision (PROPOSALS §D.9).

**Section III / Composition:**
- `#rho-decomposition` / CL-2 replacement for the §4.1-marked `#internal-external-decomposition` (now `status: false`). **Re-scoped 2026-05-18 by independent recheck (`spikes/spike-rho-structure-recheck-2026-05-18.md`):** the (AV) (S1)–(S4) "exact theorem" is *not* exact (vacuous cross-term). It split into **(a) a LIGHT exact core** — **LANDED 2026-05-18** (`#internal-external-decomposition` rebuilt true on the two-term identity forced by canon + the one-line constitutive no-go; independent-verify confirmed the canon-forcing; CHANGELOG 2026-05-18) — and **(b) a HEAVY Joseph-reserved refinement** — the conditional 𝓜/π/cross split (Regime-C confound), provably the **same object as the identifiability-floor 4th-instance question**. (b) is now folded into the single reserved decision whose **durable home is [`PROPOSALS.md`](PROPOSALS.md) §D.9 "RESERVED DECISION (2026-05-18)"** (not the cycle working file).
...
- Heredity axiom for `#post-composition-consistency` — scoping spike to test whether the architectural strengthening (composite admissibility derivable from sub-agent properties) is worth the simplification (collapses A2' Tier structure; promotes (CM2-M) from Slotine-imported to AAT-derived).
...
- [ ] **D.1 — promotion-readiness sweep on conservatively-staged appendix segments** — *mostly landed 2026-05-20; one residual blocked-on-deps + a lint cleanup pass deferred*. The originally-named 8 segments were first narrowed to 6 Class A candidates ([`audits/audit-findings-451729.md`](audits/audit-findings-451729.md) §"D.1 disambiguation summary"): `form-consolidation-dynamics` removed (Class B, correctly conservative — §F.3 stability-upper-bound genuinely open per its own Working Notes; tracked in `audits/polish-and-sentiment-ledger.md` S25); `deriv-graph-structure-uniqueness` removed (Class C, already at `stage: claims-verified` since 2026-04-21 commit `cdb824f`, audit erratum).

    Landings (after Gate 1–4 verification — [`msc/451729-d1-gate-verification-2026-05-20.md`](msc/451729-d1-gate-verification-2026-05-20.md) — and Phase 4 strengthening spikes [`spikes/spike-schema-strategy-persistence-strengthening-2026-05-20.md`](spikes/spike-schema-strategy-persistence-strengthening-2026-05-20.md), [`spikes/spike-form-strategy-complexity-cost-strengthening-2026-05-20.md`](spikes/spike-form-strategy-complexity-cost-strengthening-2026-05-20.md)):
    - **Phase 1 ✓** — `deriv-recursive-update` → `claims-verified` (commit `1ec89d8`).
    - **Phase 2 ✓** — `deriv-sector-condition` → `claims-verified` (`1ec89d8`).
    - **Phase 2.5 ✓** — `form-sector-condition` (new, from A2' formulation split commits `aed98a4`+`843c178`) → `claims-verified` (`b3ec29e`).
    - **Phase 3 ✓** — `der-gain-sector-bridge` → `claims-verified` (`b3ec29e`).
    - **Phase 4a ✓** — `schema-strategy-persistence` `status: sketch` → `status: conditional`; new exact-tier appendix `#deriv-strategic-persistence-hard-ceiling` carrying the $\lambda$-independent structural cap at $\rho_\Sigma = R_\Sigma/2$ (`9054e9f`).
    - **Phase 4b ✓** — `form-strategy-complexity-cost` `status: discussion-grade` → `status: robust-qualitative`; `deriv-strategy-cost-regret-bound` added to `depends:` (`351ed95`).
    - **Phase 5 (residual, blocked)** — `deriv-edge-credence-dynamics` remains at `stage: draft`. Gate 1 staging-monotonicity blocked on multiple deps at draft (`schema-strategy-persistence` stage unchanged despite Phase 4a status advance; `hyp-edge-update-via-gain`; `scope-and-or`). Unblocking requires a stage-promotion sweep on those lower-level deps (not in D.1; needs its own scoping cycle).
    - **Phase 6 (residual, deferred)** — lint cleanup on Gate-3-failing segments (`deriv-sector-condition` 13 bare-Greek-in-prose, `der-gain-sector-bridge` 6 raw `<`/`>` + 1 bare-Greek, `schema-strategy-persistence` 4 pre-existing bare-symbol). Blocks `format-clean` not `claims-verified`; separate hygiene pass.

- [ ] **Strict-form independent-verify next-steps queued post-Phase-4** (per `doc/audit-routing-instructions.md` §8 Refinement 4):
    - **Cor A.1S.1 Model-S half + §D.3 exact form** — fresh-mathematician re-derives both from scratch before any further status elevation on `deriv-sector-condition` or `schema-strategy-persistence`. Current `exact`/`conditional` tiers rest on the 451729 auditor's substrate + Phase 4a spike's first-principles re-derivation, not on an external second-pass.
    - **Hobson 1969 / Csiszár 1991 PDF spot-check + BH-identity one-line algebra** — for `form-strategy-complexity-cost`'s elevation to `robust-qualitative`. Spike 4b §5 flagged that `deriv-strategy-cost-regret-bound`'s Working Notes record a prior mis-citation that was corrected; verification on the *corrected* citations is owed. BH-identity $D_\text{KL}(\delta_{a^*} \| Q) = -\log Q(a^*)$ is one-line and cheap.
    - **`deriv-strategic-persistence-hard-ceiling` Prop C.1 + C.2 algebra** — fresh-mathematician re-derives both from scratch + reads `#deriv-edge-credence-dynamics` Prop B.1 first-hand. Current `status: exact` rests on the Phase 4a spike's independent re-derivation + parent spot-check; strict-form second-eye-on-canon-write was deferred per Joseph's "staged is better than perfect."
...
- [ ] **`spike-rho-factorization` + `spike-rho-additive-variance-strengthening-2026-04-24` paired** — **Now `CL-2` (spike-routing 2026-05-17).** `spike-rho-factorization` is a true **no-go** (multiplicative ρ-factorization does not hold). The §4.1 honesty-mark is **already done**: `#internal-external-decomposition` was asserting the refuted multiplicative split and is now `status: false` (KNOWN-FALSE banner; CHANGELOG 2026-05-17). **Re-scoped 2026-05-18 by independent recheck** (`spikes/spike-rho-structure-recheck-2026-05-18.md`): the no-go is a one-line constitutive category error (stronger than the survey); the exact replacement is **not** the (AV) (S1)–(S4) theorem (that "exact" is vacuous) but a **two-term identity already forced by canon** — a *light* landing gated only by independent-verify. The *only* heavy/Joseph-reserved part is the conditional 𝓜/π/cross split, which **is the identifiability-floor 4th-instance question** (same object). Tracked in the spike-routing tracker + PROPOSALS SP-22 STATUS.
...
- [ ] **`spike-neutral-drift-endogenous-coupling-strengthening-2026-04-24`** — spike-routing 2026-05-17: **Joseph-reserved batch** (the `#disc-identifiability-floor` 4th-instance triage-contradiction present-truth call). Not auto-routed.
