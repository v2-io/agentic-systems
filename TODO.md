...
- **Deaths taxonomy — released follow-ons (cycle 2026-06-10; plan `msc/deaths-grounding-plan-2026-06-10.md`).** `#def-death-as-factor-loss` landed (factor-generated taxonomy; Phase 1–3 of the cycle). Open, roughly in leverage order: (a) **agency-death actuation-half derivation** — what precisely is severed in AAT coordinates and what an agent with intact adaptation but severed actuation converges to; the taxonomy's main strengthening target (its WN carries the candidate handles incl. #def-control-regret's capability-limit reading); (b) **continuity-death unified composition** — a derivation segment composing #result-persistence-condition + #deriv-persistence-cost + #der-turnover-information-recursion under the factor-(i) two-faces reading; (c) **per-death terminal forms** — reversibility analogues of (D2)'s derived absorbing barrier for (D1)/(D3)/(D4); (d) **deaths-as-harms norm segment** — the carrier's conditional normative paragraph is the seed (PRACTICA normative-register allowance; carried by philosophical arguments per Joseph 2026-06-10); (e) phenomenological-death adjudication (deliberately deferred); (f) terminology batch for the death names + `death-apt` (queued in TERMINOLOGY-TODO).
