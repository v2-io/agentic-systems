...
- `#rho-decomposition` / CL-2 replacement for the §4.1-marked `#internal-external-decomposition` (now `status: false`). **Re-scoped 2026-05-18 by independent recheck (`spikes/spike-rho-structure-recheck-2026-05-18.md`):** the (AV) (S1)–(S4) "exact theorem" is *not* exact (vacuous cross-term). It splits into **(a) a LIGHT exact core** — the two-term identity `ρ²=ρ_⋆²+Δ_agent²` already forced by canon (`#result-mismatch-decomposition` GA-1 + Prop A.1S Itô generator) + the one-line constitutive no-go; gated only by an independent-verify of the canon-forcing, then a light landing (no information geometry, no new appendix machinery) — and **(b) a HEAVY Joseph-reserved refinement** — the conditional 𝓜/π/cross split (Regime-C confound), which is provably the **same object as the identifiability-floor 4th-instance question**. Tracked in the spike-routing tracker.
