...
- [ ] **D.1 — promotion-readiness sweep on conservatively-staged appendix segments** — `deriv-recursive-update`, `deriv-sector-condition`, `der-gain-sector-bridge`, `deriv-edge-credence-dynamics`, `form-strategy-complexity-cost`, `schema-strategy-persistence` are at `stage: draft` despite complete formal content + "What Is Derived vs. What Is Chosen" tables + no Working-Notes blocking promotion. Either real unresolved issues should be surfaced, or a promotion sweep would improve corpus self-description. Gate 1–4 verification pass in flight (parallel agent) on these 6 Class A candidates. *Narrowed from the FINAL's list of 8 on 2026-05-20 after first-hand spot-check ([`audits/audit-findings-451729.md`](audits/audit-findings-451729.md) §"D.1 disambiguation summary"): `form-consolidation-dynamics` removed — Class B, correctly conservative — its §F.3 stability-upper-bound is genuinely open per its own Working Notes ("Stability upper bound derivation (open)") and is tracked as durable open-theory in `audits/polish-and-sentiment-ledger.md` S25; `deriv-graph-structure-uniqueness` removed — Class C, already at `stage: claims-verified` since 2026-04-21 (commit `cdb824f`), inclusion in the FINAL §D.1 list was an audit erratum.*
