...
- `#rho-decomposition` appendix promotion — (AV) variance-additive theorem + sub-regime catalog (Poisson cascade MC; large-deviation tail LD; small-Δ / PID); pairs with `#disc-additive-coordinate-forcing`. **= CL-2** (spike-routing 2026-05-17): this is the *replacement* for the §4.1-marked `#internal-external-decomposition` (now `status: false`); heavy; carries a Joseph-reserved Instance-5 decision. Tracked in the spike-routing tracker.
