...
- [ ] **rho fine-split landing (opened 2026-08-22).** `spikes/spike-rho-factorization.md` §5.4 ($f/g$ entanglement), §11 (small-$\Delta$ first-order log-additive regime), §8.2/§12 (R-KL Pythagorean conditions) and the three worked cases (§§3–5) are the "under separate investigation" item `#internal-external-decomposition` still carries; the spike's (R-V) form is superseded by the exact three-term decomposition. Landing agent in flight; verifier ≠ author before the spike files to `.integrated/`. Delete this item when the landing commits.
...
- [ ] **impl-series status-tagging pass** (B-4 remainder) — **6 of 9 AAT segments remain** (`impl-cooperative-adversarial`, `impl-orient-cascade`, `impl-unity-communication` landed 2026-08-22, CHANGELOG; that entry also records the method — editor + different-substrate verifier + regression pickaxe + WN reduced to guards). Remaining: `impl-persistence-and-limits`, `impl-causal-access`, `impl-strategy-structure` (F46 + "unique broadly-available escape"), `impl-strategy-dynamics`, `impl-composition-machinery` (F139–F145), `impl-strategic-composition`; then TST ×4 and LLM ×4. Still open across all: the hub-and-spoke / three-zone proof-flow figure (figure pipeline). Alignment-check each implication against the *weakest* status of the component it imports — strengthen-first where the source earns more than the prose claimed.
