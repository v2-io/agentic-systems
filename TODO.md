...
- **Deaths taxonomy — released follow-ons (cycle 2026-06-10; plan `msc/deaths-grounding-plan-2026-06-10.md`).** `#def-death-as-factor-loss` landed (factor-generated taxonomy; Phase 1–3 of the cycle). Open, roughly in leverage order: (a) ~~**agency-death actuation-half derivation**~~ **LANDED 2026-06-11** as #der-severed-actuation-dynamics (independently verified pre-landing; carrier (D3) row → derived-conditional; spike trail `spikes/spike-severed-actuation-dynamics-2026-06-11.md`) — its own released follow-ons, in the segment's WN: the witnessing-vs-wirehead selection problem, partial-severing dynamics, learned-helplessness prior-art adoption (Seligman; Abramson-Seligman-Teasdale) before advancing past `draft`; (b) **continuity-death unified composition** — a derivation segment composing #result-persistence-condition + #deriv-persistence-cost + #der-turnover-information-recursion under the factor-(i) two-faces reading; (c) **per-death terminal forms** — reversibility analogues for (D1)/(D4) ((D2)'s barrier and (D3)'s helplessness fixed point are now both derived); (d) **deaths-as-harms norm segment** — the carrier's conditional normative paragraph is the seed (PRACTICA normative-register allowance; carried by philosophical arguments per Joseph 2026-06-10); (e) phenomenological-death adjudication (deliberately deferred); (f) terminology batch for the death names + `death-apt` (queued in TERMINOLOGY-TODO).
