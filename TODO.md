...
- **Segment counts** — last refreshed 2026-04-24 at 111 AAD-core segments; 2026-04-25 added 3 (`#internal-external-decomposition`, `#deriv-causal-ib-exploration`, `#deriv-causal-ib-lmi`) to ~114. The "What's Settled" summary that previously lived in CLAUDE-2.md (sunset 2026-04-28) is now in `msc/FINDINGS-RANKED-DRAFT.md` (curated catalog) and segment-level `## Findings` sections; segment counts themselves are not preserved as a tracked quantity in any single file — `bin/lint-outline` provides current counts on demand. Refresh-as-quantity is no longer needed; remove this TODO entry on next cleanup pass.
