...
- **Directed-separation foundation pass — remaining tail** (the W₁-seed foundation pass itself landed 2026-05-31 — narrative in CHANGELOG 2026-05-31; tail durable in `#der-directed-separation` Working Notes; full design in `.archive/directed-separation-foundation-execution-plan-superseded-2026-05-31.md`):
...
**Tier 3 — soft scope/register tightenings:** remainder of A13 F147–F172 (F173–F181 landed in `#impl-unity-communication` 2026-08-22; A10 F88–F91 landed in `#impl-orient-cascade` the same day), A9 Codex F47–F77. Each lives in its segment's `#### Off-ramp` WN block.


## Strategic-composition cluster residuals (post-NEXT-UP archival, 2026-05-25)

The strategic-composition cluster completed 2026-05-25 (narrative: CHANGELOG 2026-05-25; navigator archived at `spikes/.integrated/NEXT-UP-archived-2026-05-25.md`). Two residuals remain in this misc-and-details layer:
...
Sources:
- Audit's full findings: `msc/naming/naming-votes/audit-471203-incremental.md` (segments 1–46 only as of 2026-04-28; will be re-extracted after audit FINAL lands).
- Aggregator's cross-architecture +3 keep votes for Greek terms (illustrating the defended-by-synoptic-voters posture): `msc/naming/_archive/naming-aggregate-r2-review.md`.
...
- **Variables $\alpha$, $\rho$, $R$ appear without gloss.** Alan: *"I've seen this formula twice now, but still don't know what the variables mean."* First mention of the persistence condition should anchor each variable in plain language; the *Cross-Domain Joining* table (which uses $\alpha$, $\rho$, $R$ without re-glossing) and the *Position & Lineage* paragraph (which mentions $\alpha \gt \rho/R$ as a structural threshold) should both re-anchor briefly. Glossing once at top is not enough across a long document.
...
2. Anchor variables ($\alpha$, $\rho$, $R$, $\delta_t$, $\eta^\ast$, $\mathcal T$, $M_t$, $G_t = (O_t, \Sigma_t)$) on first mention with one-line plain-language glosses; re-anchor in the *Cross-Domain Joining* table and any later use.
...
- **Deaths taxonomy — released follow-ons (cycle 2026-06-10; plan `msc/deaths-grounding-plan-2026-06-10.md`).** `#def-death-as-factor-loss` landed 2026-06-10; agency death's output leg #der-severed-actuation-dynamics (2026-06-11) and input leg #der-captured-objective-dynamics (2026-06-17) both landed — narratives in CHANGELOG. Open, roughly in leverage order: (a) **the two landed legs' released follow-ons**, in each segment's WN — output leg: witnessing-vs-wirehead selection problem, partial-severing dynamics, learned-helplessness prior-art adoption (Seligman; Abramson-Seligman-Teasdale) before advancing past `draft`; input leg: the $\dot\alpha$ drift-rate derivation (see the *mood-layer arc* entry below — bridges to #def-mood/#der-mood-timescale), the active/passive formal unification (one operation, two factors — the sibling-paper "meat"), partial-capture dynamics; (b) **continuity-death unified composition** — a derivation segment composing #result-persistence-condition + #deriv-persistence-cost + #der-turnover-information-recursion under the factor-(i) two-faces reading; (c) **per-death terminal forms** — reversibility analogues for (D1)/(D4) (now derived: (D2)'s barrier, (D3)'s output-leg helplessness fixed point + agent-terminal abdication, and (D3)'s input-leg captured-objective absorbing state; (D1)/(D4) remain open); (d) **deaths-as-harms norm segment** — the carrier's conditional normative paragraph is the seed (PRACTICA normative-register allowance; carried by philosophical arguments per Joseph 2026-06-10); (e) phenomenological-death adjudication (deliberately deferred); (f) terminology batch for the death names + `death-apt` (queued in TERMINOLOGY-TODO) — now also `captured-objective` / `reward-port-care` ($\mu_{\text{prox}}$) from the input leg.
- **Mood layer + prospective-mood arc — released follow-ons (cycle 2026-06-17).** `#def-mood` (Part I second-order adaptation parameter, `discussion-grade`) + `#der-mood-timescale` (the $\tau^\ast \propto \sqrt{\tau_{\mathrm{env}}}$ law, `conditional`) landed; the applied/normative half is `msc/mood-layer-sovereignty-carve-2026-06-17.md` (not canon); the prospective half is exploratory at `spikes/spike-prospective-mood-two-channel-borrowing-2026-06-17.md` (read-only on canon). Open, in leverage order: (i) **the $\dot\alpha$ drift-rate derivation** — promote #der-captured-objective-dynamics Result 1 from a no-restoring-force argument to a rate, derived under an explicit reward/affect-loop model (the #def-mood integrator feeding orient-cascade step-5d revision); this is the single highest-leverage next move because it *both* advances the capture segment *and* formally closes the loop back to #def-mood/#der-mood-timescale — the mood work and the deaths work meet in one derivation; (ii) the **two-layer mood-sets-gain composition** (mood as estimator vs mood as modulator of $K$ — confirm the same $\tau^\ast$ governs) — *advanced 2026-07-03:* a leading-order certainty-equivalence closure sketch (same $\lambda^\ast$ under symmetric local-quadratic loss; fails exactly at the asymmetric-loss boundary of item (iv)) is recorded in `audits/AUDIT-WORKING-731548/verify/06-mood-integration-verdict.md` — needs one independent re-derivation before landing as the corollary; the MG-(1)-(4) discharge itself executed in `#def-mood` Discussion 2026-07-03 (audit 731548); (iii) the **prospective-mood dynamics** proper (the two-basin/equilibrium-selection structure of hope; the burnout-crossover conjecture's premise needs support or replacement by the verified affect→investment arrow); (iv) the **asymmetric-loss** version of #der-mood-timescale (deaths-weighted vigilance shifting $\lambda^\ast$); (v) the **active/passive formal unification** of `#disc-strategic-self-coupling` (active) and capture (passive) as one factor-(iii)/(iv) co-failure. The worthiness axiom ("some goals are worthy independent of realization") is explicitly the philosophical track's, not the formalism's.
...
**Naming-cycle record packed 2026-08-22** at [`.archive/msc-naming-2026-08-22.tar.gz`](.archive/msc-naming-2026-08-22.tar.gz) (formerly `msc/naming/`). Unpack at the repo root to restore the tree. Inner paths below are relative to that unpack.

Status (updated 2026-05-09): R1 + R2 voting cohorts closed; manual canonicalize-curation pass landed 103 of 118 candidates across 8 batches into `msc/naming/naming-rename-plan.md`. Remaining 13 rows live in `msc/naming/to-canonicalize.md` (D-deferred citability special cases + ??? rows on separability-triad-rung-naming pending Joseph's call). Master-list `rename_status` field tracks all decisions; 88 currents marked across canonicalize / rename / excluded statuses. **Curation-pass full narrative in [CHANGELOG 2026-05-04](CHANGELOG.md).** **§A slug renames + §B prose-vocab renames execution-status: see [CHANGELOG 2026-05-09](CHANGELOG.md) and [TERMINOLOGY-TODO.md](TERMINOLOGY-TODO.md).**
...
Detail in `msc/naming/naming-rename-plan.md` (renamed from `naming-pilot-rename-plan.md` on 2026-05-04 to reflect the broadened scope from role-prefix-pilot mapping to all naming decisions); principles in [`doc/sop/naming.sop/principles.sop.md`](doc/sop/naming.sop/principles.sop.md); Round-1 vote archaeology in `msc/naming/naming-votes/` and `msc/naming/naming-aggregate-*`.
...
2. **Rename existing `-intro` segments to `intro-…` prefix form** for consistency with the broader prefix discipline. Mechanical sweep using `bin/rename-slug` once that tool's outstanding bug (above) is fixed. Touches ~10 segments across all four components. Pilot-then-sweep: rename one, verify links resolve everywhere, then batch the rest.
...
- [ ] **Cross-component `#slug` resolution in the build — open question.** The two 2026-05-12 cases (`hyp-checkpoint-forking-failure-modes`, `hyp-experiential-training`) were fixed by OUTLINE wiring (2026-07-14), not by the resolver; whether `bin/build-monograph` resolves references against every component `src/` or only the volume's own was not determined (2026-08-22). Settle by reading `bin/lib`, then either close or fix.
...
Triage narrative and completed dispositions live in CHANGELOG (2026-05-12 (late) → 2026-05-21) and `spikes/.integrated/MANIFEST-*`; **authoritative per-spike disposition: [`PROPOSALS.md` §D.9 SP-22 STATUS (2026-05-17/18)](PROPOSALS.md) + `spikes/ROUTING.md`.** Open items only:

- [ ] **`spike-bridge-lemma-nonlinear-strengthening-2026-04-24` §7.2 passivity / dissipativity** — Tier 2 math, ready to land. Target: `#dissipativity-template` appendix + Class 1/2/3 port-structure addition to `#der-directed-separation`. **`CL-1` (spike-routing 2026-05-17):** §7.1 is integrated (spike filed), but §7.2 is a *coupled* landing with `spike-passivity-composition` + `spike-pid-a2prime` — one `#dissipativity-template` integration-plan, **not** three independent half-segments. Heavy; tracked in `spikes/ROUTING.md` + PROPOSALS SP-22 STATUS.
- [ ] **`spike-passivity-composition`** (B2; Willems passivity for heterogeneous Kalman+PID composition) — Tier 2; part of CL-1.
- [ ] **`spike-pid-a2prime`** (B3; PID A2' via SPR/KYP positive-real; explicit $\alpha_{\mathrm{PID}}$) — Tier 2/3; part of CL-1.
- [ ] **`spike-update-operator-sector`** — orphan, regression-checked CLEAR 2026-05-17 (genuine deferred orphan, not corrected-away); **tractable landing pending**: $\alpha$-op/$\beta$-op refresh into `#deriv-sector-condition` + operator-layer no-go into `#disc-identifiability-floor` (present-tense canon); parent-owned §266(iii)/§8.2 placement.
...
Surfaced by Joseph (recurring, has been forgotten several times). scrbook's native `\appendix` numbers appendix chapters `\Alph` (A..Z) and hard-errors
*"Counter too large"* at the 27th — vol-1 has ~45 appendix chapter-level segments across **two** `## *Appendices*` H2 groups (Part IV "Details",
Part V "Operational Domains"). kaobook already solved this
(`bin/lib/typeset.rb:344-348`: `\AlphAlph` + a kaobook-only
`\asfAppendixToCremap` ToC down-shift); scrbook had simply never applied the project's chosen scheme.

*(Partial fix already landed: `\AlphAlph` defined in `mono/scrbook/preamble/setup.tex` and emitted once after `\appendix` in `bin/lib/typeset_scrbook.rb` `when 'Appendices'` (guarded by `@appendix_emitted`); group 1 "Details" renders correctly A…AM. Context for the open bug below.)*

- [ ] **Open bug — second `*Appendices*` group collapses.** Part V
  ("Operational Domains") entries all render the overflowed native `\Alph`
  form. `.toc` evidence (decisive): `\numberline {B\GenericError{ }{LaTeX Error: Counter too large}…}` with hyperref anchors `appendix.Alph40…45`
  — i.e. the real chapter counter *does* advance (40–45) but `\thechapter`
  is back to native `\Alph` there, **not** `\AlphAlph`. Root-cause hypothesis: the `\renewcommand{\thechapter}` is emitted only on the first
  `*Appendices*` group (`@appendix_emitted` guard); KOMA's appendix machinery and/or the second `\part` re-establishes
  `\thechapter=\Alph{chapter}`, clobbering the override for group 2. Fix candidates: (i) re-emit the `\renewcommand` on **every** `*Appendices*`
  group, not just the first; or (ii) hoist a permanent rebind into the preamble (after `\appendix` semantics are known) so nothing can clobber it. Verify KOMA `\appendix`+`\part` interaction before choosing.
- [ ] **Also requested (Joseph 2026-05-17), parked with this:** appendices in the ToC should read *as if they were segments* (section-register indent/weight) while staying `\chapter` in the body. Mechanism already exists in kaobook: `\asfAppendixToCremap`
...
- [ ] **Design decision Joseph flagged — pick before finishing.** Either
  **(1)** continue the single established `\AlphAlph` counter across both groups (group 2 → AN, AO, …); or **(2)** restructure appendices into per-group numbering — e.g. `A.1`–`A.10`, `B.1`–`B.10` (group letter +
  within-group number, counter resets per `*Appendices*` group). Option (2)
  also addresses the legibility smell of ~45 flat appendices and would make the second-group collapse moot (per-group reset sidesteps the >26 overflow entirely). Monograph-convention call, not mechanical — Joseph's decision gates the final implementation.
- Cross-refs for whoever picks this up: kaobook reference impl
  `bin/lib/typeset.rb:344-348`; the `when 'Appendices'` branch in
  `bin/lib/typeset_scrbook.rb` (~lines 266-280) carries an inline comment explaining why `\asfAppendixToCremap` was *initially* excluded (separate concern) — that comment needs updating once the ToC-as-segments port lands, since Joseph has now requested it.

## 2026-07-03 — Audit 731548 (Fable de-novo, Part I + impl-*) routing: remaining routed items

Tier-1 correctness (B-1, B-2, B-4 Regime-III) executed and in CHANGELOG 2026-07-03; B-3 is PROPOSALS SP-30 (needs Joseph). Routed here (from the FINAL's Tier-2/3, each verified by the audit's independent round — route from `audits/audit-731548-FINAL-2026-07-02.md`, not the working dir's running outline):

- [ ] **impl-series status-tagging pass** (B-4 remainder) — **3 of 9 AAT segments remain** (`impl-cooperative-adversarial`, `impl-orient-cascade`, `impl-unity-communication` landed 2026-08-22 pilot; `impl-persistence-and-limits`, `impl-causal-access`, `impl-strategy-structure` landed 2026-08-22 batch two — CHANGELOG records the method: inventory of audit reports / audit gold / spikes / trackers / the AISI corpus first, then a fork writer that read the chapter whole, then a different-substrate verifier, regression pickaxe, WN reduced to guards + lifted gold). Remaining: `impl-strategy-dynamics`, `impl-composition-machinery` (F139–F145), `impl-strategic-composition`; then TST ×4 and LLM ×4. Still open across all: the hub-and-spoke / three-zone proof-flow figure (figure pipeline). Alignment-check each implication against the *weakest* status of the component it imports — strengthen-first where the source earns more than the prose claimed.
- [ ] **Source-segment defects reported by the batch-two impl passes (2026-08-22; whole-read + pickaxe before fixing):** `#disc-sandbox-evaluation-ceiling` — prior art (Bellot / Richens / Everitt, ICML 2025, same CHT anchor) means its Novelty Claim should be retired; frontmatter `discussion-grade` vs an exact-conditional body; cites R-443 Thm 1 for an iff that is Thm 5 / Cor. 1–2; the limited-target-experiments clause. `#def-pearl-causal-hierarchy:41` belief-about-L2 half-sentence. `#scope-ciy-observational-proxy` $\Omega_t$ population-identity sentence. `#norm-explicit-strategy-condition` F35. `causal-access-intro:29` $\lambda$ parameterization. Hafez IDT paper: acquire and check the author list (`ref/INDEX.md:103`; `relata fetch hafez-2026-informational --apply`). `#def-strategy-dag` ES ~:153 "edge independence is a consequence of causal sufficiency" (multi-sentence form of the :167 fix). `#scope-and-or` F41 $\gamma$-test / completeness≠compactness (S24 strengthen-first), and its ES cites internal spike paths (D-1). `#disc-additive-coordinate-forcing` table :43 lists the chain layer as Cauchy-FE-forced while its prose/ES call it the anchor identity. The L1$'$ mixing weight's Regime-B identification gap (963715). `#form-sector-condition:50` — split the $R$-as-capacity sentence by sub-scope ($\alpha$ derived / $\beta$ posited) and add the dynamical-vs-representational capacity paragraph (731548/28). `#der-temporal-nesting` — add mood as a rung in the timescale table and discharge its separation claim via `#der-multi-timescale-stability`'s $\epsilon_{\max}$ (731548/34). Two findings dispositioned "PROPOSALS candidate" in 2026-05 that never entered any tracker (grep-verified): 193847 F6 / 731548 (lxxxiii) IB-vs-structural-reserve reconciliation (`#form-information-bottleneck` ↔ `#result-structural-adaptation-necessity`); 193847 F5 `norm-adaptive-reserve-as-ethical-floor` → `04-eli-core`. From the 2026-08-22 landscape survey (`ref/prior-art-analysis/agentic-systems-landscape-2026-08-22.md`): `#der-agent-opacity` says it *adopts* Hafez's $H_b$ but defines a different object (observer/horizon-indexed) — state it as an AAT-native refinement, not an adoption; the "ICLR 2026" venue attached to Hafez 2603.01283 is not on arXiv — verify or drop. Adoption/citation targets the survey names for strengthen-first passes: Richens et al. 2025 (competence ⇒ world model; converse of `#form-agent-model`), Baltieri & Suzuki 2025 (do-surgery challenge to `#scope-agency`), the Virgo/Baltieri good-regulator theorem 2508.06326 (closest published analog to directed separation — unread), Eslami–Yu delay/dwell stability budget (strengthening target for `#result-persistence-condition`), Jørgensen–Weichwald–Hammond non-emergence lemma (Part III).
- [ ] **Source-segment defects surfaced by the 2026-08-22 impl batch** (each needs a whole-read + regression pickaxe before editing; detail in CHANGELOG 2026-08-22): `#hyp-auftragstaktik-principle` (F162/F165); `#hyp-communication-gain` (known-adversary gap; refuted distributed-tempo sum in its trust-transitivity WN); `#result-unity-closure-mapping` WN route count (F151); `#obs-gated-tempo-advantage` + `#result-adversarial-exponent-regimes` leads (one-sided noise-injection prescription from a both-channels simulation); `#result-contraction-template` (scope the strategic-half claim to the shared-truth state variable; (CC-feedback) *iff* vs sufficient); `#deriv-causal-ib-exploration` ("physically cannot stop exploring"); `#deriv-causal-ib-lmi` intro caveat + the $\tfrac12\operatorname{Tr}(\mathcal I_o P_{ss})$ trace-identity strengthening (candidate spike); `#result-coupled-diagnostic-framework` (causally-disciplined re-scope not propagated); `#der-tempo-composition` ($C_{\text{coord}}$ denominator inconsistency); `#deriv-persistence-cost` ($C \geq \mathcal T/2$ per-dimension reading); `#der-bounded-objective-as-sanity-criterion` (cite `#der-captured-objective-dynamics`); `impl-closed-loop-interiority` (03) depending on a Vol-4 segment.
- [ ] **Targeted-legacy quantifier/gloss/status-tag sweep** (FINAL §F, Tier-3 #7). Grep result/discussion segments for `iff` / `only if` / `necessary` / `exactly` / `reducible` / `unique broadly-available` / `for free` / `structural, not` and attack each against the math it summarizes. Prioritize the *oldest, least-recently-reworked* prose — the audit's post-report spot-check verified the defects are legacy escapees (TFT-inherited language), not a systemic gap; recently-reworked segments pass impeccably. B-1/B-2 were both this species.
- [ ] **Floor-routing diagnostic segment** (FINAL §F, Tier-3 #8a). Five independent auditor-arrivals converge: take "persistent high mismatch" as input, route to {raise gain / keep learning / fix sensors / change model class} with the discriminating observable per branch. The three-way routing is now stated compactly in the B-2 verifier verdict §2.3 (structured residuals ⇒ class/estimation; white + CIY-movable ⇒ act; white + unmovable ⇒ channel); the pieces are in canon post-B-2 — the segment composes them.
- [ ] **Volume-4 shadow-catalogue appendix** (FINAL §F, Tier-3 #8b). Ten Part-I theorems with Volume-4 readings that inherit Part-I's epistemic tier (exit-certainty→resurrection-not-fortress; the five failure-modes→the five deaths; temporal-nesting-violation→chronic-anxiety; etc. — list in FINAL §F). A half-page appendix cataloguing the shadow-pairs, each arriving pre-armored with Part-I rigor.
- [ ] **SOP additions** (FINAL §G, Tier-3 #9): (a) Gate-2 checklist addition — "instantiate a concrete system against each membership/exclusion clause" (produced B-3 at near-zero cost; independently endorsed by the B-3 verifier); (b) de-novo SOP — instruct auditors to defer `### Incidental audit gold` WN blocks to a post-reflection second pass and read them adversarially (the gold-lift vs de-novo-posture tension, FINAL §G).
- [ ] **INDEX "FULLY RESOLVED" spot-check** (FINAL §F). The $\rho_\star$ mismark showed a resolved-marked row can carry a false gloss introduced by a later silent redefinition; spot-check other FULLY RESOLVED rows whose landing involved a definitional change.
- [ ] **Next de-novo cycle target**: AAT Part II or III main chains, or the untested worked examples (`example-kalman/bandit/strategy` — the auditor's one untested prediction is a numeric slip there). Different finding-species predicted (derivation gaps rather than statement-layer drift).
