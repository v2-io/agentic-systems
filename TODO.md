...
- [ ] **`spike-bridge-lemma-nonlinear-strengthening-2026-04-24` §7.2 passivity / dissipativity** — Tier 2 math, ready to land. Target: new `#dissipativity-template` appendix + Class 1/2/3 port-structure addition to `#der-directed-separation`. **No longer gated**: the SP-22 architectural question was resolved 2026-05-14 (the operator-family unification push) — the answer is (γ)-hybrid, so this lands as its own `#dissipativity-template` appendix at its INDEX-stated target, not bundled and not waiting on a meta-pattern decision. Straight authoring now.
- [ ] **`spike-rho-factorization` + `spike-rho-additive-variance-strengthening-2026-04-24` paired leakage** — lands as the `#rho-decomposition` appendix; variance-additive (AV) is a derived theorem under (S1)-(S4). **No longer gated**: per the 2026-05-14 SP-22 resolution it is a (γ)-hybrid backlog landing at its own target, not inside an operator-family meta-pattern decision.
