...
1. **Hardcoded old directory name.** `bin/rename-slug:50` lists `04-eli-core/src` in `COMPONENT_SRCS` and `04-eli-core/OUTLINE.md` in `COMPONENT_OUTLINES`. The directory was renamed to `04-eli-core/` on 2026-05-01 (`fa63616`). Currently the script silently skips the missing dir — meaning any rename targeting a slug inside `04-eli-core/` segments would not touch those files at all. Fix: update both constants to `04-eli-core/...`.
