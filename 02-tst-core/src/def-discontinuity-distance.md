...
Definitional. The hierarchy is a structural observation about how software is organized, not a derived quantity. The claim that each boundary crossing increases cost is the separate claim in #der-change-proximity-principle.
...
In AAT terms, each boundary crossing increases the cost of constructing the relevant portion of $M_t$ — the agent must load more context to hold both changes in working state simultaneously.
