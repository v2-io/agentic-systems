...
A subsystem with negligible change probability consumes zero future development time. In AAD terms: its environment change rate $\rho \to 0$, so from #result-persistence-condition, any nonzero tempo suffices — the subsystem is permanently adapted. No adaptive capacity needs to be allocated to it.
