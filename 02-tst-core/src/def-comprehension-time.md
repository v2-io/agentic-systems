...
For a feature $F$ ( #def-feature) applied to system $S$:
...
Definitional. The quantity is well-defined and measurable in principle (though rarely measured in practice). The interpretation in AAT terms (see Discussion) is structurally motivated but not formally derived.

## Discussion

**In AAT terms, comprehension is constructing $M_t$.** When a developer (or AI agent) begins work on a feature, they must build a model of the relevant portion of the codebase — its structure, dependencies, conventions, state. This is precisely the construction of $M_t$ ( #form-agent-model) from environmental observations. The time to comprehend is the time to build a model of sufficient quality ( #def-model-sufficiency) to support effective action.
