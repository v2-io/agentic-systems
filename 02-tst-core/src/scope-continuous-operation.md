...
**Connection to AAD.** Perturbations are environmental disturbances ($\rho$) in the operational domain. The persistence condition ( #result-persistence-condition) applies: the team's operational tempo must exceed the disturbance rate relative to acceptable mismatch. A team that cannot recover faster than failures accumulate is in the unmaintainability regime.
