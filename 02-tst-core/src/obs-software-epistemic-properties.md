...
*Consequence — quantities exactly computable from $\mathcal{C}_t^{\text{commit}}$.* The following AAT- and TST-relevant estimators are exact functions of $\mathcal{C}_t^{\text{commit}}$, and hence are exactly recoverable from git with no sampling loss and no recall bias *as estimators of their own definitions*:
...
3. **Chronica completeness treated as definitional.** The P5 conditional-maximality result — where $\mathcal{C}_t^{\text{commit}}$ is the unique maximal exteriorized subset under current standard protocols — is a software-specific result. Other domains lack universal content-addressed cryptographic recording; any AAT-core quantity that is exactly computable from the committed-state subset in software is approximate in proportion to recording-fidelity loss elsewhere.
