...
The distinction is sharp because it separates identifiable regimes: code-internal tests are deterministic functions of code (Level 3 literal); agent-coupled questions require the past trajectory of external events (Level 3 proxy at best). Literal Level 3 access on a non-trivial class of questions requires the **conjunction** of three properties: (α) deterministic outcome functions under fixed configuration; (β) intervention cost commensurate with original implementation cost (replay must be feasible, not merely expressible); (γ) content-addressed cryptographically immutable environment state (replay must be reconstructible to bit-identity). Among adaptive domains in *current standard practice*, software is the **unique** instance where all three jointly hold: formal-verification environments satisfy (α) but typically not (β) at the cost-commensurate level; simulated physical systems and digital twins satisfy (α) and partially (β) but not (γ) under standard tooling; reproducible laboratory and robotics setups satisfy (α) only narrowly and not (β) or (γ); blockchain systems satisfy (γ) but lack (α) over agent-targeted outcomes. The uniqueness is *configurational* rather than *necessary*: a future tooling regime that brought a simulator under content-addressed cryptographic recording with commensurate replay cost would join software in this configuration. The uniqueness claim is therefore a claim about current standard practice with named falsifiers, not a metaphysical claim about adaptive domains in general.
