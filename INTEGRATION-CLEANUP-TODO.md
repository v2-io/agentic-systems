...
## The wipe question — RESOLVED 2026-07-16: never a real directive; the directories are KEPT

D-2 dissolved on Joseph's clarification (2026-07-16): the "wipe" was a *teaching hypothetical* he posed 2026-05-19 to make an agent feel why bulk-filed spikes had to be truly integrated ("if `.integrated`+`.archived` were completely wiped today — 'no problem' or 'oh no'?") — it was transcribed as a pending decision and carried for two months. There is no wipe. The hypothetical did its work twice over: it produced the spike-routing/integration SOPs then, and the phantom decision motivated the 2026-07-16 verification + 21-item landing cycle + honest `.archived/` re-sort now. Both directories are permanent; do not resurrect the wipe from older documents that mention it as pending.
...
3. **There is no wipe** — resolved 2026-07-16 (teaching hypothetical, mis-transcribed as a directive; see the wipe-question section above). The directories are permanent.
