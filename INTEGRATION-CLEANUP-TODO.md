...
- **D-1. Final wording of the corrected principle.** The writing agent has now mis-encoded this area twice this session (over-enumeration, corrected; `need vs mention`, rejected by Joseph at root). Before a third governing-doc rewrite, Joseph's explicit ratification of the principle as stated above (or with his refinements) is owed.
...
- **D-3. The predecessor abdication and its Joseph-attributed encoding.** The README's `> [!important]` bounded-guarantee callout and the `spike-routing.md` §5 mirror carry the documented abdication framed as Joseph's policy. Retract, revise, or stand? **Do not silently edit a Joseph-attributed decision.**

## The work (the TODO proper, grouped)

### G1. Truthify the governing/onboarding docs (gated on D-1, partly on D-3)

- [ ] **`CLAUDE.md:236`** — delete the `ref/Novelty_defense_and_integration.md` "segments cite as source-of-truth" sanction. (`:165` already states the correct discipline; remove the contradicting `:236` clause; rewrite the `ref/` File-Organization line to: external papers catalogued in `ref/INDEX.md`; internal authoring scaffolds in `ref/` are not source-of-truth for segments — segments cite the *external* sources directly.) **Joseph-confirmed defect, 2026-05-19.**
...
- [ ] **`doc/spike-routing.md` §2-bis(2)** — excise the `need vs mention` test. Replace with the binary (WN is not canon; everything else is canon; canon cites only canon and itself). The "*need*" framing is gone, not deflated.
- [ ] **`doc/spike-routing.md` §5 + Refinement 10** — gated on D-3 (Joseph-attributed bounded-guarantee).
- [ ] **`spikes/README.md`** — finalize the cardinal-rule wording (currently provisional binary form, pending D-1); excise the bounded-guarantee callout per D-3; ensure sim/empirical/experiment are *explicitly* named as spike-class with the same integration duty (currently the on-ramp reads markdown-spike-shaped, under-representing the broader class).
