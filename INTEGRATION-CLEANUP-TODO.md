...
- [ ] **D-citation. Joseph's authoring/voice decision:** prose-embedded scholarly citations (current Vol-1 form), full `\cite{}` discipline, or the hybrid formal-citation skeleton recommended in `BIBLIOGRAPHY-TODO.md`. Per Relata-agent's caution: a mechanical `[Author Year]`→`\cite{}` regex sweep would migrate only ~9 of ~281 Vol-1 references and homogenize monograph voice for the other ~270 (which are embedded in scholarly prose with mid-sentence italicized titles, editions, em-dash author lists). This is voice/style territory and must be Joseph's call.
- [ ] Wire `biblatex` / natbib via the `mono/kaobook/main.tex:31` marker (the existing `% kaobiblio loaded once we wire biblatex (task 7)` seam); same for `scrbook`. ASF-side wiring should call `relata emit` once the citation discipline is decided and the run-from-anywhere caveat is handled.
- [ ] Use relata as the bibliography discipline rather than standing up an ASF-local `.bib` / `entries/` / `bin/refs` clone. The remaining ASF work is to reconcile `ref/` against discovered relata entries, add missing foundational entries, migrate segment citations by author judgment, and wire the build pipeline; details live in `BIBLIOGRAPHY-TODO.md`.
