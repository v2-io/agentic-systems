...
> *Developing / not yet ratified.* This is a **narrowing** of the integration-is-replacement "history → Working Notes" rule, captured here durably (alongside D-1/D-2/G3) before any governing-doc surgery so the principle is not lost between sessions. It is **proposal-grade**, not binding; the governing docs (`FORMAT.md`, `doc/spike-routing.md`, `doc/audit-routing-instructions.md`, `spikes/README.md`, the project memories, `CLAUDE.md`) are Joseph-gated and unedited pending his gate. The full inventory (file:line drift-flags), the single-source-vs-coherence-edit recommendation with sketched wording, the WN-follow-up-routing proposal, and the two reserved gate-decisions (A-vs-B + project-only-vs-global scope) live in the read-and-propose coherence pass [`msc/wn-discipline-coherence-pass-2026-05-31.md`](msc/wn-discipline-coherence-pass-2026-05-31.md).
...
3. **Not legitimate: unneeded spike/artifact references.** A breadcrumb that no future work needs is not just clutter — it **pins the spike in place**, because the spike-archivability test (`doc/spike-routing.md` §2-bis(2): "can the spike move to `.integrated/` without canon breaking") trips on any in-repo pointer a reader is sent to follow. An unneeded WN reference therefore actively blocks the routing cycle. Reduce-or-remove, never repoint (`feedback_spike_references_only_in_working_notes` pt 5).
