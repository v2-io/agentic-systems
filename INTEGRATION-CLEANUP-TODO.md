...
## Developing discipline — what earns a Working-Note (LANDED 2026-06-02, Option B — project-scope)

> **LANDED 2026-06-02 (Option B, project-scope).** The authoritative statement now lives in [`doc/sop/format.sop.md`](doc/sop/format.sop.md) §"What earns a Working Note"; the other repo sites were narrowed to pointers (`doc/sop/agents.sop.md`, `doc/sop/audit.sop/routing.sop.md`, `spikes/README.md`). **Deferred to the global/memory pass** (the cross-project scope half of the original §6.2 decision): the memory-layer carriers — project `feedback_integration_is_replacement` / `feedback_spike_references_only_in_working_notes` and the global `~/.claude/` layer — inherit the same narrowing there, not here. The items below are the decided content (now authoritative in FORMAT, kept here as the cycle record); the reasoning trail is [`msc/wn-discipline-coherence-pass-2026-05-31.md`](msc/wn-discipline-coherence-pass-2026-05-31.md) (bannered landed). **Item 5 remains *provisional*** and coupled to D-2.

1. **A Working Note earns its place only if it assists future work.** Legitimate WN content: **forward pointers** (open follow-on, gating sub-spikes, unresolved questions), **regression-guards** (a disconfirmed prediction recorded so it is not re-attempted; a deliberately-corrected-away form recorded so it is not re-landed), and **dead-end warnings** (an approach tried and found not to work, so the next agent does not re-walk it). The test is prospective: *does a future worker need this to do the work better or avoid a known trap?*

2. **Not legitimate: vanity-changelog.** Pure past-work narration — "landed 2026-05-16," "the X cycle lifted this," "previously carried a false bound" — assists no future work. That is `CHANGELOG.md`'s job (and, for routing/process history, the tracking file / `.integrated/` MANIFEST). It does not belong in a Working Note merely because Working Notes are "not canon and therefore a free dumping ground." Not-canon licenses *forward-work* content, not *backward-narration* content.

3. **Not legitimate: unneeded spike/artifact references.** A breadcrumb that no future work needs is not just clutter — it **pins the spike in place**, because the spike-archivability test (`doc/sop/spikes.sop.md` §2-bis(2): "can the spike move to `.integrated/` without canon breaking") trips on any in-repo pointer a reader is sent to follow. An unneeded WN reference therefore actively blocks the routing cycle. Reduce-or-remove, never repoint (`feedback_spike_references_only_in_working_notes` pt 5).

4. **This narrows integration-is-replacement.** Today the integration-is-replacement rule (CLAUDE.md "Landing a strengthened result" §, `doc/audit-routing-instructions.md` §5/§6, the two memories) sends history to **"CHANGELOG + the cycle tracking file + the segment's own Working Notes."** The sharpening **cuts the Working-Notes half down to only the future-work-assisting slice**: history-proper (the ghost narration, the "not a weakening," the cycle autobiography) goes to CHANGELOG / tracking / `.integrated/` **only**; the Working Note keeps a line **only** if that line is a forward-pointer, regression-guard, or dead-end warning that future work needs. The body-signal is unchanged (the urge to write "this is not a weakening" is the tell the ghost is undeleted) — but now it fires against putting that ghost in *Working Notes* too, not only the body/catalog. Where this rule is currently stated and where it drifts: see the 2026-05-31 coherence inventory.

5. **Provisional further thought (Joseph, hedged — hold as provisional).** *Every segment may assume its thinking-traces live in `{spikes,audits}/.integrated/`, so segments need no per-segment histories at all.* If this holds, WN backward-narration is not merely redundant with CHANGELOG — it is redundant with a richer, addressable trace corpus, and the future-work-assisting slice is the *entire* legitimate WN-history surface. **Why it is "not yet":** it rests on `.integrated/` being trustworthy, which the un-discharged bulk-64 (D-2 / F3 / G2) currently undermines — `.integrated/` membership is an *unverified* record for everything moved before the per-cycle-verification policy. Until the bulk-64 is verified or consciously set down, segments cannot safely assume their traces are recoverable from `.integrated/`. Couple this to D-2.

6. **Gap Joseph named: there is no process for draining WN follow-ups.** A forward-pointer WN item (the *legitimate* kind, per point 1) has no defined route from "noted in a Working Note" to "actually picked up." Today it can sit in a WN indefinitely with the WN as its sole record of obligation — which is exactly the silence that makes work expensive to recover. **Proposed (2026-05-31 coherence pass):** a WN future-work item is *routed to a real tracker* (the standing cycle per `doc/sop/spikes.sop.md` §0c — TODO / PROPOSALS / PRACTICA / the audit cycle / `spikes/PROPOSED.md`) at the time it is written, and the WN then holds a *pointer* to that tracked obligation, never the sole obligation; plus a periodic WN-hygiene pass (folded into existing tracker-touches per `feedback_prune_completed_from_trackers`, not a new ceremony) that prunes satisfied/landed forward-pointers and demotes any backward-narration that crept in. Full proposal in the 2026-05-31 coherence report.
...
`02-tst-core/simulations/` is real, git-tracked, **un-integrated spike work parked in the component tree itself** — 11 `.py` files (`lindy_*`, `regime_transitions`, `three_regimes`, `stochastic_*`), 2026-03-20, README added 2026-05-15 but the integration duty never discharged. Not in `spikes/`, not in `INDEX.md`, not routed, not covered by `doc/sop/spikes.sop.md` or `spikes/README.md`. An entire class of spike-equivalent work is invisible to the discipline because it isn't markdown and doesn't live under `spikes/`.
...
`spikes/.integrated/MANIFEST-2026-05-12.md` records 64 spikes bulk-moved into `.integrated/` on 2026-05-12. The README's `> [!important]` callout (attributed *"Joseph, 2026-05-17"*) admits *"Spikes that predate this policy have not [had content verified] — before `.archived/` existed (notably the 2026-05-12 bulk move of 64 spikes), some incomplete-and-not-needed spikes may have been swept into `.integrated/`. Teasing those back out is not worth the effort and is explicitly not done. Do not re-audit `.integrated/`; verify forward."* This is mirrored in `doc/sop/spikes.sop.md` §5 ("bounded guarantee").
...
- **D-3. The predecessor abdication and its Joseph-attributed encoding. — RESOLVED 2026-05-30 = revise.** Both `spikes/README.md` and `doc/sop/spikes.sop.md` §5 rewritten from the *"not worth the effort / do not re-audit `.integrated/`"* abdication to honest **un-discharged integration debt**: the false 2026-05-17 attribution was dropped, the legitimate forward/per-cycle guarantee retained, and the bulk-64 now reads as debt that **must be verified — or consciously set down — before any wipe**, which pre-frames D-2. *Original question (now answered):* The README's `> [!important]` bounded-guarantee callout and the `doc/sop/spikes.sop.md` §5 mirror carried the documented abdication framed as Joseph's policy; Joseph chose *revise* (not retract — the unverified-status fact is preserved; not stand — the abdication + false attribution are gone).
...
> **Reference-cleanup DONE 2026-05-30 (commit `9099e58`; CHANGELOG 2026-05-30).** The *reference* half of G2 — every canon→internal-artifact reference across the four volumes — is discharged: load-bearing content landed self-contained, the rest deleted/demoted, diff-voice stripped, `Search Log` kept FORMAT-correct, `FINDINGS.md` regenerated. What remains open below is distinct: the un-integrated artifact *corpora* terminal homes and the bulk-64.

- [x] **The ~44 markdown canon refs** (F1) — DONE 2026-05-30. Per-segment triage. Land the content as appendix/discussion segment(s) where it is load-bearing; delete the reference where it is vanity. The ~3 in `## Findings` are the most external-visible (they roll to `FINDINGS.md`) and the highest-priority subset.
- [x] **The two this-arc Discussion refs** (F1 inner) — DONE 2026-05-30 (now at `disc-stability-certificate:86` / `result-sector-persistence-template:112`; the `(independently adjudicated …)` parenthetical dropped). Clean reduce — drop the `(independently adjudicated ...)` parenthetical. The writing agent held this rather than patch in isolation (would have been the "shape of progress" failure); the next agent can do it in the larger pass, not as a standalone "win."
- [x] **The ~10 sim/empirical canon refs** (F2) — DONE 2026-05-30. Per-segment triage. `obs-section-i-validation-simulations` rewrites toward self-contained statement + (at publication) external-citable artifact reference. `result-*` and `der-*` segments depending on `track-b-nonlinear-sims/` variants likewise.
...
- [~] **`spikes/track-b-nonlinear-sims/`** (F2) — *canon dependencies resolved 2026-05-30; the corpus's own terminal home remains open.* Same treatment; this corpus is *live* in `spikes/` and would not be touched by a `.integrated/` wipe, but the canon dependencies on it must still be resolved.
- [x] **`empirical-discontinuity/` toolkit dependencies** in TST segments (F2) — DONE 2026-05-30 (canon refs resolved; the measured $\alpha \approx 0.118$ landed self-contained in `#hyp-exponential-cognitive-load`).
...
This is the structural reason `ref/Novelty…` was sanctioned. Until ASF has formal-citation infrastructure, prior-art assertions in segments will continue to find ad-hoc internal homes. **Current operating tracker:** [`BIBLIOGRAPHY-TODO.md`](BIBLIOGRAPHY-TODO.md) now carries the ASF-side citation workstream and supersedes the older local-`.bib` / `bin/refs` framing below where they conflict.

- [x] **D-citation. Joseph's authoring/voice decision — DONE 2026-06-05.** Adopted the strengthened hybrid discipline: rich scholarly prose remains allowed, but bibliography-worthy scholarly sources get formal natbib-compatible cite commands and load-bearing external dependencies get locator-backed, verification-ready formal cites. Mechanical regex migration remains rejected; segment migration is an author-judgment pass.
- [x] **Wire `biblatex` / natbib via the monograph build — DONE 2026-06-05.** `bin/build-monograph` now emits `references.bib` from the assembled current volume markdown via `relata emit`, loads kaobiblio / biblatex with `natbib=true` for both targets, fails on missing citation keys, runs LuaLaTeX → biber → LuaLaTeX → LuaLaTeX, and commits the generated `mono/<slug>-v<sem>.references.bib` snapshot when builds are refreshed.
- [x] **Use relata as the bibliography discipline rather than standing up an ASF-local `.bib` / `entries/` / `bin/refs` clone — DONE 2026-06-05 as architecture.** The remaining ASF work is to reconcile `ref/` against discovered relata entries, add missing foundational entries, and migrate segment citations by author judgment; details live in `BIBLIOGRAPHY-TODO.md`.
