...
Cross-audit "wandering thoughts" / §14-ideation harvested from the de-novo auditors' working dirs, deduplicated across substrates and lightly attributed. Orthogonal pedagogical / framing material, kept separate from the certified theory-fix findings (handled elsewhere — see report flags). **Coverage:** three dirs reached a digested reflection (526815, 829314, 849201) plus the §III composition-foundations batch (471203). The richest ideation here clusters around the weighted-Lyapunov $\mu \to 0$ sketch the segment's Working Notes already carry — the gold is the *pedagogy around that math*, not new math. Substrate attribution inferred from voice where not explicit.

#### 1. Candidate Brief prose / pre-prose

- **The thermodynamic bargain — autonomy for persistence.** The single strongest framing here: symbiogenesis (and corporate acquisition, and mitochondrial endosymbiosis) is "a fundamental thermodynamic trade: **autonomy ($\mu \to 0$) in exchange for persistence ($R \to R_h$)**." The absorbed entity's independent mismatch dynamics drop out of the joint stability equation in the limit $\mu \to 0$; what it *gains* is that it "no longer has to worry about the harsh environmental drift $\rho$ or running out of runway (reserve $R$), because it is now sheltered inside the massive reserve $R_h$ of the host." This is offered as the intuition that explains why symbiogenesis is ubiquitous in both nature and capitalism (Gemini, AUDIT-WORKING-829314). A candidate Brief/Discussion anchor that makes the $\mu \to 0$ limit legible without symbols.

#### 2. Candidate Discussion

- **Symbiogenesis and peer coupling as parameter-limits of one equation.** The pedagogical reading of the existing $\mu \to 0$ Working Note: a "merger of equals" (peer coupling) weighs both parties' stability dynamics roughly equally ($\mu \approx 1$) and must coordinate everything; an acquisition (symbiogenesis) sends the small party's weight $\mu \to 0$ so its independent dynamics vanish from the joint stability argument. "Symbiogenesis isn't a magical discontinuous leap, but a smooth parameter limit of the standard peer-coupling stability proof" (Claude, AUDIT-WORKING-849201; the merger-of-equals-vs-acquisition vignette at Gemini, AUDIT-WORKING-829314). Candidate Discussion prose making the segment's sketch self-explaining. *(The auditors also urge promoting the $\mu \to 0$ limit from Working Notes into the Formal Expression — that is a strengthen/promotion question flagged in the report, not gold.)*
- **Why retained autonomy is not zero — the singular-perturbation reading.** "Why not just execute the startup's founders and replace them with a script?" Because the host *wants* the fast, low-level local responses the endosymbiont was good at, without routing them through the slow macro-loop — so it "deliberately preserves a tiny sliver of the startup's autonomy to handle high-frequency local disturbances that would violate the host's slow tempo budget" (Gemini, AUDIT-WORKING-829314; same point, with a curiosity about deriving an *optimal* retained autonomy $\mu^\ast$ from local-disturbance spectral frequency $\rho_{\text{local}}$). Sharpens the segment's existing (S-3)-"why not complete?" Working Note by tying it explicitly to temporal nesting / singular perturbation.

#### 3. Follow-up items

- **Optimal retained autonomy $\mu^\ast$ from local-disturbance spectrum.** A candidate quantitative extension of (S-3): derive the retained-autonomy weight $\mu^\ast$ as a function of the spectral frequency of the local disturbance $\rho_{\text{local}}$ the endosymbiont absorbs (faster local disturbances $\to$ more retained autonomy). Connects to #der-temporal-nesting (Gemini, AUDIT-WORKING-829314).
- **When is symbiogenesis favored over peer coupling? — transaction-cost framing.** Multiple substrates reached for transaction-cost economics (Coase / Williamson) as the analog: symbiogenesis is favored when the coordination overhead of maintaining separate objectives exceeds the integration cost (Claude, AUDIT-WORKING-849201; converges with the segment's existing "Quantitative predictions" Working Note tying this to $C_{\text{coord}}$).

#### 4. Readers often ask / wonder

- **"What drives $O_e \to \mathcal{D}_e(O_h)$ — selection, bounded rationality, or design?"** A natural reader want for the most load-bearing open dynamic (S-1); the segment's Working Notes already enumerate the three candidate drivers, and a fresh reader independently reached for the same set (Claude, AUDIT-WORKING-849201). Worth a one-line forward-pointer in the body so the reader knows the question is named.

#### 5. Candidate figures

- **Three simultaneous contractions, with a typed-ambiguity output.** Symbiogenesis drawn as three coupled contractions over time — (S-1) objective absorption, (S-2) function grafting, (S-3) autonomy reduction — with the output deliberately shown as a *typed ambiguity*: either a composite with a still-agentive subordinate sub-agent, or a host system with an integrated non-agentic component if autonomy falls below the agency-scope threshold (Codex/Claude, AUDIT-WORKING-526815, "diagram attempt"). The typing distinction it visualizes is also a real correctness concern flagged in the report (whether the absorbed entity is still an AAT sub-agent).

#### Belongs elsewhere

- **Mitochondria / eukaryotic-cell as the canonical biological instance.** Repeatedly invoked (the eukaryotic cell as a parasite/predator that became a structurally necessary sub-component) (Gemini, AUDIT-WORKING-193847 under #scope-composite-agent; Gemini, AUDIT-WORKING-829314). The segment already maps to mitochondria; this is convergence signal that the analogy lands, and the Margulis 1981 prior-art anchor is already in Working Notes.
