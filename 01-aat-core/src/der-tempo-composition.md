...
Let $\mathcal{T}_i$ be the adaptive tempo of sub-agent $i$ within a composite group of $N$ agents. Let $\mathcal{T}_c$ be the adaptive tempo of the composite macro-agent $A_c$ defined by an admissible coarse-graining $\Lambda$.
...
- **Brooks's Law.** Adding an agent increases $\sum \mathcal{T}_i$ but may also increase $\varepsilon^\ast$. The turning point — where adding a member lowers realized external tempo — occurs when:
...
- **Composite persistence condition.** The composite persists iff $\mathcal{T}_c \gt \rho_{\text{eff}} / \lVert\delta_{\text{critical}}\rVert$, equivalently:
...
**Equality conditions.** Strict equality ($\mathcal{T}_c = \sum \mathcal{T}_i$, i.e. $C_{\text{coord}} = 0$) requires:
...
**Brooks's Law.** Adding more agents (increasing $\sum \mathcal{T}_i$) only increases the composite tempo $\mathcal{T}_c$ if the corresponding increase in closure defect $\varepsilon^\ast$ doesn't let $C_{\text{coord}}$ dominate. The model provides a formal analog of Brooks's Law: if communication overhead ($C_{\text{coord}}$) grows faster than aggregate capability ($\sum \mathcal{T}_i$) when adding agents, then adding people to a late project makes it later. Whether this specific mechanism (coordination overhead consuming tempo) is the dominant cause in practice is an empirical question.

**Connection to #der-team-persistence.** The persistence condition for the composite agent requires $\mathcal{T}_c \gt \rho_{\text{ext}} / \Vert\delta_{\text{critical}}\Vert$. Since $\mathcal{T}_c = \sum \mathcal{T}_i - C_{\text{coord}}$, high coordination overhead can push the composite below the persistence threshold, causing it to disintegrate as a coherent entity — even though each sub-agent individually persists.
...
- **Track E surface-back of catalog citations (2026-05-22).** Three prior-art neighbors surfaced 2026-05-22 from Track E catalog at `ref/prior-art-analysis/08-composite-agency.md` (Pillar 4 + key anchor papers), each marked `[^cat-2026-05-22]` for verification-deferred attribution:
   - **Janow 2009**, *A Fundamental Limit on Productivity in Organizations: Collaborative Entropy Costs*[^cat-2026-05-22] — closest single-paper precedent for the tempo-tax claim. First-principles Shannon-entropy model of coordination cost as productivity cap. Uses Shannon entropy rather than persistence-bound cybernetic accounting; AAT's contribution is the integration with `#form-composition-closure`'s $\varepsilon^\ast$ machinery and the persistence-framework dimensional accounting (Composite Tempo Inequality $\mathcal{T}_c \leq \sum_i \mathcal{T}_i - C_{\text{coord}}$).
...
- **Brooks's Law as a strict thermodynamic inequality.** The converged star of this segment: Fred Brooks observed "adding manpower to a late software project makes it later" and attributed it to $O(N^2)$ communication overhead; AAT gives the *exact* inequality for when the catastrophic slowdown actually happens — $\Delta\varepsilon^\ast \nu_c / \lVert\delta_{\text{critical}}\rVert \gt \Delta\mathcal{T}_i$ — translating "a famous software engineering aphorism into a strict thermodynamic inequality" (Gemini, AUDIT-WORKING-829314 — "an absolute triumph"; Claude, AUDIT-WORKING-849201; Claude, AUDIT-WORKING-773921 — "a formal, computable physics equation for team size limits"). The worked mechanism: a junior engineer's small $\Delta\mathcal{T}_i$ is exceeded by the $C_{\text{coord}}$ the seniors spend correcting the internal mismatch the addition created (explaining the codebase, fixing bad PRs, unblocking dependencies). A strong Brief/Discussion anchor.
- **The thermodynamic equation of teamwork.** $C_{\text{coord}} = \sum_i \mathcal{T}_i - \mathcal{T}_c$ glossed as a conservation law: "energy in (sum of tempos) equals useful work ($\mathcal{T}_c^{\text{ext}}$) plus heat ($C_{\text{coord}}$)" (Claude, AUDIT-WORKING-849201). Makes the sub-additive bound legible as a first-law accounting.
