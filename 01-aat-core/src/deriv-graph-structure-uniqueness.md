...
**(e) Connection to edge independence.** The CMC's exogenous independence condition ($\varepsilon_i$ mutually independent) is precisely the **edge-independence assumption** in the AND/OR status propagation ( #def-strategy-dag). When exogenous noise terms are independent, edge outcomes are conditionally independent given parents, and the AND/OR formulas compute correct probabilities. When they are correlated (causal insufficiency — latent common causes), the AND/OR propagation is biased by exactly the sibling covariance, with the sign set by node type — optimistic at OR-nodes, conservative at AND-nodes — so OR-dominated roots overestimate success and AND-dominated roots underestimate it ( #def-strategy-dag, Correlation Hierarchy). The validity of the Markov factorization and the validity of the independence model are the *same condition*: causal sufficiency of $\Sigma_t$. See #def-strategy-dag for the full treatment of correlated failure as the primary case.
...
The conditioning on causal sufficiency is the right level of conditionality: it is a property of *strategy quality* (did the agent include all relevant common causes as nodes?), not of agent architecture. It is testable — under exploration: on-policy residuals cannot reveal a missing common cause ( #der-causal-insufficiency-detection, the no-go), but pairwise sibling covariance under the agent's own interventions can — and repairable in practice (add the common-cause node). When causal sufficiency fails, the Markov factorization fails, and this manifests as correlated failure and sign-determined $\hat P_\Sigma$ bias — overestimation for the common OR-dominated-root case ( #def-strategy-dag). The edge-independence assumption in AND/OR propagation and the causal sufficiency condition for the Markov property are the same condition viewed from different angles.
