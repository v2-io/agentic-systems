...
- Open: whether to add light back-references ("this is the anti-collapse move") at the remaining strong instances (I1 #der-interaction-channel-classification, U1 #def-unity-dimensions) to reinforce recognition for a linear reader — held pending a view on whether that over-instruments the segments. Tier-3 partials (P1 $\kappa$-as-scalar category error; P2 regime-vs-class; P3 codebase-$\rho$; P4 plan-vs-execution) are adjacent and flagged in the sweep with their sibling-pattern overlaps, deliberately not folded in to keep the boundary against separability / coordinate-forcing / constructive-impossibility clean.
