...
**Trajectory-relativity.** $S(M_t)$ is measured against *this agent's* interaction history $\mathcal{C}_t$ ( #def-chronica), not against a model-state equivalence class. Two copies of the same $M_t$ exposed to different event streams will each have their own $S$ measured against their own divergent $\mathcal{C}_t$; neither sufficiency value is the other's. This is the scope commitment in #scope-agent-identity: AAT applies to agents instantiated on singular causal trajectories, and sufficiency is trajectory-indexed accordingly. Claims of the form "the model has sufficiency $S$" make sense only relative to a specific trajectory; aggregated claims across copies of a given $M_t$ require additional machinery.
