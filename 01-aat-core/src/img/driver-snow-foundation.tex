...
\def\armAt{45}      % current arm angle (just shy of far right)

% Windshield path (for re-use)
\def\windshield{[rounded corners=10pt] (\wxBL,\wyBL) 
    .. controls (-3.3, 1.3) .. (\wxTL,\wyTL) 
    .. controls (0, 2.75) .. (\wxTR,\wyTR) 
    .. controls (3.3, 1.3) .. (\wxBR,\wyBR) 
    -- cycle}
...
\begin{scope}
  ...
  \begin{scope}
    ...
    % Lane edges (perspective lines curving right toward horizon)
    \draw[road, line width=1.2pt] (-1.8, 0.05) .. controls (-0.8, 1.3) and (-0.2, 2.0) .. (0.2, 2.32);
    \draw[road, line width=1.2pt] ( 1.8, 0.05) .. controls ( 1.0, 1.3) and ( 0.5, 2.0) .. (0.6, 2.32);
    % Dashed center lane
    \draw[lane, line width=1.0pt, dash pattern=on 8pt off 6pt] 
        ( 0.0, 0.05) .. controls ( 0.1, 1.3) and ( 0.15, 2.0) .. (0.4, 2.32);
    % Horizon (curved hill to avoid flat horizontal line competing with affordances)
    \draw[black!55, thick] (-4, 1.9) .. controls (-2, 2.4) and (2, 2.5) .. (4, 1.8);
    % A faint distant vehicle suggestion (shifted right for curve, up for hill)
    \draw[thick, black!55] (0.3, 2.22) -- (0.45, 2.22) -- (0.43, 2.29) -- (0.32, 2.29) -- cycle;
  \end{scope}
...
% Cabin context: removed roof line and lower dashboard cowl frame per request
...
\begin{scope}
  ...
  % Arm base emphasis (thickening at the bottom)
  \draw[wiper, line width=4.5pt, line cap=round]
     (\px,\py) -- ($(\px,\py) + ({\armAt}:{1.4})$);
\end{scope}
...
% ====================================================
\begin{scope}[shift={(2.8, -0.32)}, scale=0.30]
...
% wiper dial
\draw[callout] (1.3, -0.32) -- (2.4, -0.32);
\node[callout-lbl, anchor=east] at (1.25, -0.32)
...
\begin{scope}[yshift=-3.0cm]
              ...
              ]
  {
    ...
    blade in visual field & cost(wiper\_rate)                   & deliberation cost & \#der-deliberation-cost \\
