...
A composite-level extension follows from #deriv-strategic-composition. The class partition applies to individual agents based on *within-agent* coupling. Composition introduces a second form of coupling — *across-agent* coupling through the shared environment. A composite of Class 1 (Separated) sub-agents with *aligned* objectives remains Class 1; with *partially-opposing* objectives becomes a Class 2 (Partial) composite from Class 1 (Separated) sub-agents — each sub-agent individually remains Separated, but each sub-agent's model includes a model of other sub-agents' goal-dependent policies, so the composite's coupled state acquires intrinsic coupling. Composite-level directed separation fails through across-agent coupling, not within-agent coupling.
