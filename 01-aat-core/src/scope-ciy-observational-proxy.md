...
**Regime A — Randomized interventions.** The agent's action assignment is randomized, or known-and-adjusted so that sequential ignorability and positivity hold (RL agents exploring with an exploration policy uncorrelated with the latent outcome causes, scientists randomizing treatment, organisms probing). CIY is directly estimable from the agent's execution data and non-negative by construction. This is the standard case for active agents within the adaptive loop ( #der-loop-interventional-access). What provides the identification needed for clean interventional estimates is the randomized / ignorable assignment together with positivity — not mere action variation, which is insufficient when actions are policy-driven by state or history that also affects the outcome.
...
- ~~**Regime A wording: tighten "action variation" to "identifiable intervention."**~~ **Resolved 2026-05-30** (transportability spike, §3.5 do-vs-conditioning reconciliation). Regime A now reads "action assignment is randomized / known-and-adjusted (sequential ignorability + positivity)" in both the opening prose and the formal Regime A statement, with an explicit note that mere policy-driven action variation is insufficient. The original note (Codex/Claude, AUDIT-WORKING-526815): "the agent varies its actions" is not sufficient for clean estimates under policy-driven action choice — same do-vs-conditioning concern as in `#der-loop-interventional-access`.
- **Conflict-as-signal — Regime B for LLM software agents (AUDIT-WORKING-963715 `34-38`, Claude; lifted 2026-08-22 via #impl-causal-access).** Within a session an LLM coding agent can vary actions and observe results (Regime A-like); across sessions, attribution of codebase changes to downstream effects needs assumptions — so the typical deployment regime for LLM software agents is B, against the body's "software development is typically Regime A." Worth resolving in the regime table (software-as-domain vs LLM-agent-in-software as the unit).
