...
For an agent with error dynamics $\dot e=-F(e)$ about an equilibrium $e^\ast$ ($F(e^\ast)=0$, $F\in C^1$ near $e^\ast$, Jacobian $J:=DF(e^\ast)$), a **stability certificate** is a symmetric positive-definite $\mathcal{M}$ for which the one-point sector condition holds in the $\mathcal{M}$-inner-product on a ball $\mathcal{B}_R(e^\ast)$:
...
Swept in the gold-lift pass (per `de-novo-audit-instructions.md` §7.15): **no incidental pedagogical/framing gold found.** This segment was split out 2026-05-14, so it is newer than most de-novo audit cycles and has no dedicated per-segment reflection. The auditor mentions that touched it are orientation-level only — AUDIT-WORKING-526815's predictions named the stability-certificate spine as the framework's top-level organizing picture ("when does a metric/certificate exist, when is it forced, where does it go blind, what does projection/composition do to its guarantee") and flagged this segment for "watch local-to-global" caution; that framing is already carried by the certificate spine `#disc-stability-certificate`, not new content here. Marker left so future sweeps know this segment was checked.
