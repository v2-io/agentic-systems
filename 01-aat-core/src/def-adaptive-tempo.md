...
- **Open — the "effective rate of useful information acquisition" gloss is loose wording** (Codex/Claude, AUDIT-WORKING-526815): $\eta^\ast$ is a correction *fraction*, not an information payload; "quality-adjusted correction event rate" was proposed as the more accurate reading. Not adjudicated 2026-07-14 (that adjudication settled the tier, not the gloss); the Fisher-inflow identification in `#deriv-tempo-additivity` §1 is the candidate precise referent.
- **Regression guard — do not restore a general additive upper bound or $\mathcal T_{\text{add}}$.** The signed deviation $\Delta$ takes both signs (`#deriv-tempo-additivity`); a CMI-penalty form is impossible (no-go N1); no convention-free per-channel attribution exists for $n \geq 3$ (N2). This definition's honest tier remains `conditional` (channel independence; shared-eigenbasis/isotropy). The $\mathcal T_{\text{add}}$-as-upper-bound re-symboling is a closed non-move. Trail: `spikes/.integrated/spike-adaptive-tempo-redundancy-penalty-2026-07-15.md`.
