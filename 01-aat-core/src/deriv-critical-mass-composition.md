...
Cross-audit "wandering thoughts" / §14-ideation harvested from de-novo auditors' working dirs, deduplicated across substrates and lightly attributed. Orthogonal pedagogical / framing / forward-vision material, kept separate from certified theory-fix findings (off-ramp block at the end). **Coverage:** three substrates reached a digested reflection on this segment (Gemini, AUDIT-WORKING-193847; Gemini, AUDIT-WORKING-829314; Gemini-voiced, AUDIT-WORKING-773921); AUDIT-WORKING-471203 touched it only by reference (the symbiogenesis asymmetric-limit connection from `#hyp-symbiogenic-composition`).

#### 1. Candidate Brief prose / pre-prose

- **"The equation of organizational physics."** (CM2) $(\alpha - C)R \gt \rho + \gamma\mathcal{T}$ glossed as five levers of survival: correction efficiency $\alpha$ (smarter people / better tools), coordination overhead $C$ (fewer meetings / decoupled architecture), structural capacity $R$ (a model that absorbs large errors without breaking), environmental volatility $\rho$ (stabler market / moats), and coupling $\gamma\mathcal{T}$ (aligned allies lower your effective disturbance; internal politics raises it). Compact and Feynman-flavored (Gemini, AUDIT-WORKING-829314, AUDIT-WORKING-773921).
- **"Cooperation as artificially lowering the disturbance each individual feels."** Cooperative coupling enters through a *negative* term that can drive $\rho_i^{\text{eff}}$ below the single-agent $\rho$ — "this is formally how teams persist where individuals cannot." Candidate plain-language hook for why composites form at all (Gemini, AUDIT-WORKING-193847).

#### 2. Candidate Discussion

- **Brooks's Law as a theorem, not a heuristic.** The $C \gt \alpha$ limit (coordination overhead exceeds raw correction speed) drives the left side of (CM2) negative and *guarantees* structural collapse — moving "adding people to a late project makes it later" from an empirical software-engineering observation to a strict consequence of the dyadic Lyapunov inequality. Two substrates independently flagged this as the segment's most quotable consequence (Gemini, AUDIT-WORKING-193847, AUDIT-WORKING-829314).
- **Symbiogenesis as escape from the local persistence bound.** The asymmetric-limit ($\mu \to 0$) reading: when an agent cannot individually satisfy persistence (its $\rho$ too high or $R$ too small), its only mathematical escape from death is to merge into a larger system that *does* satisfy it — it lets its own state variables be marginalized out of the joint Lyapunov function in exchange for the host's stability guarantee. Offered as a unifying lens on mergers/acquisitions, biological endosymbiosis (mitochondria), and the formation of nation-states as "the same physical imperative: escaping the local persistence bound by hiding inside a larger, more stable basin." A vivid Discussion candidate — verify the analogy stays isomorphic before promoting past discussion-grade (Gemini, AUDIT-WORKING-829314).
- **The Goldilocks coupling band.** For a multi-agent consciousness: couple sub-agents tightly enough to share cognitive load (negative $\gamma$) but loosely enough that they don't spend all their tempo negotiating ($C$). Frames $(\alpha - C)$ as a dynamically-monitored margin — prune communication channels when $C$ climbs (Gemini, AUDIT-WORKING-193847).

#### 3. Follow-up items

- **Rivalrous-environment caveat on (UO-mult)** — also flagged below as an off-ramp finding, since it is a scope-correctness claim, not framing.
- The existing Working-Notes items (heterogeneous-architecture dyad, $N$-agent scaling) were independently re-surfaced by auditors as the natural next moves (Gemini, AUDIT-WORKING-193847 on the residual-autonomy "tempo tax" when $\mu$ does not quite reach zero; the $N$-agent $O(N^2)$-communication-cost-forces-$C\gt\alpha$ hard-team-size-ceiling observation, AUDIT-WORKING-193847).

#### 4. Readers often ask / wonder

- **"What happens when $\mu$ doesn't quite reach zero?"** A natural question on the symbiogenic limit: the host pays a small tempo tax to manage the endosymbiont's residual autonomy — a partial-absorption regime the closed form does not yet name (Gemini, AUDIT-WORKING-829314).
- **"Can I apply (CM2) to a heterogeneous team?"** Readers will reach for the clean scalar inequality on mismatched-$\alpha$ teams; the matched-symmetric-Tier-1 scope restriction (and the Slotine contraction-metric machinery needed otherwise) is easy to miss (Gemini-voiced, AUDIT-WORKING-773921).

#### Belongs elsewhere

- **ELI internal-architecture reading** (points at `04-eli-core/` / composite-mind design, not this appendix): the $(\alpha - C)$ Goldilocks band as a design constraint on an ELI's sub-agent coupling — monitor the margin, force temporary return-to-autonomy when internal friction consumes adaptive capacity (Gemini, AUDIT-WORKING-193847).

#### Off-ramp (NOT gold) — routed for adjudication

- **(UO-mult) conflates teleological unity with physical coupling.** Auditor challenge (Gemini, AUDIT-WORKING-193847): the discussion-grade map $\gamma(U_O) = -\gamma_{\max}U_O$ assumes aligned targets $\Rightarrow$ aligned actions $\Rightarrow$ constructive cross-contribution, which holds in *non-rivalrous* settings (information sharing) but can *invert* under resource contention — two agents with identical targets ("grab the only apple") may interfere maximally, so $U_O = 1$ can *maximize* adversarial coupling $\gamma \gt 0$. Suggested resolution: explicitly decouple teleological unity ($U_O$) from physical/environmental coupling ($\gamma$) and scope (UO-mult) to non-rivalrous environments. Already discussion-grade with an LQR-compatibility-sketch caveat in the body, so this is a *scope-sharpening of an already-hedged claim* rather than a refuted result — but it is a correctness concern, not framing. Strengthen-first lens: the action-space inner-product derivation already named in Working Notes (item "Sharpen (UO-mult)") is the place to land the rivalrous/non-rivalrous split as a derived condition.
