...
- Incidental audit gold from the 2026-05-31 lift is integrated: the trust/manager-worker framing, the LLM-stack mapping, the micromanagement/forgetting unification (sharpened here into the dual (C1)/(C2) reading), and the "does slowing the outer loop guarantee convergence?" reader question (answered in the Tikhonov remark: no — a multi-equilibrium fast loop is outside (S2-fast) regardless of $\epsilon$) all landed in the body above. Provenance: the pre-promotion segment history and the spike trail.
