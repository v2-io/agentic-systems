...
### Candidate scope question — boundary integrity as a second constitutive commitment (2026-08-12, agent-proposed during an AISI-facing applications read)

The gold above (§3, Gemini AUDIT-WORKING-193847) records that *"$\Omega$ is opaque, but the boundary itself must be impermeable to direct tampering from the outside, or the definition collapses"* — if the environment can directly edit the agent's internal state, the agent ceases to be an agent — and marks it "worth an explicit scope sentence or a Working-Notes flag." This note is that flag, plus the reason the question may have gained urgency since the harvest.

**The observation.** The segment names *one* constitutive commitment: the information-loss boundary (the agent cannot read $\Omega$ directly). There appears to be a second, relied on everywhere and stated nowhere: the boundary is one-way-lossy but not *permeable* — $\Omega$ cannot write to internal state. Every downstream object assumes it. $M_t = \phi(\mathcal C_t)$ presumes $M_t$'s only inputs are the chronica's; the mismatch signal presumes $\hat o_t$ was generated by the agent's own model rather than installed; #der-directed-separation's $f_M$ presumes its arguments are the ones the type signature names. Under a permeable boundary these are not merely harder to establish — they are not well-typed, because there is no longer a determinate fact about which state is the agent's.

**Why it may be worth naming now.** Two cases where permeability is not hypothetical:

1. *Adversarial.* Prompt injection, context poisoning, and weight tampering are, on this reading, not attacks *on* an agent but violations of the precondition that makes "agent" the right analytical object for the system under attack. That is a stronger and more useful statement than the security framing, and it is the sort of thing the framework's constructive-impossibility posture is good at: name the boundary, name what is lost when it fails, and the naming does work.
2. *Native, no adversary.* The most-converged reader question in this segment's own gold (§4, four substrates independently) is that for a context-window agent the agent's outputs become part of the $\Omega$ it perceives next turn — so internal state and observation channel are the same object. If that is right, logogenic agents do not merely *risk* boundary permeability; they are constituted with it.

**A conjecture attached, marked as conjecture.** Boundary permeability and GUC Class 3 coupling ( #der-directed-separation) may be one fact seen from two angles rather than two facts: directed separation fails because $f_M$ and $G_t$ occupy one forward pass; boundary integrity fails because internal state and $\Omega$ occupy one token stream. Both would then be consequences of "everything is one undifferentiated stream." If they *are* one fact, Volume 3's story simplifies considerably; if they are two, the difference is worth stating precisely, since the wrapping construction ( #der-class-coercion-via-wrapping) repairs the first and it is not obvious it repairs the second.

**What this is not.** Not a finding — nothing here says the theory is wrong. It is a candidate scope sentence (or a second named constitutive commitment alongside the information-loss boundary), and a question about whether the repair machinery covers both failures or one. Agent-proposed forward pointer, not a steward decision.
