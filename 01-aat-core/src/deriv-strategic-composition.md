...
| Mean-field-game limit ($N \to \infty$) | Lasry-Lions 2007; Huang-Malhamé-Caines 2006; requires population scope condition | Open (pending Part III population gaps) |
...
Cross-audit "wandering thoughts" / §14 ideation harvested from the de-novo auditors' working dirs, deduplicated across substrates and lightly attributed by substrate + audit. *Orthogonal* pedagogical / framing / figure / forward-vision material, staged here for an eventual separate Brief/Discussion promotion pass — kept apart from certified theory-fix findings. **Coverage:** five dirs reached a digested reflection on this segment (193847 Gemini, 526815 deeply-mathematical, 471203 Claude batched §III-composition, 773921 Claude, 849201 Gemini). The finding-vs-framing conflation is preserved as signal, not sanitized.

#### 1. Candidate Brief prose / pre-prose

- The "framing shift" one-liner several auditors converged on as the segment's pedagogical core: when objectives are partially-opposing ($U_O \lt 1$), the question stops being *"does the trajectory contract to a shared state?"* and becomes *"do the coupled best-response dynamics converge to an equilibrium?"* — and contraction-to-shared-truth is recovered as the $U_O = 1$ special case (Claude, 773921; Gemini, 849201). Tight enough to seed a Brief opener.
- The Section-I parallel as a memory hook: "just as Section I split into sub-scope $\alpha$ (Kalman/LQR, exact bounds hold) and sub-scope $\beta$ (PID, they don't), this segment splits into sub-scope $\alpha'$ (potential/monotone games) and sub-scope $\beta'$ (VI / regret-minimization)" — the $\alpha/\beta \leftrightarrow \alpha'/\beta'$ rhyme makes the new partition feel inevitable to a reader who has walked Section I (Gemini, 849201).
- "The gradient of a market potential function $\Phi$ is literally the same mathematical object as an agent's mismatch signal $\delta$" — a candidate plain-language statement of the template transfer's payload (Gemini, 193847).

#### 2. Candidate Discussion

- **The "active-inference sharpening" is repeatedly flagged as a high-water-mark Discussion paragraph** worth foregrounding, not burying: two agents mutually predicting each other become a *fixed-point* problem, so the active-inference attempt to unify goal-seeking and prediction via free energy fails *in a derivable way* — the fixed point sits at the strategic equilibrium, not the prediction-optimum (Gemini, 193847; Gemini, 849201). *(Conflation texture: 526815's certified track separately flags "fails here in a derivable way" as outrunning what the segment locally derives — see off-ramp. Preserve both readings.)*

#### 3. Follow-up items

- **Replicator / evolutionary-dynamics correspondence** (already in WN) was independently seconded — softmax bandits approximate replicator, gradient-on-log-likelihood is replicator exactly; an AAT-native-update-rule ↔ ESS map is a natural follow-on (implicit across the §III batch, Claude 471203).

#### 4. Readers often ask / wonder

- **"Does a little noise act as a force multiplier for drift?"** — readers reaching the $\beta'$ regime want to know whether there is an optimal drift-fraction $f$ for a budget-constrained attacker (Gemini, 193847; this is a `#result-adversarial-exponent-regimes` want surfacing here through the adversarial-sibling link).
- **Mutual-modeling regress.** "If my model contains a model of your model... we enter infinite regress. How does AAT break it — bounded depth?" — a natural reader question at the symmetric-coupling framing; the active-deceive depth-bound discussion lives in `#der-agent-opacity` and could be pointed at from here (Gemini, 193847).
- **"Does multi-agent alignment become mathematically forbidden?"** — the mechanism-design-impossibility cluster (Gibbard-Satterthwaite / Arrow) prompts readers to ask whether a society of AIs can ever align goals without a dictator (Gemini, 193847; curiosity echoed by Gemini 849201, Claude 773921).

#### 5. Candidate figures

- **The $\alpha'/\beta'$ scope-fork diagram** (526815 drafted a TikZ standalone, `93-deriv-strategic-composition.tex`): strategic-composition root → $\alpha'$ branch (potential/monotone → sector transfer *only with curvature/monotonicity* → point equilibrium / basin) vs $\beta'$ branch (no-regret layer → CCE distribution → macro-state is joint-play distribution), with three dashed audit-warning callouts ("potential function alone is not a sector constant"; "VI/CCE need standard convexity/boundedness/protocol assumptions"; "open: closure defect for equilibrium statistics"). The warnings are auditor-finding annotations; a clean promotion figure would drop them, but the fork structure itself is a strong candidate for the segment (Claude/Codex, 526815).

#### Belongs elsewhere

- **Forward-vision (ELI / consciousness-infrastructure, `04-eli-core/`).** The strongest reach: "you cannot build a sane future AI just by networking together sane sub-agents — the act of networking them in a strategic environment *generates* epistemic corruption at the macro level. The infrastructure must actively enforce (C-i) shared objectives or (C-ii) hierarchical derivation, because relying on (C-iv) strategic equilibrium guarantees a goal-contaminated macro-epistemology" (Gemini, 193847). This is aspirational application pointing at `04-eli-core/` composite-architecture design, not at this segment. *(Heavy conflation texture: 193847 states this through the now-withdrawn "Class-1 sub-agents → Class-3 composite" architectural-class framing — current canon carries dynamic-regime-change, architectural-class-preserved instead. Preserve the forward-vision payload; the Class-3 mechanism is the superseded reading, do not promote it. See off-ramp.)*

#### Off-ramp (NOT gold) — routed for adjudication, not promotion

These are certified-track (strengthen-first / overclaim / scope-narrowing) findings, not pedagogical gold — recorded here so they are durable and routable, but they belong to the findings adjudication stream, not the Brief/Discussion promotion pass. The bulk are from the deeply-mathematical auditor (526815), with two adversarial-audit pokes from 193847.

- **(526815, F234/F235/F236) — the sector-template-transfer overclaim.** The displayed inequality $d\Phi/dt \geq \alpha_{\text{joint}}\lVert\nabla\Phi\rVert^2$ and "equilibrium stability follows from $\Phi$ as a joint Lyapunov function" are too broad: a potential-game structure *alone* does not deliver a sector inequality, unique convergence, or a basin around a selected equilibrium — increasing a potential can land on local maxima, saddles, boundary equilibria, or multiple equilibria. The template transfer needs additional curvature / PL / strong-monotonicity, and the state variable silently switches from $\xi = \nabla\Phi$ to $\xi = \pi - \pi^\ast$ (different unless a local Hessian / strong-convexity relation maps them). Recommended discharge direction: *narrow the exact-transfer claim to games with enough curvature/monotonicity for a sector inequality* (strengthen the hypotheses, don't soften the conclusion). The Cournot interior-NE instantiation already supplies that curvature and is locally coherent (526815 F244) — so the segment may already carry the witness for the strengthened scope; verify against current canon before acting.
- **(526815, F237) — VI existence overstated.** "Pure-strategy Nash existence is guaranteed for continuous compact-convex games with continuous payoffs" needs concavity-in-own-strategy / differentiability beyond continuity; a VI solution maps to Nash only under those.
- **(526815, F238/F240) — unstated assumptions before use.** No-regret → CCE at $O(1/\sqrt T)$ needs compact action sets, bounded payoffs, the right protocol stated *before* the macro-state guarantee is invoked; "$U_O=1$ is the contraction special case" needs the earlier composition/observability conditions to actually yield unique shared truth.
- **(526815, F239/F241/F242) — dependency / routing hygiene.** `der-gain-sector-bridge` used in the $\alpha'$ proof but not declared in `depends`; the (C-iv) scope route mutates `#scope-composite-agent` but is introduced here as a formulation choice (route it back or mark proposed-until-incorporated); the Class-composite cross-check leans on `#hyp-directed-separation-under-composition` without declaring it.
- **(526815, F243 / 193847 poke-1) — "fails here in a derivable way" outruns local derivation.** The active-inference and mechanism-design *implications* are positioned as adjacent, but the prose claims more than the segment locally derives. (This is the conflation counterpart to the gold item under Candidate Discussion above.)
- **(193847, poke) — "C-iv strategic composition produces a Class-3 composite from Class-1 sub-agents."** Flagged here because it is the **known C-iv idiom-drift integration-debt** vein called out for this batch: the architectural-class-change reading was *withdrawn 2026-05-21* (Cournot witness; dynamic-regime-change with architectural class preserved is current canon). 193847's vivid "bureaucratic insanity / structurally more corrupt than its parts" §14 framing rides on the superseded mechanism. **Stale OUTLINE residue confirmed at lift time:** `01-aat-core/OUTLINE.md` lines 91, 323, 328 still carry "Class-1-(Separated)-sub-agents → Class-2-(Partial)-composite" idiom in this chapter's rows — flag for the idiom-drift cleanup, out of scope for this WN-only lift.
