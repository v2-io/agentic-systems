...
- **Mechanism-design impossibility as `#disc-identifiability-floor` Instance 4.** Would require working out the external-theorem anchoring, the boundary characterization (Bayes-Nash / randomization / subsidies as structural escapes), and the strengthened consequence (which AAT machinery the escapes operationalize). Follow-on spike. The Gibbard-Satterthwaite arm specifically was tested in `spikes/spike-4th-identifiability-floor-instance-2026-05-20.md` §4 §"Strengthen-first check" with three reframings attempted and discarded: the named AAT-side escape (sub-scope $\alpha'$ potential-game machinery) addresses best-response-dynamics convergence rather than what GS forbids (strategy-proofness of the mechanism); GS forbids what a *designer* can *implement* under incentive-compatibility constraints, not what an *agent* can *identify* from a *regime*, so the actor-under-the-no-go and the kind-of-remedy differ from Instances 1–3; the standard GS escapes (Bayes-Nash IC, preference-domain restriction, randomization, subsidies) are not currently AAT machinery. Outcome C with a derived honest re-home: route GS to `#disc-separability-pattern`'s strategic-composition ladder general-open tier as the near-term home, not M1. Myerson-Satterthwaite and Arrow remain in the mechanism-design-impossibility cluster as candidates for a sibling meta-segment ("Implementation Impossibility") if mechanism-design becomes a first-class framework concern — a Joseph-reserved structural decision, not a floor-instance landing.

- **Composite-class-inheritance refinement.** The Class 1 (Separated) sub-agents → Class 2 (Partial) composite argument needs cross-checking against `#hyp-directed-separation-under-composition` for consistency. The refinement is structurally sound but deserves dedicated cross-reference work.
