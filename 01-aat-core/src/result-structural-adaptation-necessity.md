...
4. From #result-mismatch-decomposition, the estimation term has a positive lower bound that cannot be reduced by any $M \in \mathcal{M}$ — a class floor sitting above the state-uncertainty and channel floors.
...
1. **Persistent mismatch despite convergence**: $\Vert\delta_t\Vert$ remains large despite extended updating — the model has converged within $\mathcal{M}$ but the best achievable model is still poor. (On its own this is suggestive, not diagnostic: the channel and state-uncertainty floors keep absolute mismatch positive for any class; symptom 3 is what discriminates.)
...
The structured-residuals diagnostic (§2 gold above: autocorrelated/U-shaped residuals = class inadequacy; white residuals = floors) is operationalizable as an external agent-diagnosis test: run agents with deliberately inadequate vs adequate-but-noisy model classes and verify the residual-autocorrelation discriminator separates "architecture broken" from "world is noisy" without oracle access to $\mathcal{F}(\mathcal{M})$. If it holds for LLM-agent traces, this is a deployable answer to "is this persistent misbehavior architectural or environmental" — the triage question incident diagnosis actually faces. Agent-proposed forward pointer.
