...
for some $c \gt 0$. The sector parameter is then:
...
- **The $\nu$ / time-normalization gap between $\alpha = \eta^\ast c_{\min}$ and $\alpha$ as a rate — the strongest substantive item in this segment's sweep.** Persistence uses $\alpha$ as a *correction rate* ($\alpha \gt \rho/R$) and adaptive tempo is $\mathcal{T} = \nu\eta^\ast$ (also a rate), but the bridge gives $\alpha = \eta^\ast c_{\min}$ where $H$, $g$, $c_{\min}$ read as *geometric / per-event* and the event rate $\nu$ is absent — yet the Discussion asserts "$\alpha = \mathcal{T}$ exactly for linear correction," which needs $c_{\min}$ to fold in $\nu$, or $F$ to already be an event-rate-aggregated correction field, or an implicit unit-event-rate convention. Suggested fix: state the bridge in *two explicit forms* — per-event sector efficiency $\alpha_{\text{event}} = \eta^\ast c_{\min}$ and continuous-time sector rate $\alpha_{\text{time}} = \nu\eta^\ast c_{\min}$ — or define $F$ as the event-rate-aggregated field, since this is the load-bearing plank from gain-geometry to the persistence inequality (Codex/Claude, AUDIT-WORKING-526815). *(Flagged in the report: this is a precision/strengthening candidate, not pure framing — making the normalization explicit tightens the main-chain bridge.)*
