...
- Reasoning trail (filed): the correction that produced this no-go and the certifiability-discontinuity framing are in `spikes/.integrated/spike-w1-leakage-vacuity-2026-05-31.md` and `spikes/.integrated/spike-w1-w2-boundary-intuition-2026-05-31.md` (cycle narrative: CHANGELOG 2026-05-31).
- The exact-enumeration sim exhibiting the $\Theta(\varepsilon^2)$ flatness (`spikes/sim-w1-w2-boundary.py`, figure `spikes/w1-w2-boundary.png`) sits unintegrated in `spikes/`, blocking on the sim-integration process (G3 in `INTEGRATION-CLEANUP-TODO.md`).
