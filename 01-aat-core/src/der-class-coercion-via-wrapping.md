...
2. **Input-structure extraction** — competence at inferring latent structure *from the query's surface form*, including inferring the operator's $G_W$ from query patterns. This is the channel (C3) names.
...
The two competences pull in *opposite directions* on $\kappa_{W_1}$:

- *Accuracy effect (lowers $\kappa$).* A more competent world-simulator returns posteriors closer to the true conditional $P(\cdot \mid q_M)$, which is goal-independent by definition. In the limit of perfect world-simulation under fully decorrelated query-content / goal-content, $\kappa \to 0$ on the merits.
- *Inference effect (raises $\kappa$).* A more competent input-structure extractor uses any available query-content / goal-content correlation in the pretraining distribution to infer $G_W$ from the goal-blind query and bend its response accordingly. The same competence that makes the simulator accurate makes the implicit goal-inference more effective, raising the realized $\kappa$ even when no $G_W$ argument is structurally exposed.

For pretrained components where the two competences scale together (the typical case for LLMs trained on goal-rich corpora), the net direction of $\kappa$ in the fidelity limit is *empirical and component-specific*, not structural. The framework's current treatment of (C3) names the inference effect (via the pretraining-distribution mutual information bound) but does not separate it from the accuracy effect, and the separation appears to open more science than it closes.
