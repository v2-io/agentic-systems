...
> Off-ramp note: 526815 also raised a substantive *structural* challenge to the W₁ leakage bound — that $\kappa_{W_1} \le I(A(q_M); G_W \mid q_M)$ is identically zero for a stateless component under exact conditioning on $q_M$, so the bound as stated may not measure the leakage it intends (the real channels being query-content correlation, hidden component state, conversation history). This is a strengthen-first / correctness matter for the chapter's load-bearing construction, **not** Working-Notes gold — flagged in the lift report for the findings/adjudication track.
