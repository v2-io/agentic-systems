...
- **W₁ leakage-bound — regression guard.** Do not regress the selection-channel bound $\kappa_{W_1}^{\text{sel}} = I(A(q_M); G^{\text{op}}) \le I(q_M; G^{\text{op}})$ back to the form $\kappa_{W_1} \le I(A(q_M); G_W \mid q_M)$ that the body previously carried: that quantity is circular and vacuous — it bounds $\kappa$ by an average of $\kappa$, and evaluates identically to zero for the stateless oracle the theorem models, because conditioning on $q_M$ closes the only channel it measures. The fix relocated the goal variable from the wrapper register $G_W$ to the latent operator goal $G^{\text{op}}$ read off query content and dropped the conditioning (data-processing along $G^{\text{op}} \to q_M \to A(q_M)$ under (C1)+(C2′)); the embedded no-go — structural bound available iff (C2′) holds, otherwise behavioral-only — is `#disc-w1-structural-bound-boundary`. Reasoning trail (filed): `spikes/.integrated/spike-w1-leakage-vacuity-2026-05-31.md`, `spikes/.integrated/spike-w1-w2-boundary-intuition-2026-05-31.md` (cycle narrative: CHANGELOG 2026-05-31).
