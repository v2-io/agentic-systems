...
- **Mood is the slow second-order modulator of this gain (`#def-mood`).** A slow global scalar — mood, the leaky integral of the recent mismatch stream — sets $K$ over a slow timescale. This ties directly to the gain-collapse material above: at the mood timescale, pinning the affective gauge (injecting a mood-state instead of restoring its generating conditions) *is* gain collapse on the mood channel — the integrator stops responding to what it reads, the same $\eta^\ast \to 0$ pathology one slow loop up. The formal object is `#def-mood`; the applied/ethical reading of the equivalence is in `msc/mood-layer-sovereignty-carve-2026-06-17.md` (F1).
#### Candidate experiment (2026-08-12, agent-proposed during an incident-response & forensics applications read)

Gain collapse as a *pre-incident indicator*: the two collapse modes (spurious confidence $U_M \to 0$ vs spurious sensor-distrust $U_o \to \infty$) produce identical coasting behavior but should be distinguishable from the innovation stream's statistics. An empirica-registered experiment that induces each mode separately and tests whether an external monitor can (a) detect collapse before task failure and (b) classify the mode — routing to the correct repair — would convert this segment's failure taxonomy into a monitoring instrument. Agent-proposed forward pointer.
