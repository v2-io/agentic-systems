...
**Relationship to the model.** The model $M_t = \phi(\mathcal{C}_t)$ ( #form-agent-model) is a compression of the chronica. How much of $\mathcal{C}_t$'s predictive information survives compression is measured by model sufficiency ( #def-model-sufficiency).
