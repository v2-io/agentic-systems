...
**Noise-correlation scope.** Both sums are additive, inheriting the cross-channel noise-independence condition of #def-adaptive-tempo, whose exact scope is derived in #deriv-tempo-additivity — and the deviation under correlated sources is **signed**, in both directions. In the *common-source* direction (allies reporting from one upstream intelligence source, redundant status reports echoing the same dashboard), the additive communication tempo overcounts with a closed-form redundancy penalty, and under a *persistent* shared source it saturates outright: past the source's bias floor, additional correlated allies contribute zero marginal correction capacity — the multi-agent echo chamber. In the *synergistic* direction, allies whose report errors anti-correlate (or whose errors correlate with a reference ally's known failure modes) allow **triangulation**: the joint information exceeds the additive accounting, so the additive formula *under*counts — a genuinely super-additive composition effect, and a structural reason to value *diverse* allies over *many* allies, including allies nearly useless in isolation whose errors track a good ally's blind spots. No scalar dependence summary (in particular the mutual information between communication sources) can carry the exact correction — it is sign-blind ( #deriv-tempo-additivity, no-go N1); it survives only as a witness that the additive form is off. The exact joint object requires the cross-source noise covariance.
...
- **2026-07-16 — noise-correlation scope corrected (ripple from #deriv-tempo-additivity).** The former "channel independence caveat" asserted the additive communication tempo is an upper bound with an MI-dependent penalty; both claims were refuted by the 2026-07-15 derivation (the deviation is signed; sign-blind measures cannot carry the correction). Replaced with the two-sided scope: common-source overcounting/saturation AND anti-correlated-error triangulation (super-additive). Do not restore the upper-bound reading without checking the common-source structure holds for the application. The triangulation direction is a candidate for its own derived treatment (ally-diversity design consequence) if a downstream consumer needs it quantitatively.
