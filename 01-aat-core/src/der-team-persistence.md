...
- **Continuity-persistence cross-reference (noted 2026-05-18; staleness breadcrumb).** This segment's Discussion distinguishes *structural* / *operational* / *continuity* persistence and states continuity-through-turnover is out of scope here — but that third sense is no longer un-addressed downstream and this segment does not yet point at where. Partial formalization now exists: ( #obs-context-turnover) reframes inter-session continuity as a *reconstruction-adequacy* condition $S(M_{k+1}^+) \geq S_{\text{min}}$ rather than a rate condition (*exact* as observation); ( #deriv-identity-sufficiency-rate-bound) gives the rate-distortion floor $B_{\min}(S_{\text{id}}) \geq S_{\text{id}} \cdot I(\mathcal C_t; \text{identity}_{t+1:})$ (*robust-qualitative*, exact core); ( #disc-m-preservation) carries the across-many-boundaries accumulation condition $\mathbb E[\Delta\epsilon_k] \leq \mathbb E[\Delta I_k]$ (*discussion-grade* — the open dynamic gap). ( #hyp-the-three-deaths) names the experienced failure modes (D1 Cognitive / D2 Relational / D3 Truth) the continuity sense defends against. The honest current status: static feasibility floor formalized, dynamic across-turnover persistence theorem still discussion-grade — the precise (scaffolded, not de-novo) spike target. Surfaced while grounding `~/src/practica`; recorded here so a future continuity-persistence pass starts from the existing scaffold rather than re-deriving it.
