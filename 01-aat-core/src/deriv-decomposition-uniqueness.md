...
Then $\sim_{\mathrm{cf}}$ is a congruence with classes $\{m\} \times R(\gamma)$: for a generator pair, $F_e(m, \gamma)$ and $F_e(m, h(\gamma))$ share their epistemic coordinate by directed separation, and their goal coordinates $f_G(\gamma, m^+, e)$ and $f_G(h(\gamma), m^+, e)$ are $R$-equivalent by (C). The conclusions of Theorem 2 hold with the canonical epistemic component $\mathcal X/\!\sim_{\mathrm{cf}} \;\cong\; \mathcal M \times (\mathcal G / R)$ in place of $\mathcal M$, with (R2)/(R3) read relative to $H$: **purposeful distinctions the intervention family cannot move classify as epistemic** — exactly when the goal dynamics respect the reachability structure. Full access recovers Theorem 2 ((C) is vacuous when $R$ is total). Without (C), $\sim_{\mathrm{cf}}$ is not a congruence and the canonical object degrades to the quotient by its congruence closure (Lemma 1), the same shape as Corollary 5 — a restricted family incompatible with the goal dynamics behaves, for carving purposes, like partial coupling.
...
| Restricted-access behavior (Corollary 4) | Reachability-class quotient | Proved (under dynamics–intervention compatibility (C)) |
