...
- **The Kalman+LQR "Level-2 available" table row may be too generous** — a real technical subtlety surfaced by two substrates. The segment requires for L2 that *"the agent chose the action (it was not determined by the same causes that determine the observation)."* But under LQR the action *is* determined by the same causal system (the policy applied to the estimated state — the action is endogenous to the estimation loop). So strictly, Kalman+LQR may not have L2 *for the control actions* in Pearl's sense; the innovation-conditioned-on-action framing is necessary but not sufficient (Claude, AUDIT-WORKING-266847; same endogeneity point at AUDIT-WORKING-526815). Candidate clarity-fix: either tighten the table-row qualifier or add a sentence that the L2-availability there is "structural channel present, identification not guaranteed." *(This overlaps the do-vs-conditioning concern logged across the chapter — see the off-ramp note in the lift report.)*
