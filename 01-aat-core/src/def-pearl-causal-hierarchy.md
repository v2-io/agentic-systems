...
- ~~**The Kalman+LQR "Level-2 available" table row may be too generous.**~~ **Resolved 2026-05-30** (transportability spike, §3.5 do-vs-conditioning reconciliation). The table row and the two prose passages ("Availability vs. exploitation"; the intro clarification) now read "structural channel present, identification not guaranteed," naming the endogeneity explicitly (the LQR action is determined by the policy applied to the estimated state, so its data does not in general identify the interventional effect; the separation principle then also blocks *exploitation*). The original note (Claude, AUDIT-WORKING-266847; same endogeneity point at AUDIT-WORKING-526815): the segment's L2 condition requires *"the agent chose the action (not determined by the same causes that determine the observation),"* which the LQR control action violates.
