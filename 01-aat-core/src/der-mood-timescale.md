...
- **Open — mood-sets-gain composition.** `#def-mood` has mood *modulating* $K$, not *being* the estimator; the two-layer version should confirm the same $\tau^\ast$ governs the mood layer. A leading-order certainty-equivalence closure sketch (same $\lambda^\ast$ under symmetric local-quadratic loss; fails at the asymmetric-loss boundary) is in `audits/AUDIT-WORKING-731548/verify/06-mood-integration-verdict.md` and needs one independent re-derivation before landing as the corollary (TODO §mood arc (ii)).
