...
- **Opacity modulation ( #der-agent-opacity).** The coupling effectiveness $\gamma_A$ is not primitive: it decomposes as $\gamma_A^{\text{adv, effective}} \propto H_b^{A\mid B}/H_b^{\max}$, so this segment's tempo $\mathcal T_A$ enters adversarial dynamics as an *effective* tempo $\mathcal T_A^{\text{effective}} = \mathcal T_A \cdot H_b^{A\mid B}/H_b^{\max}$ (Model D). The superlinear ratio then carries a squared bilateral-opacity factor $(H_b^{A\mid B}/H_b^{B\mid A})^2$; the exponent $b$ is unchanged (opacity is a prefactor, not an exponent — open whether bilateral opacity reshapes $b$). A slow-but-opaque adversary can match a fast-but-transparent one at $\mathcal T_A H_b^{A\mid B} \approx \mathcal T_B H_b^{B\mid A}$.
