...
- **Status-label drift: frontmatter `conditional` vs body "Exact, with a partly definitional character."** Flagged by nearly every substrate that read the segment closely (Codex/Claude, AUDIT-WORKING-384279; Claude, AUDIT-WORKING-471203; Codex/Claude, AUDIT-WORKING-526815; Codex/Claude, AUDIT-WORKING-527914; Claude, AUDIT-WORKING-584721; Claude, AUDIT-WORKING-742613). Consensus: the C1/C2/C3 constraints are *framework-internal commitments* (not external conditions that might not hold), so the result is exact *within the framework* and "conditional" is a slight underclaim. Candidate: align the two — either frontmatter $\to$ `exact` with conditional-on-C1+C2+C3 noted in prose, or prose $\to$ explicitly "conditional under framework-internal commitments." Editorial; recurs strongly enough to be the segment's top follow-up.
    - **Resolved (verified 2026-08-12).** Frontmatter now reads `status: exact` and the Epistemic Status paragraph reads *"Exact, with a partly definitional character,"* naming C1/C2/C3 and marking C3 as definitional — i.e. the first of the two candidate repairs was taken. The two layers agree; the drift the six substrates flagged is closed. Observation left verbatim above as the first-encounter record; this line is the resolution, per the harvest's existing convention of annotating in place (cf. `#def-agent-environment` §4, "the segment now addresses this in its…"). *Noted while reading in outline order for an AISI-facing applications pass; the general point — that the gold contains resolved items that do not say so, and a reader mining it will re-raise closed issues — is a reconciliation-pass item for the harvest as a whole, not local to this segment.*
