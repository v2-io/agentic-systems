...
- **Provenance.** Derived in `spikes/spike-adaptive-tempo-redundancy-penalty-2026-07-15.md` (full trail, counterexample arithmetic, ripple-site record). Ripple audit executed 2026-07-16 (full-ingestion pass incl. segment git histories): `disc-independence-audit` §3 repaired (2026-07-15, with the landing); `der-team-persistence` corrected (false upper-bound + MI-penalty claims deleted, two-sided scope + triangulation stated, depends added); `def-strategic-tempo` scope made explicit in Epistemic Status (no false claim existed there); `der-tempo-composition` WN corrected two-sidedly — sharper finding: the synergistic direction conditions the sub-additivity bound $\mathcal T_c \leq \sum \mathcal T_i$ itself (fusion over pooled raw observations can exploit cross-agent noise correlation), flagged there for a follow-on derivation; `hyp-communication-gain` corroboration WN re-pointed; the CMI-subtracting figure candidate in `def-adaptive-tempo` WN annotated as N1-refuted-as-drawn.
