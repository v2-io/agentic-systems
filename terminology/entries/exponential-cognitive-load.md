...
where $k > 1$ is the compounding factor per boundary crossing. Even modest $k$
(1.1–1.2) create large differences at scale. The relationship may instead be
linear, sub-exponential, or structure-dependent — TST states this explicitly as
a hypothesis requiring validation.

Status `weak`: low vote count (2 votes / 2 architectures) and the term is
hypothetical even within TST. Tracking the name while the empirical picture
develops.
