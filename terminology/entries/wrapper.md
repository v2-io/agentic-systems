...
Wrappers can be more elaborate than the bare-minimum form — multi-component typed $M_W$ (e.g., PROPRIUM's VERA / MEMORATA / CONSORTIA / PERCEPTA / CHRONICA), multi-layered typed $G_W$ (AXIOMATA / OPERATA / PRAXES), substrate-heterogeneous query handling (auxilia hierarchy at varying cost-tiers per [`#def-auxilia-hierarchy`](../../04-eli-core/src/def-auxilia-hierarchy.md)). The bare-minimum form is what the class-coercion theorem requires; the elaborations are domain-specific structure.
