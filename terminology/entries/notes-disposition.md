...
Notes disposition is Gate 4 in the segment promotion workflow — the final gate, advancing a segment
from `format-clean` to `candidate`. A segment with unresolved Working Notes cannot advance.
...
The completion criterion is simple: the `## Working Notes` section is empty or absent. The segment
says what it means to say — no unresolved threads, no deferred promises that haven't been handed
off, no open questions that a reader would need to know about to evaluate the claim.
