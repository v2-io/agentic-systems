...
The Search Log field is the fifth and final field in a `## Findings` entry. It carries dated entries disclosing what literature search has been conducted and how. Each entry records: the date,
the search status (a depth tier), and a one-sentence note on what was searched and what was not.

Entries accumulate over time — older entries stay for traceability as searches deepen. The field is the discipline that prevents `Claim novelty on...` from being hubris: a claim of novelty under cursory search is honest about what it is; under comprehensive search, much stronger; under intuition-only, weaker but better than implicit silence.
...
An *intuition entry* is explicitly permitted — what the author's pre-search instinct says about where prior art might lie, which adjacent literatures would be natural targets. For AI agents,
this includes intuitions from training rather than active retrieval. Tagging as `intuition-only`
makes the source explicit. Intuition entries are valuable: they orient future searches and make the agent's priors visible so they can be confirmed or refuted.
