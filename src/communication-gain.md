...
- The communication gain enters the distributed tempo: $\mathcal{T}_i = \sum_k \nu_i^{(k)} \eta_i^{(k)*} + \sum_{j} \nu_{ji}^{\text{comm}} \eta_{ji}^*$. This is the formal basis for #team-persistence — teams persist where individuals cannot because cooperative communication adds to each agent's effective tempo.
