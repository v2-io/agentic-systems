...
- *Convergence*: As the model accumulates information, $U_M$ decreases, so $\eta^* \to 0$. The model becomes increasingly resistant to individual observations. This IS Kalman filter convergence, Bayesian posterior concentration, and RL learning rate annealing.
...
|--------|-----------|-----------------|
| Kalman filter | $K_t = P_{t\|t-1} H^T (H P_{t\|t-1} H^T + R)^{-1}$ | **Exact.** Scalar case is exactly $U_M/(U_M + U_o)$. |
