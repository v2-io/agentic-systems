...
> **Origin**: `~/src/agentic-tft/04-narrative-as-implementation.md` (Feb 2026 session, pre-AAT restructuring). TFT references should be read as AAT. PROPRIUM references point to `~/src/firmatum/`.
>
> **Relevance**: Key architectural decision for `03-llm-core/`. The argument that AAT quantities (mismatch, gain, tempo) are *estimated in language* by logogenic agents, not computed numerically. Three evidence lines: embedding experiments, language already encoding AAT quantities, causal structure argument. This is what makes logogenic agents' orient cascade different from the generic AAT formulation.
>
