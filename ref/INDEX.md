...
### Decision theory / revealed preference / utility representation (added 2026-05-30)

*Classical representation-theorem machinery grounding the real-valued objective-functional codomain in `#form-objective-functional`. All four are foundational textbook results cited directly per the prior-art-integration discipline (adopt with original names + citation; no local PDF required to be a tracked reference entry).*

| Filename | Citation | AAT sites |
|---|---|---|
| `vonneumann-morgenstern-1944-theory-of-games.pdf` *(standard textbook — direct citation)* | von Neumann, J. & Morgenstern, O. (1944). *Theory of Games and Economic Behavior*. Princeton UP. Expected-utility representation: completeness + transitivity + continuity (Archimedean) + independence ⟹ a cardinal $u$ (unique up to positive affine transformation) under which lotteries are ranked by expected $u$. | `#form-objective-functional` (A4 independence; the expected-value form of #def-value-object's $\mathbb{E}[V_{O_t}]$) |
| `debreu-1954-representation.pdf` *(standard textbook — direct citation)* | Debreu, G. (1954). "Representation of a preference ordering by a numerical function." In R. M. Thrall, C. H. Coombs & R. L. Davis (eds.), *Decision Processes*, Wiley, pp. 159–165. Complete + transitive + continuous preference on a second-countable (or connected separable) space ⟹ a continuous ordinal utility (unique up to continuous strictly-increasing transformation); the lexicographic counterexample on $\mathbb{R}^2$ admits no real-valued representation at all. | `#form-objective-functional` (A1–A3 ordinal representation; the E1 lexicographic non-representability boundary) |
| `afriat-1967-construction-utility.pdf` *(standard textbook — direct citation)* | Afriat, S. N. (1967). "The construction of utility functions from expenditure data." *International Economic Review* 8(1):67–77. Finite choice data are rationalizable by a continuous, monotone, concave utility iff they satisfy GARP (the Afriat inequalities are feasible) — existence, not uniqueness, and only on the observed comparisons. | `#form-objective-functional` (the existence-not-uniqueness / acting-yields-only-a-choice-function limit; E2 cyclic exclusion) |
| `samuelson-1938-pure-theory-consumer-behaviour.pdf` *(standard textbook — direct citation)* | Samuelson, P. A. (1938). "A note on the pure theory of consumer's behaviour." *Economica* 5(17):61–71. Origin of the revealed-preference program — preferences inferred from observed choice rather than assumed. | `#form-objective-functional` (revealed-preference lineage) |
