...
The *full* `#def-agent-spectrum` tetrad parallelism redesign — choosing the four quadrant names as one deliberately-parallel set — is still open. `blind seeker` is the interim canonical term; the other three names stand. Joseph has a separate agent brainstorming option-sets; this should land as a deliberate tetrad decision through the `bin/term` machinery, not another one-cell drive-by. Fold the #132 override record into `naming-rename-plan.md` when the tetrad decision lands.

Arguments that travel with the tetrad (why the R2 keep-pursuer vote does not bind): (1) *"pursuer" lexically narrows the objective axis* — predator→prey, the low/narrow end of objective-richness, backwards for the cell it marks; "seek" admits structured / plural goals and keeps the seeker/tracker -er pair; (2) the four names are not a grammatically parallel set (`Reactive system` / `Blind seeker` / `Adaptive tracker` / `Actuated agent` mixes category-nouns with role-nouns).
