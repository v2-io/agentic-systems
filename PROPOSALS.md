...
AAT's scope-condition presupposes an agent exists. Section I does not cover the *transition into agency* (pre-scope-condition content). For `03-logogenic-agents/` and especially `04-eli/` this is the scope-entry analog of Section I's scope-condition — currently an unnamed prerequisite.

**Scoping question:** does this belong in AAT core or in `04-eli/`? Reflection 19 introduces a candidate measurable quantity $S_{\text{id}}(M_t)$ (identity sufficiency, IB-analog of model sufficiency); that quantity points toward AAT-core placement.

**Primary aspect: completeness. Secondary: reach.** **Value: +5 for AAT-core / +7 for `04-eli/` where it unblocks the hardest subproject.** Not urgent at AAT level; genuinely load-bearing at logozoetic level.
...
(α) **Separate appendix segments, one per spike's Tier-2/3 product.** Each spike lands as its own focused appendix (`#deriv-passivity-composition`, `#deriv-pid-a2prime`, etc.). Pros: clean scope per segment; easier promotion gates; matches existing AAT discipline. Cons: ~9 new appendices in `01-aad-core/` (the appendix section is already crowded); some segments would carry overlapping operator-family content (e.g., passivity-composition and operator-sector-unification both invoke Bauschke-Combettes operator classes); cross-references would multiply.
