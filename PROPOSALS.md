...
**SP-14 confirmation from 2026-04-25 audit (B6 in `audits/pending-findings-2026-04-25.md`).** Audit's J4 marks `#deriv-persistence-cost` as "appendix-grade in placement, framework-grade in importance" — the channel-capacity floor $C_{\text{channel}} \geq \mathcal T/2$ pairs naturally with the threshold inequality $\alpha R \gt \rho$ as a two-prerequisite persistence story (one rate, one capacity), connects AAT to Landauer thermodynamic foundations without committing to thermodynamic-machinery-as-master, and gives Kalman-Bucy a privileged role (saturates the bound per Mitter-Newton 2005). Worth elevating SP-14's framing from "modest unification" to "substantive positive contribution" when executed.
...
- `spike-pid-a2prime.md` (B3; PID A2' via SPR/KYP positive-real; explicit $\alpha_{\text{PID}}$) — targets either $\alpha$-list refresh in `#deriv-sector-condition` or new appendix `#deriv-pid-a2prime`
...
**Independence: low for the bundle as a unit; medium for the scoping decision.** The scoping pass itself is read-only and parallelizable with most other work. The subsequent authoring (whichever architecture is chosen) touches `01-aat-core/src/` appendix territory and at least one meta-segment surface; depending on the decision, conflicts with Bundle 1's framework-face reframe if ($\beta$) is taken simultaneously. Serialize authoring with Bundle 1; scoping can run in parallel.

**STATUS — architectural question RESOLVED 2026-05-14 (the deep C1 push; `spikes/spike-operator-family-unification/`).** The ($\alpha$)/($\beta$)/($\gamma$) decision is settled, and *not* in favour of ($\beta$). The push verified that there is no "unified operator-family meta-pattern" peer to M1/M2/M3 — operator-sector is the *interior* of the equilibrium stability-certificate cone, not a fourth pattern alongside the others. The genuine unification landed as **two segments**: `#result-certificate-existence` (exact; the Lyapunov-theorem anchor, the segment-level home of the contraction-over-drift / O-BP10 organizing principle) and `#disc-stability-certificate` (the cross-sectional spine; M1/M2/M3 + composition exhibited as its facets), plus the Sylvester-recognition Finding in `#disc-identifiability-floor` and the OUTLINE preamble reframe. **This decouples SP-22's two questions:** the *meta-segment question* is answered (it is the certificate spine, now landed — not an operator-family bundle); the *Tier-2 backlog* (the nine ready-to-land math pieces — dissipativity-template, $\rho$-decomposition, PID/update-operator $\alpha$-list, jacobian-b1, kl-to-state-distance, l1-evidence-axiom) is **($\gamma$)-hybrid triage**, each landing at its own INDEX-stated target, *not* bundled and *not* gated on the spine. The remaining SP-22 work is therefore the ($\gamma$) backlog landings only; the architectural-decision portion is closed. CHANGELOG 2026-05-14 carries the cycle narrative; the spike's `99-verdict.md` carries the honest open edges.

**STATUS — ($\gamma$) backlog dispositioned by the spike-routing cycle, 2026-05-17/18** (`spikes/ROUTING.md`; `spikes/.integrated/MANIFEST-2026-05-17.md`; CHANGELOG 2026-05-17). The architectural portion closed 2026-05-14 (above); the ($\gamma$) backlog is now routed, verified (confirmer ≠ adjudicator, first-hand `src/`), and regression-checked:
...
- **Orphan, regression-checked CLEAR, tractable this cycle:** `spike-update-operator-sector` ((O-A2') + a sharp no-go; $\alpha$-op/$\beta$-op refresh into `#deriv-sector-condition` + the operator-layer no-go into `#disc-identifiability-floor` as present-tense canon; parent-owned §266(iii)/§8.2 placement).
...
**Re-grounding note (2026-05-31).** The result the Thesis above states — *forkability $\Rightarrow$ Pearl-Level-1, hence the causal-hierarchy theorem structurally forbids sandbox-to-deployment inference* — was subsequently **found false and replaced**. A deeply-mathematical de-novo auditor (AUDIT-WORKING-526815) challenged the forkability $\to$ Level-1 step, and read directly against the primary sources (Bareinboim 2022 Pearl-hierarchy Defs 4–10 / Thm 1; R-443 transportability Defs 1–2 / Thm 1) the challenge holds: forkability is orthogonal to the $\mathcal L_1 / \mathcal L_2$ distinction (CHT footnote 19 names forkable RCTs / A-B tests as the canonical Level-2 *source*), so a forkable sandbox is *not* a Pearl-level demotion. The genuine ceiling is **transportability / external validity** — sandbox and deployment are distinct populations related by a Bareinboim–Pearl selection diagram, and the no-go is the failure of transport across that diagram, which is *sharper, not weaker* (the result survives as a transportability no-go rather than a causal-level demotion). Landed 2026-05-31 in the causal-access transportability integration (commit `52d85b2`; CHANGELOG 2026-05-30 Causal Access): `#disc-sandbox-evaluation-ceiling` rewritten to the transportability form (`depends:` gains Bareinboim–Pearl 2014), with the sibling `#der-loop-interventional-access` gated on (C1)–(C3). The Thesis text above is retained as the record of the original (now-superseded) SP-25 proposal; the landed canon is the transportability form.
...
**Status: Executed — landed 2026-05-29** (name **anti-collapse** per Joseph; placement per the agreed "plant + meta-vocabulary + recall" design). Landed: `#disc-anti-collapse` in Meta-Architecture I (sibling-register to `#disc-constructive-impossibility-posture`, a style claim not a fifth facet) with the general shape + instance table + inverse dual; a *plant* at the first instance ($\beta$-vs-$\rho$, `#form-information-bottleneck`, Part I); an *earned recall* at the satisfaction-gap/control-regret split (`#def-satisfaction-gap`); and the Meta-Architecture I preface introducing it as a second style claim. Lint-clean; `bin/lint-outline` registers it with no missing-dep / orphan / ordering violation. See CHANGELOG 2026-05-29. *(Historical: the sweep ran first — 7 convergence-validated instances across all three Parts — and reframed "which-knob" → anti-collapse; details below retained as the reasoning trail.)* The instance sweep ran (`audits/.gem-hunt-trail/SP-26-disambiguation-sweep/sweep.md`): **7 clean instances across all three Parts** (not thin — clears the over-claiming bar). Parent-verified the two strongest first-hand — I1 `#der-interaction-channel-classification` ("the emitter-side collapse into a scalar $\gamma_A\mathcal T_A$ loses information that is load-bearing … 'more tempo' vs 'different model class' address structurally different failure modes" — Regime II-a/II-b) and I2 `#scope-observation-ambiguity-modulation` ($\kappa\times\mathcal{A}$, not $\kappa$ alone; the segment even records the $\kappa/\mathcal{A}$ conflation as a *fixed bug*). **The sweep improved the framing:** not "which-knob" but **anti-collapse / diagnostic-disambiguation** — *AAT refuses to collapse two things a naive model would merge, because the collapse hides a difference that routes to a different repair* — which also absorbs the inverse case (`#scope-edge-update-causal-validity`: two causes driving the *same* knob). The sweep's calibrated call (concur): name it, but as a *style/discipline* recognition (sibling to `#disc-constructive-impossibility-posture`, "M1.5 not M5") — **not** a fifth cross-sectional facet (it carries no theorem of its own). **Surfaced for Joseph (positioning/voice + a new canonical term → naming-discipline territory):** (a) the name (anti-collapse / diagnostic-disambiguation / other); (b) placement — a discussion-grade `disc-*` style segment with the instance table (canonical home, additive), and/or a "Reading AAT" framing paragraph (cheap, immediate positioning). Ready to draft + land on his steer. **Value:** +3 framework / +4 positioning (the "integration not invention" line undersells this; cf. `math-novelty-recognition`). **Independence: high.**
...
**Interactions.** Upstream `#def-chronica` / `#form-agent-model` (the lossy-$\phi$ commitment) + downstream `#def-death-as-factor-loss` / `#def-identity-sufficiency` (the Zi-am-tur substrate-switch empirical symptom this would *explain*).
...
**Status: Executed as a clarification — landed 2026-05-29** (see CHANGELOG; commit lands a `#scope-agency` Discussion paragraph "Which contrast counts — the channel distinction, and the observation-mediated boundary"). **Refined during landing — the proposal's "tighten the formal condition to require $\Omega$-routing" was *not* done, deliberately:** close reading showed it would over-reach. `#def-observation-function:13` *intentionally* admits active perception ($o$ depending on $a$), and `#der-loop-interventional-access:25` frames the Level-2 character broadly as "$a_t$ causes $o_{t+1}$" — so formally excluding active-perception-only contrast from the *agency set itself* would contradict the framework's deliberate active-perception inclusion. The true fix distinguishes the two contrast-*channels* and identifies the $\Omega$-routed one as the substrate that grounds causal-structure learning (capturing both agents' gems — the proprioceptive over-inclusion *and* the observation-mediation under-inclusion — without amputating active perception). If Joseph prefers the stronger move (a formal-set tightening, and a corresponding stance that active-perception-only systems are outside agency *by definition* rather than at its boundary), that is a foundational call he can direct; the clarification stands as the conservative, non-over-reaching landing. **Value:** +3 framework (sharpens what the foundational scope condition is *for*; removes a real over-read of "the loop is a Level-2 engine"). **Independence: high.**
...
#### SP-30 — Ratify the totality reading of $\Omega$ (fix `#scope-adaptive-system`'s false exclusion-rationale; scope in standard RL)

**Source:** audit 731548 B-3 (Fable de-novo, Part I), adversarially verified with full historical access (`audits/AUDIT-WORKING-731548/verify/04-scope-exclusion-verdict.md` — REAL-AND-OPEN, *upgraded* during verification). Routed as a proposal, not a direct fix, because the repair touches `#def-agent-environment` (a `depends: []` root definition) plus two constitutive atoms — SP-24/SP-25 precedent.

**Thesis.** A fully-observed MDP with **unknown transition kernel** (standard tabular RL, $o_t = \Omega_t$) has $H(\Omega_t \mid \mathcal C_t) = 0$ — excluded by `#scope-adaptive-system`'s predicate — yet needs the full adaptive machinery, since mismatch is *prediction* error $\delta_t = o_t - \hat o_t$, alive under full observation with unknown $T$. The exclusion's stated rationale ("complete knowledge … the adaptive machinery becomes vacuous", repeated in `#def-agent-environment`) is *false* for this case by the framework's own definitions; the prose dates to the earliest FORMAT-era commit and has survived every cycle (audit-384279 certified Ch.1 "clean"). Canon's worked RL examples pass the predicate only by an **unratified per-example hyperstate choice** (`#example-bandit` silently sets $\Omega$ = reward-law parameters, scope row marked *exact*), while `#disc-exploit-explore-deliberate` notationally splits $\theta$ from $\Omega_t$ the *other* way. **The fix: ratify the totality reading** — a paragraph in `#def-agent-environment` stating what random object $\Omega_t$ is (unknown law-content lives inside the hyperstate), making the scope predicate exactly track "some learnable thing remains unknown." The verifier's four-quadrant check shows the totality reading is the *unique* candidate repair classifying every knowledge/observability corner correctly (hidden-state/known-law Kalman: in ✓; observed-state/unknown-law RL: in ✓; observed-state/known-law, deterministic *or purely aleatoric stochastic*: out ✓); the prose-only restatement and the one-step-predictive-entropy alternatives both fail it.

**Scope of execution** (the verifier's side-effect audit — *not* a one-paragraph no-op): (i) the totality-reading paragraph in `#def-agent-environment` + the corrected exclusion sentence and Discussion chain in `#scope-adaptive-system`; (ii) **two constitutive atoms become frame-relative reinterpretations needing rewording, not theorems** — under the hyperstate construction the *extended* kernel is known-as-a-function, so `#def-action-transition`'s "the agent does not know $T$ exactly" is false of $\tilde T$ and true of the realized $T_\theta$ ("unknown-law ⇔ unobserved law-component of state"), and its "if $T$ were known, action selection would collapse to plain optimization" needs rewording; same family for `#def-observation-function`; (iii) one clause in `#disc-exploit-explore-deliberate` (its environment-bits/inference-bits accounting notates $\theta$ as distinct from $\Omega_t$; under the totality reading both channels reduce uncertainty about the same random object, still distinguishable as observation-driven vs computation-driven); (iv) one look at `03-llm-core` `#scope-observation-ambiguity-modulation` regime (i). Bonus: cleanly subsumes 193847's unresolved computational-opacity flag (perfect perception + intractable prediction ⇒ the effective law-component is not extractable from $\mathcal C_t$ — an extension deserving its own scrutiny, not claimed here). No downstream derivation relies on $T$ being outside $\Omega$ (checked: NOTATION, `#post-causal-structure` zero-coupling, GA-1, `#result-mismatch-decomposition`).

**Findings subsumed.** 731548 B-3; the verifier's double dissociation (Kalman: in scope by the predicate while violating the opacity atoms; fully-observed unknown-$T$ MDP: satisfies the atoms' spirit while failing the predicate — the two formalizations currently measure different things, and this proposal is the reconciliation); partially the `#def-observation-function` WN "candidate scope-clarification" (four-substrate convergent, unresolved).

**Status: Open — needs Joseph** (root definition + constitutive atoms; a foundational ontology call: what *is* $\Omega_t$). The evidence strongly suggests the totality reading is already the framework's intent — canon's own examples exercise it silently — so the proposal ratifies existing practice rather than innovating. **Reshaping in progress (2026-07-04):** the repair as drafted is one noun short — the deeper fix is the *typed epistemic target* $S_t = (\Omega_t, \theta)$ with law-content $\theta$ as a named slot (the totality reading becomes its corollary). Workshop + push-agent deposits at `spikes/epistemic-target-ontology/` (00-spike + three push reports). **The gating claim is now verified (2026-07-04, `01-ga1-verification.md`):** unknown observation-law breaks the decomposition under state-only $\Omega$, and more — GA-1 under an unspecified housing is *indeterminate* (observationally-equivalent twin), and no assignment of unhoused $\theta_h$ preserves both the identity and the labels (two-horn no-go). The typed repair restores both simultaneously. Remaining before execution: Joseph's root-ontology call (the decision package is assembled in the spike folder). **Value:** +4 framework (the broadest membership test stops misclassifying a canonical system class; one missing definition currently paid for across ≥4 segments) / +2 paper (RL is a reviewer-facing column). **Independence: medium-high** (foundational segments; serialize with anything touching Ch.1; consistent with executed SP-24 — the coupling reframe kept the information-loss boundary constitutive, and this states what the withheld object *is*).
