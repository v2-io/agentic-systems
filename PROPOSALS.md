...
**STATUS — (γ) backlog dispositioned by the spike-routing cycle, 2026-05-17/18** (`msc/spike-routing-2026-05-17.md`; `spikes/.integrated/MANIFEST-2026-05-17.md`; CHANGELOG 2026-05-17). The architectural portion closed 2026-05-14 (above); the (γ) backlog is now routed, verified (confirmer ≠ adjudicator, first-hand `src/`), and regression-checked:
...
#### RESERVED DECISION (durable home — 2026-05-18). The Instance-4 / Object-B / CL-2-heavy unification — *one decision*, Joseph's.

This entry **is** the queued home (self-contained; the spike-routing tracker and TODO point *here*, not vice-versa). The independent identifiability-floor spike (`spikes/spike-identifiability-floor-instance4-resolution-2026-05-18.md`) resolved the long-contested question by splitting it; the recheck spike (`spikes/spike-rho-structure-recheck-2026-05-18.md`) and the CL-2 §7 linkage proved these threads are **the same object**:

- **Resolved truth (established; the §4.1 mark is live in canon):** canon's `#disc-identifiability-floor` "Instance 4" conflated **Object A** — universal-C/non-(PI), a *category error, not a floor* (the cited source `#deriv-observation-ambiguity-bias-bound:127` already says so; honesty-marked KNOWN-DEFECTIVE in `disc-identifiability-floor.md` 2026-05-18, canon-grounded) — and **Object B** — architecturally-distinct/behaviorally-identical agents, a *genuine* floor (= the rho Regime-C confound = CL-2's heavy refinement = the neutral-drift §8/§10.1 candidate).
- **Math gate: CLEARED 2026-05-18** (`audits/SPIKE-VERIFY-471802`, confirmer ≠ spike). No claim refutes; Object B is a genuine never-landed floor (regression-axis clear via exclusion-pickaxe); the §7 CL-2 linkage holds on `#result-mismatch-decomposition` C3 (`status: exact`), nothing smuggled. **True tier: exact-in-linear-Gaussian-sub-scope / robust-qualitative-general — *not* flat exact.** Land-worthy **conditional on** the verdict's named repairs, which a landing agent must read from the verdict (not the spike alone): (i) the boxed "every moment of ‖δ‖ identical" clause is overclaimed (true only for innovation/output + similarity-invariant summaries); (ii) a Lyapunov-equation sign slip (`FΠ+ΠFᵀ=σσᵀ`, not `…+σσᵀ=0`) — conclusion survives but the display is sign-wrong; (iii) carry the sub-scope/general tier boundary consistently (a strengthen-before-soften tier-honesty pass, not a downgrade). The gate caught the spike overclaiming — the fourth time the confirmer-≠-author discipline paid out this cycle.
- **The decision (Joseph's; one move, not four):** (i) relabel/retire the Object-A "Instance 4"; (ii) install Object B as the genuine 4th floor / third rank-collapse member; (iii) land CL-2's heavy 𝓜/π-cross refinement (same object); (iv) the meta-segment renumber + the `disc-identifiability-floor` Findings "four" reconciliation. Plus the separate Joseph-batch items (language-as-causal-substrate Theorem-1 + C2★; attention-governance; temporal-nesting-rg B/C/D) remain their own calls.
- **Status:** honestly tiered and released (the §0c discharge): canon carries the §4.1 mark + honest tiers; this is the durable queue entry; nothing is asserted beyond the gated truth.
