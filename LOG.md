...
3. **`#form-consolidation-dynamics`** — offline regime of `#der-recursive-update`'s between-event dynamics with IB-gap-reduction objective. Names the **stability-plasticity feasibility window** that closes an asymmetry in `#schema-strategy-persistence` (plasticity lower bound only, no stability upper bound). Required as architectural primitive in `03-llm-core/` under context-turnover.
