...
%----------------------------------------------------------------------------
% Widow / orphan suppression.
%
% Widow = last line of a paragraph stranded at the top of the next page
% Orphan (LaTeX "club") = first line of a paragraph stranded at the bottom
% of the previous page
%
% Penalty 10000 effectively forbids LaTeX from breaking there. The cost
% is occasional extra whitespace at page bottoms when LaTeX has to pull
% an orphan down or push a widow forward; we accept that for cleaner
% paragraph integrity. Single-sentence stragglers at the top of a fresh
% page read as broken; the whitespace trade-off is the right one.
%----------------------------------------------------------------------------
\widowpenalty=10000
\clubpenalty=10000
...
%----------------------------------------------------------------------------
% Equation + table numbering — both reset per chapter. Display is
% \thechapter.\arabic — kaobook's default arabic chapter, so "(3.5)" and
% "Table 3.1" sit alongside "Definition 3.4". Section / equation / table /
% segment numerals all read as a single arabic family across the volume.
%----------------------------------------------------------------------------
...
% AAT's Details appendix alone has more than 26 chapter-level entries.
%
% Formula: for n in [27, 26+26²], chunk = (n-1) ÷ 26, rem = ((n-1) mod 26)+1,
% emitted as \@Alph{chunk}\@Alph{rem}. Implementation uses the \numexpr
% arithmetic-shift trick: floor((N-1)/26) ≡ round((2N-27)/52). Fully
% \edef-expandable AND truncating. \numexpr (rounds), \divide/\multiply
% (not edef-safe), and \inteval (also rounds, despite the name) all fail
% one or the other constraint — see sibling scrbook preamble for the full
% failure-mode write-up from the 2026-05-22 build-break diagnosis.
%----------------------------------------------------------------------------
\makeatletter
