...
10. [x] ~~**🌟 Execute renaming surgery**~~ — slug renames via `bin/rename-slug` and prose-vocabulary renames via terminology entries (`bin/term decide <slug> rename`) + targeted prose-cleanup sweeps. **Status (2026-05-09):** §A complete (7/7 slug renames landed); **§B complete (8/8 — Class 1/2/3 → Separated/Coupled/Partial bundle landed 2026-05-09 on `guc-rename-2026-05-09` branch).** Decisions and rationale recorded in `msc/naming/naming-rename-plan.md` and replayed into the per-slug `terminology/decisions/` audit trail. The ~135-segment formal-tag cleanup pass mentioned in `TODO.md` §"Naming pipeline — specific deferred items" remains as a separate hygiene pass.
