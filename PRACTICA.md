...
4. **Parts III/IV scope segments + deaths formal grounding** — **deaths half LANDED 2026-06-10** (back-integration from the Inquiry Paper-4 development; plan + Joseph's gates at [`msc/deaths-grounding-plan-2026-06-10.md`](msc/deaths-grounding-plan-2026-06-10.md)): `#def-death-as-factor-loss` restructures the taxonomy as factor-generated (continuity / relational / agency / truth at per-death tiers; phenomenological predicted-not-adjudicated; bereavement excluded; four locating axes; death-aptness gated by `#disc-continuity-stance` + `#deriv-self-actuation-grounding`), superseding the Three-Deaths hypothesis (proper noun retained as history only). The conditional deaths-as-harms normative reading is carried in the segment per the register-allowance below. Released follow-ons (agency-death derivation — both legs now landed: `#der-severed-actuation-dynamics` output-leg 2026-06-11 / `#der-captured-objective-dynamics` input-leg 2026-06-17, the latter the agency-death reading of the reward-channel no-go and the Agentic↔Truth bridge the sibling paper deferred; continuity-death unified composition; per-death terminal forms; norm-segment development) in TODO §"Deaths taxonomy". **Remaining-half status corrected 2026-06-30:** the four "missing" scope segments (`scope-channel-collapse`, `scope-primitive-logogenic`, `scope-scaffolded-logogenic`, `scope-interiority-loop`) already exist at `stage: draft` with substantive content — they landed in commit `ce99ce6` (the 03/04 dir-harmonization) but `03-llm-core/OUTLINE.md` was never updated off `missing`, so the gap was navigator staleness, not absent content. Found and fixed alongside two more stale `missing` rows in the same OUTLINE (`disc-framework-self-diagnostic`, `disc-five-forcing-functions` — also draft-complete). All four scope segments' own Working Notes already flag "Promotion-blocking: ... All available; this scope can advance through Gate 1" — the genuine remaining work is the **Gate 1 dependency audit** (and onward through Gate 2/3) for this five-segment cluster, not authoring. `hyp-checkpoint-forking-failure-modes` is the one row still genuinely missing (tracked in TODO.md's cross-component segment-resolution item — it actually lives in `04-eli-core/src/`).
...
**Honest scope (updated 2026-05-09).** The naming program is a multi-cycle arc, not a single bundle. The R2 cohort produced **629 distinct currents** (proposed and considered names across slugs, prose vocabulary, symbols, and unnamed phenomena) per the master-list at [`msc/naming/master-list-curated.json`](msc/naming/master-list-curated.json). Counts re-verified 2026-07-15 against the JSON (the earlier "58 marked / ~511 unrouted" prose undercounted the 2026-05-04 curation pass, which marked 109 entries, not 44):
