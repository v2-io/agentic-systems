...
1. **Naming refactor closure** (🌟 already-active) — Class 1/2/3 → Separated/Coupled/Partial *with* the Class 2 ↔ 3 numbering swap (semantic-reversal: post-rename Class 1 = Separated, Class 2 = Partial *was Class 3*, Class 3 = Coupled *was Class 2*). Brings architecture into ordering-alignment with the other six AAD ladders (cleanest → middle → worst, matching Pearl's hierarchy and the separability ladder). The class-coercion segments landed today use the old vocabulary throughout; doing the rename next prevents accumulating retrofit debt. **Authoritative execution plan: [`msc/class-rename-execution-plan-2026-05-09.md`](msc/class-rename-execution-plan-2026-05-09.md)** (surface-area enumeration, warning-callout discipline, migration-note convention, terminology-system entry shape, git-tag step, open questions, phased sequence).
