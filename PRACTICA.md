...
10. [ ] **🌟 Execute renaming surgery** — slug-renames via `bin/rename-slug` (the seven queued in [`msc/naming/naming-rename-plan.md`](msc/naming/naming-rename-plan.md) §"Pending subject-noun renames — surfaced post-R2"); prose-vocabulary renames via LEXICON edits + targeted prose-cleanup sweeps (Class 1/2/3 → Separated/Coupled/Partial *with coordinated Class 2 ↔ 3 numbering swap*; Knowledge Type axis name; logozoetic agent → ELI; the four batch-F1 prose-handle renames; grafting → strategic grafting). Includes the ~135-segment formal-tag cleanup pass mentioned in `TODO.md` §"Naming pipeline — specific deferred items".
11. [ ] **Land canonicalize commitments to LEXICON** — 88 currents marked `canonicalized*` in `master-list-curated.json` rename_status; LEXICON.md edits ensure each has a canonical entry. Several sub-types (clean / nuance / FORMAT-layer / compound / adopted-standard) imply different LEXICON-entry shapes per [`msc/naming/naming-rename-plan.md`](msc/naming/naming-rename-plan.md).
