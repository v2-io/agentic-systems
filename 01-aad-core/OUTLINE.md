...
*The derivation chain for this section is mature (see `msc/spike-v3-purposeful-agent.md`). Most of it provides better justification and epistemic labels for architecture that already existed. The genuinely novel results are: the satisfaction gap / control regret split ( #def-satisfaction-gap, #def-control-regret) with convention hierarchy ( #def-value-object), the $G_t$ complexity bound (in #der-orient-cascade), and the graph structure uniqueness argument including the CMC-based Markov proof (see #deriv-graph-structure-uniqueness).*
