...
*Exact* under the **Fisher-local invariance regime**: for any smooth log-likelihood admitting local quadratic expansion (equivalently, a non-degenerate Fisher information matrix), the natural-gradient Bayesian update at first order in the step size has the form $\eta^\ast = U_M / (U_M + U_o)$ with $U_M, U_o$ read off the curvature of prior and likelihood respectively. The linear-Gaussian (Kalman gain) and conjugate-Bayesian cases are instances of this regime where the local expansion is also globally exact; for general smooth models the natural-gradient invariance theorem guarantees the form is exact at the linearization and approximate to the order that the local quadratic captures the loss geometry. Outside this regime — heavy-tailed posteriors, structurally non-smooth likelihoods, multimodal uncertainty where the local quadratic misrepresents global structure — the dependence remains *robust qualitative*: the **direction** is preserved (gain rises with model uncertainty, falls with observation noise) and the **first-order form** is recovered locally; what need not hold is global quantitative fidelity. The qualitative direction is the load-bearing claim for downstream tempo and persistence machinery; the Fisher-local exact form is the load-bearing claim for the Kalman, conjugate, and natural-gradient instantiations.
