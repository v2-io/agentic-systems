...
**Anchor role in the additive-coordinate-forcing pattern.** The log-of-product decomposition here anchors two further AAD uniqueness theorems that force logarithmic coordinates at other layers: reverse-KL at the divergence level ( #strategy-cost-regret-bound §6.1) and log-odds at the update level ( #edge-update-natural-parameter). Each of those theorems cites the chain-layer identity here as the analog motivating its additivity axiom. The catalog and the precise anchor-plus-two-theorem characterization live in #additive-coordinate-forcing.
