...
**L1 (Augmented DAG)** is the practical sweet spot. The agent models correlation structure explicitly by adding common-cause nodes to $\Sigma_t$ and restructuring the DAG so that the common cause is **factored above the correlation it creates**. The construction principle: place the common-cause node as an AND-prerequisite *above* the OR/AND structure whose children it correlates. This ensures that, conditional on the common cause being satisfied, the children are independent and standard AND/OR propagation is correct.

*Example* (see #worked-example-L1 for full treatment): Two OR-alternatives sharing infrastructure dependency ($\theta_C = 0.8$, $\theta_{1|C} = 0.9$, $\theta_{2|C} = 0.7$). L0 computes $\hat P_\Sigma = 0.877$; actual is $0.776$ (overestimation = $\rho$, the covariance from shared infrastructure). Naive L1 (common cause as parent of both alternatives, alternatives remain OR-siblings) gives the *same* overestimate because the OR-propagation still treats siblings as marginally independent. Correct L1 ($G = \text{AND}(C, G_{\text{sub}})$ where $G_{\text{sub}} = \text{OR}(A_1, A_2)$) gives the exact answer because $A_1$ and $A_2$ are conditionally independent given $C$. The sector condition is verified (Prop B.6 in #strategic-dynamics-derivation) with $\alpha_\Sigma = \min(1/(n_C+1),\;\theta_C(1-\varepsilon)/(n_{A_1}+1),\;\theta_C \varepsilon/(n_{A_2}+1))$ — combining evidence-starvation and exploration-gating effects. B.5b transfers losslessly.

All L0 formal results transfer to correctly constructed L1 DAGs because the augmented DAG is a standard AND/OR DAG that satisfies causal sufficiency. The cost is a larger, more complex graph with a lower sector parameter (the common-cause gating slows calibration of downstream edges); the benefit is that $\Phi^{L1}$ matches actual plan success rather than the biased $\Phi^{L0}$. The practical challenge is twofold: (1) identifying which common causes matter enough to model explicitly, and (2) restructuring the DAG correctly — both are modeling judgments, not mechanical procedures.
