...
- **LMI generalization landed.** The multidimensional resolution is in `#deriv-causal-ib-lmi` (matrix Lagrangian on the Fisher Information Matrix; blank-wall attack resolved by complementary slackness on direction). Spike trail: `msc/spike-causal-information-bottleneck.md` §7 (critique) and `msc/spike-causal-ib-lmi-repair.md` (LMI derivation).