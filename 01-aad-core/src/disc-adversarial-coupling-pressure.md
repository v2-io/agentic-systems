...
In adversarial settings, opponents do not merely deceive through the observation channel; they apply strategic pressure that drives a target *across architectural classes* — from Class 1 (modular) toward Class 2 (fully merged). Coupling is itself an attack surface: a Class-1 agent can be deceived only through the observation channel, while a coupled agent exposes additional input ports through $O_t$ and $\Sigma_t$. The result is a use-case expansion for `03-logogenic-agents/`'s coupled formulation: the population that needs coupled-formulation analysis is broader than the architecturally-coupled. Any agent under sustained adversarial pressure that can drive its $\kappa_{\text{processing}}$ upward sits in the same scope, regardless of native class.

## The phenomenon

*[Discussion]*

A Class-1 agent's $f_M$ accepts inputs only through the event stream; its $O_t$ and $\Sigma_t$ have no legitimate input port from outside that stream. An adversary's only attack channel is engineered observation ambiguity, and the bias they can induce is bounded by `#deriv-observation-ambiguity-bias-bound`'s constant $C$ relative to the ambiguity they can manufacture.

A coupled agent — Class 2, or Class 3 with active $O_t \to M_t$ pathways — exposes additional channels. Three operational mechanisms recur across the literatures on propaganda, social engineering, marketing, and influence operations:
...
The Class-2 coupled formulation handed off to `03-logogenic-agents/` was originally scoped to agents whose native architecture is non-modular: LLMs whose attention processes goals and observations together; biological cognition with motivated-reasoning pathways. Adversarial coupling pressure expands this population: a Class-1 organization without internal $O_t \to M_t$ pathways but operating in a hostile information environment that targets its leadership's identity and commitments is, for analytical purposes, in the coupled-agent population during that engagement. The native architecture is irrelevant once the coupling exists in operation.
...
|---|---|---|---|
| **Truthification** | self-driven | increases | restores diagnostic; opens Section II results; instantiated by *defensive scaffolding* below |
...
Restoring modularity under adversarial pressure is structurally a Section III move. Institutional scaffolding — peer review, prediction registers, double-entry bookkeeping, adversarial procedure, prediction markets, source-authentication discipline, structured red-teaming — constructs a composite agent *(individual + scaffolding)* with better Class-1 properties than the bare individual, even when the individual's native architecture is Class 2 or driven there by adversarial pressure. The scaffolding externalizes channels of the orient cascade (belief-update checked by parties whose $O_t$ doesn't share the individual's; prediction registers fixing $M_t$-claims before outcomes resolve; adversarial procedure forcing the feasibility check) so that resolution can run sequentially across the composite even when it cannot run sequentially within any single component.

The defensive composite is a Class-3 architecture (partially modular at the composite level) whose modularity properties exceed those of its sub-components — an inversion of the usual Class-1-sub-agents → Class-3-composite refinement: here the sub-agents are Class 2 (or under adversarial pressure toward it), and the composite is the modularity-restoring level.
...
- The Class 1 / 2 / 3 partition and $\kappa_{\text{processing}}$ measure are *robust qualitative* per `#der-directed-separation`.
...
- **OUTLINE.md placement.** The segment is in `01-aad-core/src/` but is not yet in `01-aad-core/OUTLINE.md`. Natural home: under Section II (it sits at the architecture-classification layer) or as a meta-segment alongside `#disc-separability-pattern` and `#disc-identifiability-floor`. The latter is probably the better fit — it's a meta-pattern that cuts across Section II and Section III rather than a Section II result proper.
