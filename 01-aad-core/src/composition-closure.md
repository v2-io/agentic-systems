...
**(A1) AAD agent structure.** The macro-state decomposes as $X_c = (M_c, G_c)$. The macro-update is recursive:

$$X_{c,t+1} = f_c(X_{c,t}, o_{c,t+1})$$

The macro-policy is state-dependent: $a_{c,t} = \pi_c(X_{c,t})$. This ensures the macro-agent has the same structural components as a single AAD agent ( #agent-environment, #recursive-update, #action-selection).
...
The closure defect $\varepsilon^\ast$ is well-defined given (A1)-(A4), (P1)-(P3), and specified norms. The macro-dynamics admissibility (A1)-(A4) and the projection admissibility (P1)-(P3) are independent formulation choices — (A1)-(A4) specify what "AAD-shaped macro-dynamics" means; (P1)-(P3) specify what "informative, regular, genuinely reductive projections" means. Both are needed for $\varepsilon^\ast$ to be an infimum over a non-trivial, non-degenerate class. The projection admissibility conditions have a proposed definition with one exact instantiation (two-Kalman case, `msc/spike-projection-admissibility.md`); general-case computability of (P1) remains open.
