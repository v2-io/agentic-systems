...
The macro-policy is state-dependent: $a_{c,m} = \pi_c(X_{c,m})$. This ensures the macro-agent has the same structural components as a single AAD agent ( #def-agent-environment, #der-recursive-update, #der-action-selection).
...
| (A1) Macro AAD structure $X_c = (M_c, G_c)$ with recursive update | Import from #def-agent-environment + #der-recursive-update + #der-action-selection | Formulation choice (requirement) |
...
| $N$-agent scaling of $\varepsilon^\ast$ (polynomial vs. exponential in team size) | Coupling-structure dependent; `spikes/spike-composition-scaling-N.md` | Open |
