...
A note on placement: causal information yield (CIY, in #def-causal-information-yield) is a Pearl-Level-2 quantity and could conceptually wait until Section II's purposeful agents. It lives here because its operational role is to score actions for their informational value to the cycle. Causality enters AAD's dynamics first through action selection and observation generation, not through purposeful planning. Section II will lift CIY into a strategy-revision context; Chapter 3 establishes the per-action quantity it operates on.
...
- The CIY-placement paragraph addresses what could otherwise read as a placement anomaly. The depends-list pulls Pearl forward from Chapter 1, but the operational role is here.
