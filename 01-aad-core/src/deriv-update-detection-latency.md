...
This is the specific link between AAD's constructive meta-pattern ( #disc-additive-coordinate-forcing, via #deriv-edge-update-natural-parameter's theorem) and a downstream detection-latency consequence. The rate cannot be escaped without abandoning evidential additivity — which would invalidate the update rule on AAD-internal grounds, not merely operational ones.
