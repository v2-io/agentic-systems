...
**Regret-bound derivation of KL direction.** Under AAD's canonical scope, $\pi^\ast = \delta_{a^\ast}$ is deterministic ( #def-value-object). Define the strategy-induced regret against $\pi^\ast$ as $R(Q_{\Sigma_t}) := V(a^\ast) - \mathbb{E}_{a \sim Q_{\Sigma_t}}[V(a)]$, where $V(a) = Q_O(M_t, a; \pi_{\text{cont}}, N_h)$ is the action-value ( #def-value-object, $O_t$ induces $V$ via #form-objective-functional). Under bounded value range $V_{\max} := \max_a V(a) - \min_a V(a)$:
