...
**Not yet started:**
- 🚧 **Markdown-first restructure (Stages 1+2 landed, Stage 3 partial)** — design at [`msc/markdown-first-pipeline.md`](msc/markdown-first-pipeline.md). The pipeline restructures around a chunked intermediate: `source-outline + source-segments → index + chunks → assembled markdown → LaTeX → PDF`.
  - ✅ **Stage 1 (ingest)** — `common/lib/ingest.rb` walks `OUTLINE.md` and produces `mono/.build/<slug>/{index.md, chunks/*.md}`. Each chunk is print-ready markdown with the documented metadata-block contract. Per-section academic numbering computed natively; appendix chapter letters via `\AlphAlph`. Source hashes recorded for incremental rebuild.
  - ✅ **Stage 2 (assemble)** — `common/lib/assemble.rb` stitches index + chunks into the canonical per-volume markdown at `mono/<slug>-v<sem>.md`. Cross-refs resolved to numbered anchor links. The Python `bin/build` (legacy) moved to `_obs/`.
  - 🚧 **Stage 3 (typeset)** — `common/lib/typeset.rb` converts assembled markdown to LaTeX. `Kramdown::Converter::AsfVolumeLatex` recognizes the role-prefix italic convention on H2/H3 (`*Preface*`, `*Part*`, `*Appendices*`, `*Chapter*`) and the segment-header metadata block (parsed into kramdown IAL by a preprocessor); emits `\addchap` / `\part` / `\chapter` / `\segmenthead` / `\segmentappendixchapter` and the `\mainmatter` boundary. Resolved cross-refs of form `[Type N.M](#slug)` become `\cref{seg:slug}` (bare number via existing crefformat). HTML anchors `<a id="…"></a>` are suppressed. Currently emitted as a side-by-side artifact (`mono/.build/<slug>/body-typeset.tex`) on every build — the legacy direct-walk LaTeX path still drives the actual PDF compile.

    **What remains for Stage 3 switchover:** (a) Segment-internal subhead transitions in `AsfVolumeLatex` need parity with the per-segment `AsfLatex` flow — specifically the `\begin{segmentwidesection}` wrapper for Discussion/Findings H5s and the `\begin{workingnotes}` wrapper for Working Notes H5 (the parent converter triggers these by `el.options[:level] == 2`, which doesn't match the bumped levels in the assembled markdown). (b) `main.tex` simplification — drop `\input{frontmatter}` and the explicit `\mainmatter`/`\setchapterstyle{kao}` (the typeset output emits these itself at the part/appendices boundary). (c) Wire `typeset` as the sole `body.tex` source in `bin/build-monograph`, retire `assemble_body`. (d) Verify all four PDFs are equivalent to or improved over the current legacy output before deleting the legacy code path.

    The Stage 3 side-by-side artifact is exercised on every build and any breakage surfaces immediately. Switchover is a focused next-session task: handle the subhead transitions, then flip the bin/build-monograph default and compare PDFs.
