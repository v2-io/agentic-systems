...
**Not yet started:**
- 🚧 **Markdown-first restructure (in progress)** — design at [`msc/markdown-first-pipeline.md`](msc/markdown-first-pipeline.md). The pipeline restructures around a chunked intermediate: `source-outline + source-segments → index + chunks → assembled markdown → LaTeX → PDF`. The chunked intermediate solves three problems in one move: it removes the duplication between the (retired) Python `bin/build` and the Ruby `build-monograph` walks; it makes incremental builds intrinsic (per-chunk hashes); it commits to the consolidated markdown as the canonical citable artifact with the PDF as one rendering of it. Implementation lands in three stages — Stage 1 (ingest: source → chunks + index), Stage 2 (assemble: chunks + index → assembled markdown), Stage 3 (typeset: assembled markdown → LaTeX → PDF). Each stage commits separately and the build still works after each.
