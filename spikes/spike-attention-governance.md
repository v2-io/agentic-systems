...
**Core observation**: AAT's scope condition requires observations exist (𝒪 ≠ ∅) and residual uncertainty (H > 0), but says nothing about **finite channel capacity**. Every real agent has limited attention — it can't monitor everything simultaneously. This constraint is load-bearing for the theory and may need first-class treatment.
...
3. **Preemptive reorientation triggers.** What makes an ambient signal "break through" and demand attention? AAT has no mechanism for signals from unmonitored regions to interrupt the current processing focus.
...
This is the "governance of reorientation" we identified as missing from AAT. It's a meta-loop that monitors the quality of the primary loops and intervenes when coupling quality degrades.
...
Currently, AAT has CIY (Causal Information Yield) which quantifies the information value of an action. But CIY doesn't distinguish between actions that produce information about the current strategy's execution (within attended scope) and actions that produce information about currently unmonitored regions (outside attended scope).
...
The missing piece is exactly what AAT could provide: a severity-proportional response mapping grounded in the theory's quantities:

| Signal severity | Interpretation | Response | AAT mapping |
|---|---|---|---|
...
- Finite attention is a real and load-bearing constraint that AAT should address
