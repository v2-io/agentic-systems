...
This is a **truth-finding** sub-spike, not a paper-novelty audit. Per AAD's prior-art-integration discipline (`CLAUDE.md` §Prior art integration: *"AAD's contribution is integration, not invention. The individual pieces are mostly known; the synthesis is the contribution. Trying to make every piece unique is NIH syndrome."*), the working aim is to find **what is true** about how the wrapping construction relates to prior frameworks — what those frameworks already establish, what overlaps, what is genuinely structurally distinct, and where the wrapping move *is the same as* a known move that should simply be cited and adopted.
...
- Subramanian-Sinha-Seraj-Mahajan 2020, "Approximate information state for approximate planning and reinforcement learning in partially observed systems" (`Sub20`). Already flagged in `Novelty_defense_and_integration.md` as the strongest direct formal neighbor to AAD's bridge lemma.
...
**Already mapped** by `spikes/temporal-nesting-rg/03-rg-0c-strategy-recursion.md` §6 ("Q9: Distinction from HTN / Options / MAXQ — honest scope"), which concluded that the (O, Σ) recursion in AAD is a typed instance of HTN/options/MAXQ-style hierarchical decomposition. The honest scope statement lifted from that verdict:

> "the recursion, taken on its own, is essentially the same move as options / MAXQ. AAD's contribution is the typing of the move (AND/OR, single-parameter credences, identifiability, Correlation Hierarchy, well-formedness)."
...
**But the architectural commitment itself — separating $M$ from $G$ from action — is established prior art.** AAD's wrapping construction is *rediscovering and formalizing* what the cognitive-architectures community has had for decades. This is fine and worth crediting clearly.

**Verdict for Q8**: **Substantial overlap**. The architectural commitment (modular agent with separated belief / goal / action state) is established prior art across SOAR, ACT-R, CLARION, GWT and others. AAD's wrapping construction adds: type-level formalization, bridge-lemma error bounds, LLM-specific admissibility conditions, Brooks-Law tempo decomposition. The integration with the rest of AAD machinery is the contribution; the wrapping move itself is rediscovery / formalization of a well-established cognitive-architectures pattern.

**Citation recommendation**: AAD should cite SOAR (Laird 2012 as canonical), ACT-R (Anderson 2007), CLARION (Sun 2016), GWT (Baars 1988; Dehaene 2014) explicitly when introducing the wrapping construction. The framing should be: "The wrapping construction formalizes a pattern long-established in cognitive architectures (SOAR, ACT-R, CLARION, GWT). AAD's contribution is the type-level formalization of directed separation, the bridge-lemma cost accounting, and the integration with the rest of the AAD machinery (sector-Lyapunov persistence, tempo-composition, etc.). The architectural commitment itself is not novel."
...
**This is exactly the wrapping construction's directed separation**, in canonical Bayesian-decision-theory vocabulary that predates AAD by 60 years.
...
3. **MDP-homomorphism / approximate state abstraction** (Ravindran-Barto 2004 onward): provides the bridge-lemma vocabulary AAD needs but operates on already-modular systems; wrapping adds the constructive-modularity-around-non-modular-core aspect.
...
# **(V1) Substantial overlap with one or more existing frameworks. AAD's contribution is in the integration with the rest of the AAD machinery, not in the wrapping move itself. Cite generously, claim integration only.**

The wrapping construction's load-bearing move (type-level enforcement of directed separation by goal-blindness of $f_M$) is established prior art in two formal traditions (POMDP Bayesian belief-state architecture; cognitive-architectures modular design) and is naturally expressible in a third (categorical lens framing). AAD does not invent the move; it formalizes a pattern that has been embodied in several prior frameworks for decades.

**What AAD does add** (the integration content that justifies promoting the wrapping construction to a formal AAD result):
...
- **Integration with sector-Lyapunov persistence** (`#result-sector-persistence-template`) — the wrapper inherits the persistence condition once it satisfies (A1)–(A4). This is the load-bearing AAD-specific consequence.
...
When the wrapping construction lands as an AAD segment (per the brief's §6 plan, probably `#der-class-coercion-via-wrapping` or `#result-class-coercion`), the segment's prior-art positioning should:
...
- The claim that AAD's wrapping construction is rediscovery + formalization is *not* a weakness. AAD's stated contribution is integration, not invention. Documenting the rediscovery clearly is the honest move.
