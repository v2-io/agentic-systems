...
### Paper 1: `01-tragedy-confident-agent` ←→ AAT source segments
...
### Paper 3: `03-llm-hallucinate-bound` ←→ AAT source segments
...
- **Whether the no-go-forces-axiom pattern is its own meta-pattern or a refinement of M1**. The shape is structurally aligned with M1 (external theorem → AAT machinery as unique escape), but it has a constructive twist (the AAT axiom that the no-go forces is the load-bearing one *for an AAT-internal theorem*, not just a no-go on inference). Could go either way.
