...
4. **Cycle-close spike moves (2026-08-22 drain, first-hand).** `spike-rho-factorization` was filed, then **un-filed the same day**: the no-go is in canon but the spike's positive fine-split content (§5.4 entanglement, §11 small-$\Delta$ regime, §8.2 R-KL) is exactly what `#internal-external-decomposition` marks "under separate investigation" — a landing, not a filing op. **Landed and filed later the same day** as `#deriv-mismatch-budget-attribution` (verifier ≠ author; INDEX Spike H row). **Held, not filed:** `spike-rho-additive-variance-strengthening-2026-04-24` — `#rho-decomposition` (the AV-theorem appendix it recommended) **never existed**; distinctive payload is `orphaned`, heavy landing, not a filing op. `spike-neutral-drift-endogenous-coupling-strengthening-2026-04-24` — §8/§10.1 in canon; remainder beyond that thread still lives only in the spike (its own header says so); held. (The resolution / triage / rho-recheck feed spikes were already moved.)
