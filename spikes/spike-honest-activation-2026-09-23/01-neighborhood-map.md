...
- **One minute later, commit `11747fbf`**: the row enters `04-eli-core/OUTLINE.md` as `exploratory`.
- **2026-05-01, commit `8ed2a736`** (the Part III/IV restructure, authored under Joseph's name): *"not a virtue commitment"* is appended to the row, and the Vol 4 stage note says such rows "follow from PROPRIUM-A-v2's failure-modes analysis". The status later changed to `missing`. So clause 2's virtue-versus-physics contrast was added in Joseph's restructure, not by the auditor.

**Reading.** The row fuses two unrelated upstream things, an experiment-ethics protocol name and a PROPRIUM failure mode, through an auditor's ideation. The auditor's implied mechanism is *detected* manipulation causing defensive distrust: the $U_o \to \infty$ ("nihilism") mode of gain collapse. Joseph's 2026-09-23 reframe concerns *undetected* manipulation with inflated trust. Revision 1 of `03` finds that undetected deception realizes *both* of canon's modes, depending on the trust-model class (dogmatism under correlation neglect; nihilism toward honest channels under liar-component models). The wording has no special claim on the result, and neither does its provenance.
...
1. **Row vs Kalman: a tension that turned out not to be one.** In the regime where the gain formula is exact, the gain sequence does not depend on observed values (S0). The first version of this map concluded that the row is "false there", adopting the brief's guess. That was wrong. Canon defines gain collapse by *inappropriateness* ($U_M \to 0$ relative to the actual error), and S0 compares a gain sequence to itself. Under canon's definition the row is conditionally true (`03` R2). S0 locates the mechanism; it does not refute the clause. (Corrected after `de-novo-feedback-1.md` F1.)
2. **`#obs-developmental-trajectory`: which calibration protects.** The segment says $U_M$ *calibrates*, and calibrated is not the same as low; a calibrated $U_M$ is exactly what deception destroys (R2). So the segment is closer to consistent with this spike than the first version of this map said. What it leaves unstated is *which* calibration protects: source-conditioned calibration of the trust model, not the level of $U_M$. (Softened after F10.)
