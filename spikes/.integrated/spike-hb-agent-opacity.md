...
related:
  - spikes/spike-hafez-integration-audit.md
  - spikes/spike-composition-no-go.md
  - spikes/spike-strategic-composition.md
  - spikes/spike-interaction-channels.md
...
The Jacobian-level B1 strengthening spike (`spikes/spike-jacobian-b1-strengthening.md`) proposes parameterization-invariance (PI) as a candidate fourth meta-pattern instance, extending `#scope-agent-identity`'s token-level commitment. (PI) says AAD predictions should not depend on arbitrary coordinate choices.
...
- **`spikes/spike-hafez-integration-audit.md` §Gap 1** — mark as resolved; the spike's recommendations (2, 3) on $H_b$ gap promotion and IDT-as-Class-2-architecture-reference are addressed by the `#der-agent-opacity` segment and the `#der-directed-separation` cross-reference.
...
- **Čencov 1982, *Statistical Decision Rules and Optimal Inference*.** The (PI) / parameterization-invariance candidate fourth meta-pattern, per `spikes/spike-jacobian-b1-strengthening.md`. §11 shows $H_b$ is coordinate-free at the action-marginal level, compatible with any (PI)-extension of `#scope-agent-identity`.
