...
**Status (2026-08-22): LANDED.** The no-go was already canon (`#internal-external-decomposition`). The positive content landed in the new appendix segment `#deriv-mismatch-budget-attribution` (`01-aat-core/src/`), restated on the exact three-term decomposition of `#result-mismatch-decomposition` rather than this spike's (R-V) form: §5.4 / §12 entanglement and "policy is a measure change" → Proposition 1 (policy-as-measure; exact under GA-1 + chronica-measurable policy) and Corollaries 1a–1c; §11 small-$\Delta$ regime → Proposition 2 (exact log form, first-order log-additivity, remainder $\le x^2/4$); §6.3 / §8.2 / §12 KL form → Proposition 3 (exact, no flatness needed — strictly stronger than (R-KL)) and Proposition 4 (class-ceiling Pythagorean split, conditional on e-flat class); §§3, 5 worked cases → Discussion anchors (well-specified Kalman / LQR invariance; misspecified-drift entanglement); §4 Beta-Bernoulli → one Discussion remark at the strategy layer (note: §4.2's per-observation variance is off by a factor of 4 — $\operatorname{Var}[\ell(y)] = 4p^\ast(1-p^\ast)\ell_0^2$; the structural point stands); §7 → sentence added to `#disc-additive-coordinate-forcing` §Variance-additive cases. Deliberately **not** landed: (R-V) with cross terms and the $\Delta_\pi^2$ term (superseded — cross terms vanish on the exact decomposition; the "cross" is the policy-dependence of the measure), the reference-agent choices for $\rho_{\text{external}}$ (dissolved — the floor is a kernel, not a scalar), and the parent-spike revision recommendations of §13 (the parent was rebuilt on the exact decomposition 2026-05-18). Independently verified (confirmer ≠ author, refute posture, every proposition re-derived; four required fixes applied before commit `9d2e0c9`). Reasoning trail only from here.*

## 1. Problem statement

The internal-external-decomposition spike (`spikes/spike-internal-external-decomposition.md`) rests on a working hypothesis:
...
The parent spike (`spikes/spike-internal-external-decomposition.md`) assembles $\mathcal V = \mathcal V_E + \mathcal V_I$ with specific constituent terms. Let me trace how each component is affected.
