...
4. **Write the source-class classifier.** Map file path → source-class. `*/src/*.md` → `segment`; top-level `*.md` → `top-level-narrative`; `msc/AUDIT-WORKING-*/**/*.md` → `audit-working`; `msc/**/*.md` (other) → `working-artifact`; `.archive/**` → `archived`; `audits/**` → `audit-finding`. ~30 lines.
