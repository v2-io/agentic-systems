...
**Pressure point.** The Class-2 bias bound in `#result-section-ii-survival` (line 109) and its refined form in `#scope-observation-ambiguity-modulation` (line 43):
...
*(H1) Statistical-manifold sub-case.* $\mathcal M$ is parameterized such that each $M_t \in \mathcal M$ corresponds to a probability distribution $P_{M_t}$ over world-states. Under (PI) from `#scope-agent-identity`, the canonical metric is Fisher-Rao (Čencov 1982).
...
**M1 — Preferred: new appendix segment `#deriv-observation-ambiguity-bias-bound`.** House the full derivation (Attempts A + B + E) in an appendix segment with:
- *type:* `derivation`
...
**M2 — Update `#result-section-ii-survival` and `#scope-observation-ambiguity-modulation`.** In both segments, replace the Working-Notes line *"Without the constant, the bound is order-of-magnitude guidance, not a theorem"* with a cross-reference to `#deriv-observation-ambiguity-bias-bound`. Upgrade the bias-bound statement's epistemic tier from *order-of-magnitude* to *conditional (exact under H1-H3 or H1+H4)*.

**M3 — Candidate fifth `#disc-identifiability-floor` instance.** The no-go for universal $C$ under non-(PI) norms (Attempt E) is a candidate Instance-4 for `#disc-identifiability-floor`. The floor is: without a canonical metric on $\mathcal M$, no universal information-to-distance constant exists. The unique escape is (PI) + Čencov-forced Fisher-Rao. Composition with existing Instance 3 (`#der-agent-opacity` candidate) would complete a four-instance meta-segment pattern.

**M4 — Shared KL-to-displacement lemma.** The Pinsker / Talagrand-T2 / LSI machinery is now used in two places: `#deriv-variational-sector-condition` (KL → sector-constant degradation) and the proposed `#deriv-observation-ambiguity-bias-bound` (KL → posterior displacement). Factoring the shared machinery into a *KL-to-state-distance template* (analogous to `#result-sector-persistence-template`) would be a clean structural move. This is a portfolio-level proposal worth adding to `msc/architectural-proposals-2026-04-22.md`.
...
| Candidate fifth instance of `#disc-identifiability-floor` | **Hypothesis** | Structural fit looks clean but no explicit promotion yet |
| Candidate shared KL-to-displacement template | **Discussion-grade** | Observation about structural sharing between `#deriv-variational-sector-condition` and proposed `#deriv-observation-ambiguity-bias-bound` |
