...
1. **The loop gives Level 2** (TF-02 / [`#der-loop-interventional-access`](../../01-aad-core/src/der-loop-interventional-access.md)) — derived, AAD-internal.
...
This is a non-trivial refinement of AAD's Pearl-response. It also has implications for the bias-bound machinery ([`#scope-observation-ambiguity-modulation`](../../01-aad-core/src/scope-observation-ambiguity-modulation.md), [`#deriv-observation-ambiguity-bias-bound`](../../01-aad-core/src/deriv-observation-ambiguity-bias-bound.md)): the $\kappa \cdot \mathcal{A}$ bound applies to *fresh* Level 2 access in the loop; the language-encoded Level 2 content is on a different epistemic footing (asserted by a different agent — the speaker — and inherited).
...
- **Primary landing** — a new appendix-grade segment under [`01-aad-core/src/`](../../01-aad-core/src/), candidate slug `#deriv-pearl-level2-language-encoding` or `#deriv-discourse-pearl-encoding`, with the theorem statement and proof under three postulates. This segment becomes the structural anchor `msc/llm-causal-access-note.md` Response 2 currently lacks.

- **Secondary landing** — Discussion expansion in [`#der-loop-interventional-access`](../../01-aad-core/src/der-loop-interventional-access.md) noting that the loop's Level-2 contribution is *fresh* Level 2 (on top of language-encoded Level 2), with cross-reference to the new appendix.
