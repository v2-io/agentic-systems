...
- `spikes/spike-disturbance-model-split.md:157-167` applies Markov's inequality to a fixed/stationary tail probability `P(||δ|| > R)`, not to `P(τ_R < ∞)`.
- `audits/pending-findings-2026-04-25.md:37-48` tracks a different Model S issue: the discrete-to-continuous variance gap, not the ever-exit probability.
