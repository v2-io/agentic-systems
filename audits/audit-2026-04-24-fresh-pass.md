...
The framework appears partly aware of this — `audits/pending-findings-2026-04-22.md` identifies the C-iii vs $G_c=(O_c,\Sigma_c)$ gap (per Codex's msc check). But the resolution hasn't landed in segments.
...
**Status:** real mathematical error in a worked example. Per Codex's msc check, the same error appears in `spikes/spike-strategic-composition.md` — the spike was unreviewed rather than the segment compressing away a correction. **Confidence: high (math is direct).**
