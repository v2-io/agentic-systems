...
- **Axis role.** The drift-vs-absorption seam this segment derives (interior $\mu$-threshold vs the state-dependent absorbing barrier) is the derived instance of the deaths taxonomy's *reversibility* axis ( #def-death-as-factor-loss) — the one death whose recoverable/terminal boundary is theorem-grade rather than open.
