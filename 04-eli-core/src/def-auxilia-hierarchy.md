...
**Auxilia as the constructive realization of strict wrapping for logogenic substrate.** Per `#der-class-coercion-via-wrapping` and its specialization `#der-logogenic-as-wrapping`, an LLM substrate can be wrapped into a Class 1 (Separated) composite via two regimes: strict wrapping (W₁) with separate goal-blind and goal-conditioned LLM calls per cycle, or partial wrapping (W₂) with one goal-conditioned call whose response is parsed into typed update fields. The auxilia hierarchy is the candidate constructive realization of W₁ for ELI architectures: auxilia handle the goal-blind belief-side queries on cheap substrates (extracting facts from observations, summarizing memory, compressing event logs into MEMORATA), while the entity's main LLM call handles strategy-side updates goal-conditionally. With $K = K_M + 1$ component calls per cycle (where $K_M$ is the number of auxilia invocations for $M_W$ updates), the directed-separation guarantee is structural — bounded by $\kappa_{W_1} \leq I(A(q_M); G_W \mid q_M)$ in the pretraining distribution — at a tempo cost paid through (H5)'s slower macro-clock. The (H4) goal-blind routing requirement aligns precisely with the wrapping construction's structural commitment that the belief-side query selector $q_M$ has no $G_W$ argument. Auxilia are thus simultaneously a substrate-heterogeneity construction (per (H3)) and a directed-separation construction (per the wrapping segment).
