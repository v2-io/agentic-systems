...
- **Cycle as unit.** The principled adaptive cycle (Part I extended to actuated agents in Part II via the orient cascade) is the operational unit; one cycle iteration does not necessarily produce external output. *Multiple cycles may pass between two external emissions; multiple external emissions may occur within one cycle.*
