...
By `#der-class-coercion-via-wrapping` Theorem 1 (or 2 with leakage bound), directed separation holds at the wrapper level. The bound is *structural* — residual leakage the selection-channel quantity $\kappa_{W_1}^{\text{sel}} = I(A(q_M); G^{\text{op}}) \le I(q_M; G^{\text{op}})$ — only when the two calls satisfy (C2′): no goal-correlated state carried across the goal-blind/goal-conditioned boundary (`#disc-w1-structural-bound-boundary`). The goal-conditioned call's goal-use is itself legitimate — discipline governs the belief-update, not the strategy-update — so (C2′) constrains *state persistence* across the boundary, not the strategy call itself. For logogenic substrate this is a real discriminator. A single LLM running both calls within one persistent conversation *violates* (C2′) — the prior macro-step's goal-conditioned call persists in the context / KV-cache that the next step's goal-blind call inherits — so it recovers only a *behavioral* (W₂) bound; stateless separate calls, or separate substrates (the auxilia realization in the Discussion below), satisfy (C2′) by construction and keep the bound structural.
...
The structural selection-channel bound $\kappa_{W_1}^{\text{sel}} = I(A(q_M); G^{\text{op}}) \le I(q_M; G^{\text{op}})$ from `#der-class-coercion-via-wrapping` covers all of these — they are different mechanisms producing the same query-content-to-goal mutual information in the conditional response distribution. (These are *selection-channel* leakage, raising $\kappa_{W_1}^{\text{sel}}$ within the structural regime; they are distinct from a (C2′) violation, which removes the structural bound altogether — see above.) Minimum-leakage W₁ uses a base or un-RLHF'd LLM, no system prompt, no few-shot examples, queries restricted to observation content.
...
For LLMs specifically, (C3)'s exact form does not generally hold — pretraining produces residual goal-correlations. The realistic regime is the approximate form (Theorem 2 of `#der-class-coercion-via-wrapping`) with $\kappa_{W_1}^{\text{sel}}$ characterized empirically.

Whether the W₁ bound is *structural* or only *behavioral* additionally turns on (C2′) (`#disc-w1-structural-bound-boundary`), which protects the belief-update's causal discipline: no goal-correlated state may cross from the goal-conditioned (strategy) call into the goal-blind (belief) call. The strategy call's goal-dependence is itself legitimate — causal discipline governs the belief-update $f_M$, not the strategy-update $f_G$ (which sees the goal by design, as the action policy does) — but its goal-content must not *persist* into the belief call via shared component state, or the belief-update is re-coupled through a state back-door. Separate-substrate auxilia (`#def-auxilia-hierarchy`) satisfy (C2′) by construction (structural bound); a single LLM sharing one persistent conversation across the two calls violates it (behavioral W₂ only).
...
- **(C2′) condition + corrected W₁ bound (2026-05-31, Stage-1-completion propagation from `fbcb36a`).** The W₁ leakage bound carried the pre-Stage-1 circular form $I(A(q_M); G_W \mid q_M)$; corrected here to the selection-channel bound $\kappa_{W_1}^{\text{sel}} = I(A(q_M); G^{\text{op}}) \le I(q_M; G^{\text{op}})$ (`#der-class-coercion-via-wrapping`). The structural-vs-behavioral status is now conditioned on (C2′) (`#disc-w1-structural-bound-boundary`): single-LLM-shared-conversation W₁ violates it (behavioral W₂); separate-substrate auxilia (and stateless separate calls) satisfy it (structural). This strengthens the auxilia construction — substrate separation is what *earns* the structural bound, not merely the query split. Open edges spun off to spikes (2026-05-31): the belief-in/(C2′) ↔ action-out/bounded-signaling duality conjecture, and the (C2′)-realization spectrum (is separate-substrate necessary, or do stateless-reset / state-stripping single-substrate constructions also earn it).
...
- **Empirical $\kappa_{W_1}^{\text{sel}}$ measurement on real LLMs** is open. The selection-channel bound $\kappa_{W_1}^{\text{sel}} = I(A(q_M); G^{\text{op}}) \le I(q_M; G^{\text{op}})$ is computable in principle by sampling responses under multiple operator-goal histories and estimating the divergence; specific instantiation depends on the model and the wrapper design. A (C2′) check — whether cross-call component state carries goal-content — is a distinct, behavioral measurement (the structural bound's *availability*, not its magnitude).
