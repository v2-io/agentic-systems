...
- **W₁ bound — regression guard.** Do not regress the selection-channel bound $\kappa_{W_1}^{\text{sel}} = I(A(q_M); G^{\text{op}}) \le I(q_M; G^{\text{op}})$ (`#der-class-coercion-via-wrapping`) back to the circular form $I(A(q_M); G_W \mid q_M)$, and keep the structural-vs-behavioral status conditioned on (C2′) (`#disc-w1-structural-bound-boundary`). (Cycle narrative: CHANGELOG 2026-05-31.)
- **Open edges (forward pointers).** Two follow-ons remain open at top level in `spikes/`: the belief-in/(C2′) ↔ action-out/bounded-signaling duality (`spikes/spike-goal-flow-duality-2026-05-31.md` — discussion-grade landing available, deferred); and the (C2′)-realization spectrum — whether separate-substrate is *necessary*, or stateless-reset / state-stripping single-substrate constructions also earn the structural bound (`spikes/spike-c2prime-realization-spectrum-2026-05-31.md` — verdict: sufficient-but-not-necessary; bears on a `#def-auxilia-hierarchy` over-claim, see `TODO.md`).
