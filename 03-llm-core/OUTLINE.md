...
- **Conditional theorems** — the bias bound under named sub-scopes ( #scope-observation-ambiguity-modulation, #deriv-observation-ambiguity-bias-bound); the directed-separation architectural classification ( #der-directed-separation).
...
| L   | Scope       | [#scope-channel-collapse](src/scope-channel-collapse.md)                                                     | Observation and action channels share substrate (token sequences in same vocabulary) — the architectural condition that defines this part | draft |
...
| L   | Discussion  | [#disc-framework-self-diagnostic](src/disc-framework-self-diagnostic.md)                                     | The recursive feature: AAT applies recursively to agents building it; framework as its own diagnostic with empirical instances | draft |
...
| L3  | Discussion  | [#disc-deferred-observation-processing](src/disc-deferred-observation-processing.md)                        | Capture/triage/deferred-processing observation pipeline (generalizes $f_M$-at-arrival); retention/continuity/attention capacity triple — all-finite = the primitive-logogenic condition; recall-cost-gradient hypothesis | draft |
...
| L3  | Hypothesis  | [#hyp-behavioral-self-knowledge-insufficiency](src/hyp-behavioral-self-knowledge-insufficiency.md)          | A self-edit is a mechanism counterfactual on the agent's own substrate ( #deriv-mechanism-counterfactual-separation): behavioral self-knowledge, however complete, cannot predict self-edit outcomes — so safe self-modification needs white-box self-access or PO-specified edits, stratified by the wrapping hierarchy (scaffold/text = white-box surface; weights = black-box). Epistemic sibling to #der-self-referential-closure's tempo question | exploratory |
