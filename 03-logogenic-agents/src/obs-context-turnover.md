...
Model sufficiency ( #def-model-sufficiency) is discontinuous at session boundaries:
...
1. **Intra-session** (event-driven, continuous): standard AAD dynamics apply. The coupled update $X_{\tau^+} = f_{\text{LLM}}(\text{prompt}(X_{\tau^-}, e_\tau))$ ( #def-coupled-update-dynamics) processes events and evolves $X_t$. The persistence condition applies within a session.
